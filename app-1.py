...
def dashboard():
    ...
    if st.session_state.radar_mode == "GEX":
        ...
        def _render_greek_ladder(src_col, ladder_title, cs_pos, cs_neg, val_label):
            """Render a full-width VEX/CEX Greek ladder with separate Strike and
            Conversion-price columns, and annotated max positive / negative walls."""
            st.markdown(
                f'<div style="font-family:JetBrains Mono,monospace;'
                f'font-size:10px;font-weight:600;color:#FFFFFF;'
                f'letter-spacing:1.5px;text-transform:uppercase;'
                f'margin-bottom:4px;padding:8px 0 3px 0;'
                f'border-top:1px solid {T["line2"]};">{ladder_title}</div>',
                unsafe_allow_html=True)
            ...
            # ── Y-axis: "STRIKE  |  CONV PRICE" ─────────────────────────────
            # Strike and conversion price in their own clearly labelled columns
            _ylbls_l = [
            ...
            _sp_lbl_l  = f"Strike: {_sp_near:>8,.0f}   |   {equiv_label}: {_sp_near * _hm_ratio:>8,.0f}"
            fig_lad.add_shape(
            ...
            # ── Max positive wall ────────────────────────────────────────────
            # Row-sum across all DTEs to find dominant wall strike
            _row_sums  = _mat_l.sum(axis=1)
            _max_pos_i = int(np.argmax(_row_sums))
            _max_neg_i = int(np.argmin(_row_sums))
            _max_pos_sk = _sk_l[_max_pos_i]
            _max_neg_sk = _sk_l[_max_neg_i]
            _pos_lbl = f"Strike: {_max_pos_sk:>8,.0f}   |   {equiv_label}: {_max_pos_sk * _hm_ratio:>8,.0f}"
            _neg_lbl = f"Strike: {_max_neg_sk:>8,.0f}   |   {equiv_label}: {_max_neg_sk * _hm_ratio:>8,.0f}"

            fig_lad.add_shape(
                type="line", x0=-0.5, x1=4.5,
                y0=_pos_lbl, y1=_pos_lbl,
                line=dict(color=cs_pos, width=2.0, dash="solid"),
            )
            fig_lad.add_annotation(
                x=4.55, y=_pos_lbl, xref="x", yref="y",
                text=f"  MAX + WALL  {_fmt_cell(_row_sums[_max_pos_i])}",
                showarrow=False, xanchor="left",
                font=dict(size=9, color="#000000", family="JetBrains Mono", weight=700),
                bgcolor=cs_pos, borderpad=3, opacity=0.95,
            )
            fig_lad.add_shape(
                type="line", x0=-0.5, x1=4.5,
                y0=_neg_lbl, y1=_neg_lbl,
                line=dict(color=cs_neg, width=2.0, dash="solid"),
            )
            fig_lad.add_annotation(
                x=4.55, y=_neg_lbl, xref="x", yref="y",
                text=f"  MAX − WALL  {_fmt_cell(_row_sums[_max_neg_i])}",
                showarrow=False, xanchor="left",
                font=dict(size=9, color="#FFFFFF", family="JetBrains Mono", weight=700),
                bgcolor=cs_neg, borderpad=3, opacity=0.95,
            )

            _n_strikes = len(_sk_l)
            _cell_h = max(18, min(24, int(900 / max(_n_strikes, 1))))
            _fig_h  = max(480, min(1100, _n_strikes * _cell_h + 120))
            ...
            fig_lad.update_layout(
                ...
                margin=dict(t=44, r=260, b=54, l=310),
                xaxis=dict(
                    tickfont=dict(size=10, color="#FFFFFF",
                                  family="JetBrains Mono"),
                    side="top", showgrid=False, tickangle=0,
                    title=dict(text="DTE / Expiry Date",
                ...
                yaxis=dict(
                    ...
                    title=dict(
                        text="Strike Price  |  Conversion Price",
                        font=dict(size=10, color="#FFFFFF")),
                ),
                hoverlabel=dict(
                    bgcolor=T["bg2"], bordercolor=T["line_bright"],
                ...
                title=dict(
                    text=(f"<b>{ladder_title}</b>  ·  "
                          f"<span style='color:{cs_pos};'>■ MAX + WALL</span>  "
                          f"<span style='color:{cs_neg};'>■ MAX − WALL</span>  "
                          f"<span style='color:{T['amber']};'>─ SPOT</span>"),
        ...
        # Charm ladder — full width below Vanna
        _render_greek_ladder(
            "cex_cell",
            f"CHARM EXPOSURE — {asset_toggle}",
            "#4A9FFF", "#FF3860", "CEX $M")

        # OI ladder — full width below Charm
        st.markdown(
            f'<div style="font-family:JetBrains Mono,monospace;'
            f'font-size:10px;font-weight:600;color:#FFFFFF;'
            f'letter-spacing:1.5px;text-transform:uppercase;'
        ...
        if "dte" in _raw_oi.columns and "open_interest" in _raw_oi.columns:
            ...
            if _oi_strikes:
                ...
                for _jd in range(5):
                    _ed = _today + datetime.timedelta(days=_jd)
                    _dte_hdrs_oi.append(f"{_jd}d*  {_ed.strftime('%m/%d/%y')}")
                ...
                # ── Separate Strike | Conversion price columns ────────────────
                _oi_ylbls = [
                    f"Strike: {s:>8,.0f}   |   {equiv_label}: {s * _hm_ratio:>8,.0f}"
                ...
                # Max OI wall (row with highest total OI)
                _oi_row_sums = _oi_mat.sum(axis=1)
                _max_oi_i    = int(np.argmax(_oi_row_sums))
                _max_oi_sk   = _oi_strikes[_max_oi_i]
                _max_oi_lbl  = f"Strike: {_max_oi_sk:>8,.0f}   |   {equiv_label}: {_max_oi_sk * _hm_ratio:>8,.0f}"
                fig_oi.add_shape(
                    type="line", x0=-0.5, x1=4.5,
                ...
                fig_oi.add_annotation(
                    x=4.55, y=_max_oi_lbl, xref="x", yref="y",
                    text=f"  MAX OI WALL  {_fmt_oi(_oi_row_sums[_max_oi_i])}",
                    showarrow=False, xanchor="left",
                    font=dict(size=9, color="#000000",
                              family="JetBrains Mono", weight=700),
                    bgcolor="#88DDFF", borderpad=3, opacity=0.95,
                )
                _n_oi = len(_oi_strikes)
                _cell_h_oi = max(18, min(24, int(900 / max(_n_oi, 1))))
                _fig_h_oi  = max(480, min(1100, _n_oi * _cell_h_oi + 120))
                fig_oi.update_layout(
                    ...
                    title=dict(
                        text=(f"<b>OPEN INTEREST — {asset_toggle}</b>  ·  "
                              f"<span style='color:#88DDFF;'>■ MAX OI WALL</span>  "
                              f"<span style='color:{T['amber']};'>─ SPOT</span>"),
        ...
        # ── Robust GEX column resolution ─────────────────────────────────────
        try:
            _raw3 = raw_df.copy()

            # Resolve signed GEX per row using whatever columns exist
            if "call_gex" in _raw3.columns and "put_gex" in _raw3.columns:
                ...
                # Synthesise from Black-Scholes gamma × OI
        ...
        if _3d_ok:
            ...
            # customdata matches z shape: (n_dtes, n_strikes)
            ...
            # Colorscale: deep red (−GEX) → near-black (zero) → cyan/blue (+GEX)
            ...
            for _lk, _lc, _ln in [
                ...
                _zl3  = [float(_g_arr3[_idx3, i]) for i in range(len(_g3_dtes))]
                fig_gex3d.add_trace(go.Scatter3d(
                    ...
                    textfont=dict(size=10, color=_lc, family="JetBrains Mono"),
                    hoverinfo="skip",
                    showlegend=False,
                ))

            # ── Zero plane (translucent reference) ───────────────────────────
            _z_plane = np.zeros((len(_g3_dtes), len(_g3_strikes)))
            fig_gex3d.add_trace(go.Surface(
                x=_g3_strikes, y=_g3_dtes, z=_z_plane,
                colorscale=[[0, "rgba(200,220,255,0.08)"],
                            [1, "rgba(200,220,255,0.08)"]],
            ...
            fig_gex3d.update_layout(
                ...
                title=dict(
                    text=(
                        ...
                        f"Cyan/Blue = +GEX (dealers long, stabilising)  "
                        f"Red = −GEX (dealers short, destabilising)  "
            ...
            # ── Fallback: show a clear diagnostic so user knows why ────────────
            st.warning(
                f"⚠️  3-D GEX surface could not render for **{asset_toggle}**  "
                f"(strikes found: {len(_g3_strikes)}, DTE buckets: {len(_g3_dtes)}).  "
                f"The options chain may still be loading — please wait a moment and "
                f"refresh the page.")
        ...
        if not _hm2_raw.empty and "expiry" in _hm2_raw.columns:
            ...
            try:
                ...
                # ── Y-axis: separate Strike | Conversion price columns ─────────
                _hm2_ylbls = []
                for _hs in _hm2_strikes:
                    _row_gex = float(_hm2_piv.loc[_hs].sum())
                    _es_eq   = _hs * _hm_ratio
                    _gex_str = (f"  GEX:{abs(_row_gex)*1e9:>10,.0f}"
                                if abs(_row_gex) > 1e-9 else "")
                    _hm2_ylbls.append(
                        f"Strike:{_hs:>9,.0f}  |  {equiv_label}:{_es_eq:>9,.0f}{_gex_str}"
                    )
                ...
                for _msk, _mc, _mlabel in [
                    (_hm2_max_pos_sk, T["green"], "MAX +GEX WALL"),
                    (_hm2_max_neg_sk, T["red"],   "MAX −GEX WALL"),
                ]:
                    if _msk in _hm2_strikes:
                        ...
                        fig_hm2.add_annotation(
                            ...
                            text=f"  {_mlabel}  ${_msk:,.0f}",
                            showarrow=False, xanchor="left",
                            font=dict(size=9, color="#000000",
                                      family="JetBrains Mono", weight=700),
                            bgcolor=_mc, borderpad=3, opacity=0.92,
                        )
                ...
                fig_hm2.update_layout(
                    ...
                    font=dict(family="JetBrains Mono, monospace",
                              color="#FFFFFF", size=10),
                    margin=dict(t=36, r=220, b=70, l=370),
                    xaxis=dict(
                        tickfont=dict(size=10, color="#FFFFFF",
                                      family="JetBrains Mono"),
