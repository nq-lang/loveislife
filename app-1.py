    ...
    if st.session_state.radar_mode == "GEX":
        ...
        def _render_greek_ladder(lcol, src_col, ladder_title,
            ...
            with lcol:
                ...
                # ── Y-labels: asset price | ES equiv (no "Strike:" prefix) ───
                _ylbls_l = [
                    f"{s:,.0f}  |  {equiv_label} {s * _hm_ratio:,.0f}"
                    for s in _sk_l
                ]

                # ── X-axis: Mon–Fri of current week only ─────────────────────
                _weekday_names = ["Mon", "Tue", "Wed", "Thu", "Fri"]
                _dte_hdrs = []
                _col_indices = []   # which DTE buckets map to weekday columns
                _jd = 0
                while len(_dte_hdrs) < 5 and _jd < 14:
                    _cd = _today + datetime.timedelta(days=_jd)
                    if _cd.weekday() < 5:          # Mon=0 … Fri=4
                        _dte_hdrs.append(
                            f"{_weekday_names[_cd.weekday()]}  {_cd.strftime('%m/%d')}")
                        _col_indices.append(_jd)
                    _jd += 1

                # Build sub-matrix with only the weekday columns
                _n_wk = len(_dte_hdrs)
                _mat_wk = np.zeros((len(_sk_l), _n_wk), dtype=float)
                for _ci, _orig_j in enumerate(_col_indices):
                    if _orig_j < _mat_l.shape[1]:
                        _mat_wk[:, _ci] = _mat_l[:, _orig_j]
                _ann_wk = [[_fmt_cell(_mat_wk[i, j]) for j in range(_n_wk)]
                           for i in range(len(_sk_l))]

                # ── Spot highlight: bright amber row ─────────────────────────
                _sp_near  = min(_sk_l, key=lambda s: abs(s - spot_price))
                _sp_idx   = _sk_l.index(_sp_near)
                _sp_lbl_l = _ylbls_l[_sp_idx]
                ...
                # Spot row — solid bright amber highlight line
                fig_lad.add_shape(
                    type="rect",
                    x0=-0.5, x1=_n_wk - 0.5,
                ...
                fig_lad.add_annotation(
                    x=_n_wk - 0.5, y=_sp_lbl_l, xref="x", yref="y",
                    text=f"  ▶ SPOT ${spot_price:,.2f}",
                    showarrow=False, xanchor="left",
                    font=dict(size=9, color="#000000",
                              family="JetBrains Mono", weight=700),
                    bgcolor="#FFD700", borderpad=2, opacity=1.0,
                )

                # Max positive wall
                _row_sums  = _mat_wk.sum(axis=1)
                _max_pos_i = int(np.argmax(_row_sums))
                _max_neg_i = int(np.argmin(_row_sums))
                fig_lad.add_shape(
                    type="line", x0=-0.5, x1=_n_wk - 0.5,
                    y0=_ylbls_l[_max_pos_i], y1=_ylbls_l[_max_pos_i],
                    line=dict(color=cs_pos, width=1.8, dash="solid"),
                )
                fig_lad.add_annotation(
                    x=_n_wk - 0.5, y=_ylbls_l[_max_pos_i],
                    xref="x", yref="y",
                    text=f"  +WALL {_fmt_cell(_row_sums[_max_pos_i])}",
                    showarrow=False, xanchor="left",
                    font=dict(size=8, color="#000000", family="JetBrains Mono"),
                    bgcolor=cs_pos, borderpad=2, opacity=0.93,
                )
                fig_lad.add_shape(
                    type="line", x0=-0.5, x1=_n_wk - 0.5,
                    y0=_ylbls_l[_max_neg_i], y1=_ylbls_l[_max_neg_i],
                    line=dict(color=cs_neg, width=1.8, dash="solid"),
                )
                fig_lad.add_annotation(
                    x=_n_wk - 0.5, y=_ylbls_l[_max_neg_i],
                    xref="x", yref="y",
                    text=f"  −WALL {_fmt_cell(_row_sums[_max_neg_i])}",
                    showarrow=False, xanchor="left",
                    font=dict(size=8, color="#FFFFFF", family="JetBrains Mono"),
                    bgcolor=cs_neg, borderpad=2, opacity=0.93,
                )

                _n_strikes = len(_sk_l)
                _cell_h = max(14, min(22, int(800 / max(_n_strikes, 1))))
                _fig_h  = max(500, min(1100, _n_strikes * _cell_h + 100))
                ...
                fig_lad.update_layout(
                    ...
                    font=dict(family="JetBrains Mono, monospace",
                              color="#FFFFFF", size=9),
                    margin=dict(t=36, r=130, b=36, l=155),
                    xaxis=dict(
                    ...
                    yaxis=dict(
                        ...
                        title=dict(
                            text=f"{asset_toggle}  |  {equiv_label}",
        ...
        st.markdown(
            ...
            f'VANNA · CHARM · OI  ·  0DTE–4DTE</div>',
            unsafe_allow_html=True)
        ...
        # OI ladder  (open_interest per strike × weekday DTE bucket)
        with _lad_col_oi:
            st.markdown(
                f'<div style="font-family:JetBrains Mono,monospace;'
                f'font-size:10px;font-weight:600;color:#FFFFFF;'
                ...
                f'OPEN INTEREST — {asset_toggle}</div>',
                unsafe_allow_html=True)

            _raw_oi = raw_df.copy()
            if "dte" in _raw_oi.columns and "open_interest" in _raw_oi.columns:
                # Build weekday column headers (Mon-Fri of current week)
                _wdn = ["Mon","Tue","Wed","Thu","Fri"]
                _oi_hdrs, _oi_day_offsets = [], []
                _jd2 = 0
                while len(_oi_hdrs) < 5 and _jd2 < 14:
                    _cd2 = _today + datetime.timedelta(days=_jd2)
                    if _cd2.weekday() < 5:
                        _oi_hdrs.append(
                            f"{_wdn[_cd2.weekday()]}  {_cd2.strftime('%m/%d')}")
                        _oi_day_offsets.append(_jd2)
                    _jd2 += 1
                _n_oi_cols = len(_oi_hdrs)

                _raw_oi["_dte_int"] = _raw_oi["dte"].apply(
                    lambda x: min(int(round(float(x))), 13)
                    if pd.notna(x) else 99)
                ...
                if _oi_strikes:
                    ...
                    _oi_mat = np.zeros((len(_oi_strikes), _n_oi_cols),
                                       dtype=np.float64)
                    for _ci2, _day_off in enumerate(_oi_day_offsets):
                        _bk = _raw_oi[_raw_oi["_dte_int"] == _day_off]
                        for _ii, _sk in enumerate(_oi_strikes):
                            _rws = _bk[_bk["strike"] == _sk]
                            if not _rws.empty:
                                _oi_mat[_ii, _ci2] = float(
                    ...
                    _oi_ann = [[_fmt_oi(_oi_mat[i, j])
                                for j in range(_n_oi_cols)]
                               for i in range(len(_oi_strikes))]
                    _oi_vmax = float(np.nanpercentile(
                        _oi_mat[_oi_mat > 0], 97)) \
                    ...
                    _max_oi_i    = int(np.argmax(_oi_row_sums))
                    fig_oi.add_shape(
                        type="line", x0=-0.5, x1=_n_oi_cols - 0.5,
                        y0=_oi_ylbls[_max_oi_i], y1=_oi_ylbls[_max_oi_i],
                        line=dict(color="#88DDFF", width=1.8, dash="solid"),
                    )
                    fig_oi.add_annotation(
                        x=_n_oi_cols - 0.5, y=_oi_ylbls[_max_oi_i],
                        xref="x", yref="y",
                        text=f"  MAX OI {_fmt_oi(_oi_row_sums[_max_oi_i])}",
                        showarrow=False, xanchor="left",
                        font=dict(size=8, color="#000000",
                                  family="JetBrains Mono"),
                        bgcolor="#88DDFF", borderpad=2, opacity=0.93,
                    )
                    _n_oi_s = len(_oi_strikes)
                    _cell_h_oi = max(14, min(22, int(800 / max(_n_oi_s, 1))))
                    _fig_h_oi  = max(500, min(1100, _n_oi_s * _cell_h_oi + 100))
        ...
        # Use raw_df (per-option rows). Compute signed GEX robustly.
        # then aggregate per (strike, dte_bucket).
        try:
            _raw3 = raw_df.copy()
            # ── Robust signed GEX: try every possible column layout ───────────
            if "call_gex" in _raw3.columns and "put_gex" in _raw3.columns:
                _raw3["_gex3"] = (
                    _raw3["call_gex"].fillna(0) + _raw3["put_gex"].fillna(0))
            elif "gex_net" in _raw3.columns:
                _raw3["_gex3"] = _raw3["gex_net"].fillna(0)
            elif "gex" in _raw3.columns:
                _raw3["_gex3"] = _raw3["gex"].fillna(0)
            else:
                # synthesise from BS gamma × OI × sign(flag)
                _sp_f = float(spot_price)
                if all(c in _raw3.columns
                       for c in ["gamma", "open_interest", "flag"]):
                    _raw3["_gex3"] = (
                        _raw3["gamma"].fillna(0)
                        * _raw3["open_interest"].fillna(0)
                        * 100 * (_sp_f ** 2) * 0.01 / 1e9
                        * _raw3["flag"].map({"C": 1, "P": -1}).fillna(0))
                elif all(c in _raw3.columns
                         for c in ["gamma", "open_interest"]):
                    _raw3["_gex3"] = (
                        _raw3["gamma"].fillna(0)
                        * _raw3["open_interest"].fillna(0)
                        * 100 * (float(spot_price) ** 2) * 0.01 / 1e9)
                else:
                    _raw3["_gex3"] = 0.0
            # normalise DTE
        ...
        if _3d_ok:
            ...
            _em = float(equiv_mult) if equiv_mult else 1.0
            _g_strikes_fut3 = [round(s * _em, 2) for s in _g3_strikes]
            ...
            # Colorscale matching reference: deep red → black → green→yellow→red peaks
            _gex_cs3 = [
                [0.00, "#8B0000"],
                [0.12, "#CC0033"],
                [0.22, "#FF1144"],
            ...
            fig_gex3d.add_trace(go.Surface(
                ...
                lighting=dict(
                    ambient=0.65, diffuse=0.85,
                    specular=0.30, roughness=0.35,
                    fresnel=0.15),
                lightposition=dict(x=1000, y=500, z=5000),
                contours=dict(
                    x=dict(show=False),
                    y=dict(show=False),
            ...
            for _lk, _lc, _ln in [
                ...
                fig_gex3d.add_trace(go.Scatter3d(
                    ...
                    z=_zl3,
                    mode="lines+text",
                    line=dict(color=_lc, width=5),
                    text=[_ln if i == 0 else "" for i in range(len(_g3_dtes))],
                    textfont=dict(size=10, color=_lc,
                                  family="JetBrains Mono"),
                    hoverinfo="skip",
                    showlegend=False,
                ))
            ...
            fig_gex3d.add_trace(go.Surface(
                ...
                colorscale=[[0, "rgba(200,220,255,0.07)"],
                            [1, "rgba(200,220,255,0.07)"]],
            ...
            fig_gex3d.update_layout(
                ...
                scene=dict(
                    ...
                    xaxis=dict(
                        title=dict(
                            text=f"Strike Price  ({asset_toggle})",
                    ...
                    zaxis=dict(
                        ...
                        **{k: v for k, v in _ax3g.items()
                           if k != "zeroline"}),
                    camera=dict(
                        eye=dict(x=1.55, y=-1.70, z=0.75),
                        up=dict(x=0, y=0, z=1),
                        center=dict(x=0, y=0, z=-0.1)),
                    aspectmode="manual",
                    aspectratio=dict(x=2.0, y=1.0, z=0.75),
                ),
                showlegend=False,
                font=dict(family="JetBrains Mono, monospace",
        ...
        if "call_gex" in _hm2_raw.columns and "put_gex" in _hm2_raw.columns:
            _hm2_raw["_gex_s"] = (
                _hm2_raw["call_gex"].fillna(0) + _hm2_raw["put_gex"].fillna(0))
            ...
            if all(c in _hm2_raw.columns
                   for c in ["gamma", "open_interest", "flag"]):
                _hm2_raw["_gex_s"] = (
                    _hm2_raw["gamma"].fillna(0)
                    * _hm2_raw["open_interest"].fillna(0)
                    * 100 * (_sp2 ** 2) * 0.01 / 1e9
                    * _hm2_raw["flag"].map({"C": 1, "P": -1}).fillna(0))
            else:
                _hm2_raw["_gex_s"] = 0.0
        ...
        # ── Filter to weekday expiries only (Mon–Fri of rolling 5 weekdays) ──
        if "expiry" in _hm2_raw.columns:
            _hm2_weekday_exps = set()
            _wd_count, _wd_off = 0, 0
            while _wd_count < 5 and _wd_off < 14:
                _wd_d = _today + datetime.timedelta(days=_wd_off)
                if _wd_d.weekday() < 5:
                    _hm2_weekday_exps.add(_wd_d.strftime("%Y-%m-%d"))
                    _wd_count += 1
                _wd_off += 1
            _hm2_raw = _hm2_raw[
                _hm2_raw["expiry"].isin(_hm2_weekday_exps) |
                _hm2_raw["expiry"].str.startswith("unknown", na=False)]
        ...
        if not _hm2_raw.empty and "expiry" in _hm2_raw.columns:
            ...
            try:
                ...
                for _hs in _hm2_strikes:
                    ...
                    _hm2_ylbls.append(
                        f"{_hs:.0f}  |  {equiv_label} {_es_eq:,.0f}")
                ...
                # Max +GEX and -GEX wall lines with labels
                for _msk, _mc, _mwall_lbl in [
                    ...
                    if _msk in _hm2_strikes:
                        ...
                        fig_hm2.add_shape(
                            ...
                            y0=_mlbl, y1=_mlbl,
                            line=dict(color=_mc, width=2.0, dash="solid"),
                        )
                        fig_hm2.add_annotation(
                            x=len(_hm2_exp_lbls) - 0.5, y=_mlbl,
                            text=f"  {_mwall_lbl} ${_msk:,.0f}",
                            showarrow=False, xanchor="left",
                            font=dict(size=8, color="#000000",
                                      family="JetBrains Mono"),
                            bgcolor=_mc, borderpad=2, opacity=0.92,
                        )
                ...
                _hm2_height   = max(500, min(1200,
                                             _hm2_nstrikes * 14 + 100))
                ...
                fig_hm2.update_layout(
                    ...
                    font=dict(family="JetBrains Mono, monospace",
                              color="#FFFFFF", size=9),
                    margin=dict(t=36, r=160, b=60, l=155),
                    xaxis=dict(
                        tickfont=dict(size=9, color="#FFFFFF",
                                      family="JetBrains Mono"),
                    ...
                    title=dict(
                        text=(
                            ...
                            f"Purple=+GEX  Red=−GEX  "
                            f"Gold=Spot  Green=+Wall  Red=−Wall</span>"),
