...
def dashboard():
    ...
    if st.session_state.radar_mode == "GEX":
        ...
        def _build_0dte_ladder(raw_src, col, spot=None):
            """Build 0DTE-only ladder. Returns (strikes, col_vals, strike_labels, equiv_labels)."""
            d = raw_src.copy()
            if "dte" not in d.columns:
                return None, None, None, None
            d["_dte_int"] = d["dte"].apply(
                lambda x: int(round(float(x))) if pd.notna(x) else -1)
            d = d[d["_dte_int"] == 0].copy()
            if d.empty:
                return None, None, None, None
            ...
            if not strikes_all:
                return None, None, None, None
            vals = []
            for sk in strikes_all:
                rows = d[d["strike"] == sk]
                vals.append(float(rows[col].sum()) if not rows.empty else 0.0)
            sk_labels   = [f"${s:,.2f}" for s in strikes_all]
            eq_labels   = [f"{equiv_label} ${s * _hm_ratio:,.0f}" for s in strikes_all]
            return strikes_all, vals, sk_labels, eq_labels
        ...
        def _render_greek_ladder(lcol, src_col, ladder_title,
            ...
            with lcol:
                st.markdown(
                    ...
                    f'{ladder_title}  ·  0DTE ONLY</div>',
                    unsafe_allow_html=True)

                _sk_l, _vals_l, _sk_lbs, _eq_lbs = _build_0dte_ladder(
                    _raw_hm, src_col, spot=spot_price)
                if _sk_l is None:
                    st.info("0DTE exposure data unavailable.")
                    return

                _vals_arr = np.array(_vals_l, dtype=float)
                ...
                # Max pos/neg wall indices
                _max_pos_idx = int(np.argmax(_vals_arr))
                _max_neg_idx = int(np.argmin(_vals_arr))
                _max_pos_sk  = _sk_l[_max_pos_idx]
                _max_neg_sk  = _sk_l[_max_neg_idx]

                # Key level tiles above chart
                _kl_a, _kl_b, _kl_c = st.columns(3)
                for _kc, _kl, _kv, _keq, _kclr in [
                    (_kl_a, f"Max +{val_label} Wall",
                     f"${_max_pos_sk:,.2f}",
                     f"{equiv_label} ${_max_pos_sk * _hm_ratio:,.0f}", cs_pos),
                    (_kl_b, f"Max -{val_label} Wall",
                     f"${_max_neg_sk:,.2f}",
                     f"{equiv_label} ${_max_neg_sk * _hm_ratio:,.0f}", cs_neg),
                    (_kl_c, "Spot Price",
                     f"${spot_price:,.2f}",
                     f"{equiv_label} ${spot_price * _hm_ratio:,.0f}", T["amber"]),
                ]:
                    with _kc:
                        st.markdown(f"""
                        <div style="background:{T['bg1']};border:1px solid {T['line2']};
                                    border-top:2px solid {_kclr};border-radius:4px;
                                    padding:5px 8px;margin-bottom:6px;">
                          <div style="font-family:JetBrains Mono,monospace;font-size:7px;
                                      color:{T['t3']};letter-spacing:1px;
                                      text-transform:uppercase;margin-bottom:2px;">{_kl}</div>
                          <div style="font-family:'Barlow Condensed',sans-serif;
                                      font-size:15px;font-weight:700;color:#FFFFFF;">{_kv}</div>
                          <div style="font-family:JetBrains Mono,monospace;font-size:8px;
                                      color:{_kclr};margin-top:1px;">{_keq}</div>
                        </div>""", unsafe_allow_html=True)

                # Build 2-column heatmap: Strike col | Equiv col, single value column
                # We use a table-style approach: x = [val_label], y = combined label
                # Separate Strike and Equiv as annotations via customdata
                ...
                # z shape: (n_strikes, 1) for single DTE=0 column
                _z2d = _vals_arr.reshape(-1, 1)
                _ann2d = [[_fmt_cell(_vals_arr[i])] for i in range(len(_sk_l))]
                # customdata: (n_strikes, 1, 2) → strike + equiv per cell
                _cdata = np.array([[[_sk_lbs[i], _eq_lbs[i]]]
                                   for i in range(len(_sk_l))])

                fig_lad = go.Figure(go.Heatmap(
                    z=_z2d,
                    x=["0DTE EXPOSURE"],
                    y=list(range(len(_sk_l))),  # numeric y; labels via ticktext
                    ...
                    hovertemplate=(
                        ...
                        f"<b>{equiv_label}:</b> %{{customdata[1]}}<br>"
                ...
                # Spot line
                _sp_near_idx = min(range(len(_sk_l)),
                                   key=lambda i: abs(_sk_l[i] - spot_price))
                fig_lad.add_shape(
                    type="line", x0=-0.5, x1=0.5,
                    y0=_sp_near_idx, y1=_sp_near_idx,
                    line=dict(color=T["amber"], width=2.2, dash="dot"),
                )
                fig_lad.add_annotation(
                    x=0.55, y=_sp_near_idx, xref="x", yref="y",
                ...
                fig_lad.add_shape(
                    type="line", x0=-0.5, x1=0.5,
                    y0=_max_pos_idx, y1=_max_pos_idx,
                    line=dict(color=cs_pos, width=1.8, dash="dash"),
                )
                fig_lad.add_annotation(
                    x=0.55, y=_max_pos_idx, xref="x", yref="y",
                    text=f"  +WALL",
                    showarrow=False, xanchor="left",
                    font=dict(size=8, color=cs_pos,
                              family="JetBrains Mono", weight=700),
                ...
                fig_lad.add_shape(
                    type="line", x0=-0.5, x1=0.5,
                    y0=_max_neg_idx, y1=_max_neg_idx,
                    line=dict(color=cs_neg, width=1.8, dash="dash"),
                )
                fig_lad.add_annotation(
                    x=0.55, y=_max_neg_idx, xref="x", yref="y",
                    text=f"  -WALL",
                    showarrow=False, xanchor="left",
                    font=dict(size=8, color=cs_neg,
                              family="JetBrains Mono", weight=700),
                    borderpad=2, opacity=0.95,
                )

                _n_strikes = len(_sk_l)
                _cell_h = max(18, min(26, int(640 / max(_n_strikes, 1))))
                _fig_h  = max(420, min(900, _n_strikes * _cell_h + 140))

                # Build ticktext: "Strike | Equiv" as two clearly separated columns
                _tick_labels = [
                    f"<b style='color:#FFFFFF'>{_sk_lbs[i]}</b>  <span style='color:{T['t3']}'>{_eq_lbs[i]}</span>"
                ...
                fig_lad.update_layout(
                    ...
                    margin=dict(t=44, r=100, b=54, l=220),
                    xaxis=dict(
                    ...
                    yaxis=dict(
                        ...
                        tickmode="array",
                        tickvals=list(range(_n_strikes)),
                        ticktext=[f"{_sk_lbs[i]}  {_eq_lbs[i]}"
                                  for i in range(_n_strikes)],
                        title=dict(
                            text=f"Strike  |  {equiv_label} Equiv",
        ...
        st.markdown(
            ...
            f'{asset_toggle}  ·  DEALER GREEK EXPOSURE  ·  '
            f'VANNA · CHARM · OI  ·  0DTE ONLY</div>',
            unsafe_allow_html=True)

        _lad_col_v, _lad_col_c, _lad_col_oi = st.columns(3)

        # Vanna ladder — 0DTE only
        _render_greek_ladder(
            _lad_col_v, "vex_cell",
            f"VANNA EXPOSURE — {asset_toggle}",
            "#00E5A0", "#FF3860", "VEX $")

        # Charm ladder — 0DTE only
        _render_greek_ladder(
            _lad_col_c, "cex_cell",
            f"CHARM EXPOSURE — {asset_toggle}",
            "#4A9FFF", "#FF3860", "CEX $M")

        # OI ladder — 0DTE only, separate Strike / Equiv columns
        with _lad_col_oi:
            st.markdown(
                f'<div style="font-family:JetBrains Mono,monospace;'
                f'font-size:10px;font-weight:600;color:#FFFFFF;'
                f'letter-spacing:1.5px;text-transform:uppercase;'
                f'margin-bottom:6px;padding:4px 0;">'
                f'OPEN INTEREST — {asset_toggle}  ·  0DTE ONLY</div>',
                unsafe_allow_html=True)
            ...
            if "dte" in _raw_oi.columns and "open_interest" in _raw_oi.columns:
                _raw_oi["_dte_int"] = _raw_oi["dte"].apply(
                    lambda x: int(round(float(x))) if pd.notna(x) else -1)
                _raw_oi = _raw_oi[_raw_oi["_dte_int"] == 0].copy()
                _oi_strikes = sorted([s for s in _raw_oi["strike"].unique()
                                      if spot_price * 0.90 <= s <= spot_price * 1.10])
                if _oi_strikes:
                    _oi_vals = []
                    for _sk in _oi_strikes:
                        _rws = _raw_oi[_raw_oi["strike"] == _sk]
                        _oi_vals.append(float(_rws["open_interest"].sum())
                                        if not _rws.empty else 0.0)
                    _oi_arr  = np.array(_oi_vals, dtype=float)
                    _oi_vmax = float(np.nanpercentile(_oi_arr[_oi_arr > 0], 97)) \
                        if np.any(_oi_arr > 0) else 1.0
                    _oi_max_idx = int(np.argmax(_oi_arr))
                    _oi_max_sk  = _oi_strikes[_oi_max_idx]

                    _oi_sk_lbs = [f"${s:,.2f}" for s in _oi_strikes]
                    _oi_eq_lbs = [f"{equiv_label} ${s * _hm_ratio:,.0f}"
                                  for s in _oi_strikes]

                    # Key level tiles
                    _oi_ka, _oi_kb = st.columns(2)
                    with _oi_ka:
                        st.markdown(f"""
                        <div style="background:{T['bg1']};border:1px solid {T['line2']};
                                    border-top:2px solid #88DDFF;border-radius:4px;
                                    padding:5px 8px;margin-bottom:6px;">
                          <div style="font-family:JetBrains Mono,monospace;font-size:7px;
                                      color:{T['t3']};letter-spacing:1px;
                                      text-transform:uppercase;margin-bottom:2px;">Max OI Wall</div>
                          <div style="font-family:'Barlow Condensed',sans-serif;
                                      font-size:15px;font-weight:700;color:#FFFFFF;">
                            ${_oi_max_sk:,.2f}</div>
                          <div style="font-family:JetBrains Mono,monospace;font-size:8px;
                                      color:#88DDFF;margin-top:1px;">
                            {equiv_label} ${_oi_max_sk * _hm_ratio:,.0f}</div>
                        </div>""", unsafe_allow_html=True)
                    with _oi_kb:
                        st.markdown(f"""
                        <div style="background:{T['bg1']};border:1px solid {T['line2']};
                                    border-top:2px solid {T['amber']};border-radius:4px;
                                    padding:5px 8px;margin-bottom:6px;">
                          <div style="font-family:JetBrains Mono,monospace;font-size:7px;
                                      color:{T['t3']};letter-spacing:1px;
                                      text-transform:uppercase;margin-bottom:2px;">Spot Price</div>
                          <div style="font-family:'Barlow Condensed',sans-serif;
                                      font-size:15px;font-weight:700;color:#FFFFFF;">
                            ${spot_price:,.2f}</div>
                          <div style="font-family:JetBrains Mono,monospace;font-size:8px;
                                      color:{T['amber']};margin-top:1px;">
                            {equiv_label} ${spot_price * _hm_ratio:,.0f}</div>
                        </div>""", unsafe_allow_html=True)
                    ...
                    _oi_ann  = [[_fmt_oi(_oi_arr[i])] for i in range(len(_oi_strikes))]
                    _oi_cdata = np.array([[[_oi_sk_lbs[i], _oi_eq_lbs[i]]]
                    ...
                    fig_oi = go.Figure(go.Heatmap(
                        z=_oi_arr.reshape(-1, 1),
                        x=["0DTE OI"],
                        y=list(range(len(_oi_strikes))),
                        colorscale=_oi_cs,
                    ...
                    # Spot line
                    _sp_oi_idx = min(range(len(_oi_strikes)),
                                     key=lambda i: abs(_oi_strikes[i] - spot_price))
                    fig_oi.add_shape(
                        type="line", x0=-0.5, x1=0.5,
                        y0=_sp_oi_idx, y1=_sp_oi_idx,
                        line=dict(color=T["amber"], width=2.2, dash="dot"),
                    )
                    fig_oi.add_annotation(
                        x=0.55, y=_sp_oi_idx, xref="x", yref="y",
                        text=f"  SPOT",
                        showarrow=False, xanchor="left",
                        font=dict(size=8, color="#000000",
                                  family="JetBrains Mono", weight=700),
                        bgcolor=T["amber"], borderpad=2, opacity=0.95,
                    )
                    # Max OI wall line
                    fig_oi.add_shape(
                        type="line", x0=-0.5, x1=0.5,
                        y0=_oi_max_idx, y1=_oi_max_idx,
                        line=dict(color="#88DDFF", width=1.8, dash="dash"),
                    )
                    fig_oi.add_annotation(
                        x=0.55, y=_oi_max_idx, xref="x", yref="y",
                        text=f"  MAX OI",
                        showarrow=False, xanchor="left",
                        font=dict(size=8, color="#88DDFF",
                                  family="JetBrains Mono", weight=700),
                        borderpad=2, opacity=0.95,
                    )

                    _n_oi = len(_oi_strikes)
                    _cell_h_oi = max(18, min(26, int(640 / max(_n_oi, 1))))
                    _fig_h_oi  = max(420, min(900, _n_oi * _cell_h_oi + 140))
                    fig_oi.update_layout(
                        ...
                        yaxis=dict(
                            ...
                            showgrid=False, autorange="reversed",
                            tickmode="array",
                            tickvals=list(range(_n_oi)),
                            ticktext=[f"{_oi_sk_lbs[i]}  {_oi_eq_lbs[i]}"
                                      for i in range(_n_oi)],
                            title=dict(
                                text=f"Strike  |  {equiv_label} Equiv",
                                font=dict(size=10, color="#FFFFFF"))),
                        hoverlabel=dict(
                    ...
                    st.info("0DTE OI data unavailable.")
            else:
        ...
        try:
            _raw3 = raw_df.copy()
            # Robust column detection for GEX
            if "call_gex" in _raw3.columns and "put_gex" in _raw3.columns:
                ...
                _raw3["_gex3"] = _raw3["gex_net"]
            elif "gex" in _raw3.columns:
                _raw3["_gex3"] = _raw3["gex"]
            else:
                # last resort: sum any column containing 'gex'
                _gex_cols = [c for c in _raw3.columns if "gex" in c.lower()]
                if _gex_cols:
                    _raw3["_gex3"] = _raw3[_gex_cols].sum(axis=1)
                else:
                    raise ValueError("No GEX columns found in raw_df")
            # normalise DTE
            if "dte" not in _raw3.columns and "days" in _raw3.columns:
                _raw3["dte"] = _raw3["days"]
            if "dte" not in _raw3.columns:
                raise ValueError("No DTE column found")
            ...
            # Lowered threshold: require at least 1 strike (was 2)
            _3d_ok = bool(_g3_strikes and _g3_dtes)
        ...
        if _3d_ok:
            ...
            _raw3_cols = list(raw_df.columns) if not raw_df.empty else []
            st.warning(
                f"3-D GEX surface: insufficient data "
                f"(strikes={len(_g3_strikes)}, DTEs={len(_g3_dtes)}). "
                f"Available columns: {_raw3_cols}. "
                f"Try refreshing or selecting a different asset.")

        # ── MODEL 3: Absolute GEX 2-D Heatmap — 0DTE ONLY ───────────────────
        st.markdown(
            f'<div style="font-family:JetBrains Mono,monospace;font-size:10px;'
            f'font-weight:600;color:#FFFFFF;letter-spacing:2px;'
            f'text-transform:uppercase;padding:12px 0 5px 0;'
            f'border-top:1px solid {T["line2"]};margin-top:12px;">'
            f'{asset_toggle}  ·  ABSOLUTE GEX HEATMAP  ·  0DTE ONLY  ·  '
            f'Strike × {equiv_label} Equivalent  ·  '
            f'Auto-refresh 2 min</div>',
            unsafe_allow_html=True)

        # Build 0DTE-only Strike row from raw_df
        _hm2_raw = raw_df.copy()
        _hm2_raw["_gex_s"] = _hm2_raw["call_gex"] + _hm2_raw["put_gex"]
        if "dte" not in _hm2_raw.columns and "days" in _hm2_raw.columns:
            _hm2_raw["dte"] = _hm2_raw["days"]
        _hm2_raw = _hm2_raw[_hm2_raw["dte"].notna()].copy()
        _hm2_raw["_dte_int"] = _hm2_raw["dte"].apply(
            lambda x: int(round(float(x))) if pd.notna(x) else -1)
        # 0DTE only
        _hm2_raw = _hm2_raw[_hm2_raw["_dte_int"] == 0].copy()
        _hm2_raw = _hm2_raw[
            (_hm2_raw["strike"] >= spot_price * 0.86) &
            (_hm2_raw["strike"] <= spot_price * 1.14)
        ]

        if not _hm2_raw.empty:
            # Aggregate GEX per strike (single column since 0DTE only)
            _hm2_agg0 = _hm2_raw.groupby("strike")["_gex_s"].sum().reset_index()
            _hm2_agg0 = _hm2_agg0.sort_values("strike", ascending=False)
            _hm2_strikes = _hm2_agg0["strike"].tolist()
            _hm2_vals    = _hm2_agg0["_gex_s"].tolist()
            _hm2_arr     = np.array(_hm2_vals, dtype=float)

            _hm2_vmax = float(np.nanpercentile(
                np.abs(_hm2_arr[np.isfinite(_hm2_arr)]), 97)) \
                if np.any(np.isfinite(_hm2_arr)) else 1e-9
            _hm2_vmax = max(_hm2_vmax, 1e-12)

            _hm2_max_pos_idx = int(np.argmax(_hm2_arr))
            _hm2_max_neg_idx = int(np.argmin(_hm2_arr))
            _hm2_max_pos_sk  = _hm2_strikes[_hm2_max_pos_idx]
            _hm2_max_neg_sk  = _hm2_strikes[_hm2_max_neg_idx]

            # Cell annotation
            def _fmt_gex_cell(v):
                av = abs(v)
                if av >= 1e9:  return f"{v*1e9:+.0f}"
                if av >= 1e6:  return f"{v*1e9/1e6:+.0f}M"
                if av >= 1e3:  return f"{v*1e9/1e3:+.0f}K"
                if av > 1e-12: return f"${abs(v)*1e9:,.0f}"
                return ""

            _hm2_ann = [[_fmt_gex_cell(_hm2_arr[i])]
                        for i in range(len(_hm2_strikes))]

            # Sub-table: Max +GEX, Max -GEX, Gamma Flip
            _hm2_kt1, _hm2_kt2, _hm2_kt3 = st.columns(3)
            for _ktcol, _ktlbl, _ktval, _kteq, _ktc in [
                (_hm2_kt1, "Max +GEX Wall",
                 f"${_hm2_max_pos_sk:,.2f}",
                 f"{equiv_label} ${_hm2_max_pos_sk * _hm_ratio:,.0f}",
                 T["green"]),
                (_hm2_kt2, "Max -GEX Wall",
            ...
            # Strike and Equiv labels — each in own column via ticktext
            _hm2_sk_lbs = [f"${s:,.2f}" for s in _hm2_strikes]
            _hm2_eq_lbs = [f"{equiv_label} ${s * _hm_ratio:,.0f}"
                           for s in _hm2_strikes]
            _hm2_gex_lbs = [
                (f"${abs(_hm2_arr[i])*1e9:,.0f}"
                 if abs(_hm2_arr[i]) > 1e-9 else "")
                for i in range(len(_hm2_strikes))
            ]
            _hm2_cdata = np.array([[[_hm2_sk_lbs[i], _hm2_eq_lbs[i]]]
                                   for i in range(len(_hm2_strikes))])
            ...
            fig_hm2 = go.Figure(go.Heatmap(
                x=["0DTE GEX"],
                y=list(range(len(_hm2_strikes))),
                z=_hm2_arr.reshape(-1, 1),
                colorscale=_hm2_cs,
                zmid=0,
                zmin=-_hm2_vmax, zmax=_hm2_vmax,
                text=_hm2_ann,
                texttemplate="%{text}",
                textfont=dict(size=9, family="JetBrains Mono", color="#FFFFFF"),
                customdata=_hm2_cdata,
                colorbar=dict(
                    thickness=12, len=0.70,
                    tickfont=dict(size=10, color="#FFFFFF",
                                  family="JetBrains Mono"),
            ...
            # Spot line
            _hm2_sp_idx = min(range(len(_hm2_strikes)),
                              key=lambda i: abs(_hm2_strikes[i] - spot_price))
            fig_hm2.add_shape(
                type="line", x0=-0.5, x1=0.5,
                y0=_hm2_sp_idx, y1=_hm2_sp_idx,
                line=dict(color="#FFFFFF", width=2.2, dash="dot"),
            )
            fig_hm2.add_annotation(
                x=0.55, y=_hm2_sp_idx, xref="x", yref="y",
                text=f"  SPOT  ${spot_price:,.2f}",
                showarrow=False, xanchor="left",
                font=dict(size=9, color="#000000",
                          family="JetBrains Mono", weight=700),
            ...
            # Gamma flip line
            _hm2_gf_idx = min(range(len(_hm2_strikes)),
                              key=lambda i: abs(_hm2_strikes[i] - gamma_flip))
            fig_hm2.add_shape(
                type="line", x0=-0.5, x1=0.5,
                y0=_hm2_gf_idx, y1=_hm2_gf_idx,
                line=dict(color=T["amber"], width=1.8, dash="dash"),
            )
            fig_hm2.add_annotation(
                x=0.55, y=_hm2_gf_idx, xref="x", yref="y",
                text=f"  Γ FLIP  ${gamma_flip:,.2f}",
                showarrow=False, xanchor="left",
                font=dict(size=8, color=T["amber"],
                          family="JetBrains Mono", weight=700),
                borderpad=2, opacity=0.95,
            )
            # Max +GEX line
            fig_hm2.add_shape(
                type="line", x0=-0.5, x1=0.5,
                y0=_hm2_max_pos_idx, y1=_hm2_max_pos_idx,
                line=dict(color=T["green"], width=1.6, dash="dot"),
            )
            fig_hm2.add_annotation(
                x=0.55, y=_hm2_max_pos_idx, xref="x", yref="y",
                text="  +GEX WALL",
                showarrow=False, xanchor="left",
                font=dict(size=8, color=T["green"],
                          family="JetBrains Mono", weight=700),
                borderpad=2, opacity=0.95,
            )
            # Max -GEX line
            fig_hm2.add_shape(
                type="line", x0=-0.5, x1=0.5,
                y0=_hm2_max_neg_idx, y1=_hm2_max_neg_idx,
                line=dict(color=T["red"], width=1.6, dash="dot"),
            )
            fig_hm2.add_annotation(
                x=0.55, y=_hm2_max_neg_idx, xref="x", yref="y",
                text="  -GEX WALL",
                showarrow=False, xanchor="left",
                font=dict(size=8, color=T["red"],
                          family="JetBrains Mono", weight=700),
                borderpad=2, opacity=0.95,
            )

            # Legend annotation
            _net_g_total = float(_hm2_arr.sum())
            _ann_box = (
                f"Spot: {spot_price:,.2f}  ·  "
                f"Net Γ: {_net_g_total:+.5f}B  ·  "
                f"+GEX Wall: {_hm2_max_pos_sk:,.0f}  ·  "
                f"-GEX Wall: {_hm2_max_neg_sk:,.0f}  ·  "
            ...
            _hm2_height   = max(600, min(1200, _hm2_nstrikes * 16 + 120))
            ...
            fig_hm2.update_layout(
                ...
                font=dict(family="JetBrains Mono, monospace",
                          color="#FFFFFF", size=10),
                margin=dict(t=50, r=160, b=70, l=260),
                xaxis=dict(
                    tickfont=dict(size=10, color="#FFFFFF",
                                  family="JetBrains Mono"),
                ...
                yaxis=dict(
                    ...
                    showgrid=False,
                    tickmode="array",
                    tickvals=list(range(_hm2_nstrikes)),
                    ticktext=[
                        f"{_hm2_sk_lbs[i]}  {_hm2_eq_lbs[i]}  {_hm2_gex_lbs[i]}"
                        for i in range(_hm2_nstrikes)
                    ],
                    title=dict(
            ...
            st.info("0DTE GEX heatmap data unavailable.")
