...
def dashboard():
    ...
    if st.session_state.radar_mode == "GEX":
        ...
        # ── MODEL 1b: Vanna & Charm 0-4 DTE Ladders ──────────────────────────
        # Auto-refreshed every 120s (TTL=120 on fetch). Shows Strike | ES Equiv |
        # 0DTE … 4DTE columns, value printed in cell, green above spot, red below.
        _hm_ratio = float(equiv_mult) if equiv_mult and float(equiv_mult) > 0 else 1.0
        _raw_hm   = raw_df.copy()

        # OI-weighted exposure per (strike, dte)
        ...
        def _build_dte_ladder(raw_src, col, n_dte=5, spot=None, sym_ratio=1.0):
            """
            Returns (strikes_sorted, dte_labels, matrix[strikes x n_dte])
            where matrix[i,j] = aggregated exposure at strike i for DTE bucket j.
            DTE buckets: 0,1,2,3,4 (days to expiry, calendar).
            """
            d = raw_src.copy()
            if "dte" not in d.columns:
                return None, None, None
            # bucket to integer DTE 0..n_dte-1, clip beyond
            ...
            # short date labels for each DTE column
            dte_labels = []
            for j in range(n_dte):
                exp_dt = _today + datetime.timedelta(days=j)
                dte_labels.append(f"{j}d*\n{exp_dt.strftime('%#m/%#d/%y') if hasattr(datetime.date.today(),'strftime') else exp_dt.strftime('%m/%d/%y')}")
        ...
        st.markdown(
            ...
            f'margin-top:10px;">{asset_toggle}  ·  DEALER GREEK EXPOSURE LADDERS  ·  '
            f'VANNA · CHARM  ·  0DTE–4DTE  ·  Strike | {equiv_label} | Exposure per expiry</div>',
            unsafe_allow_html=True)

        _lad_col_v, _lad_col_c = st.columns(2)

        for _lcol, _src_col, _ladder_title, _pos_cs, _neg_cs in [
            (_lad_col_v, "vex_cell",
             f"VANNA EXPOSURE  ·  {asset_toggle}  ·  Vol×Spot Sensitivity (OI-wtd)",
             "#00E5A0", "#9B7FFF"),
            (_lad_col_c, "cex_cell",
             f"CHARM EXPOSURE  ·  {asset_toggle}  ·  Delta Decay Δ/day ($M, OI-wtd)",
             "#00E5A0", "#FF3860"),
        ]:
            with _lcol:
                st.markdown(
                    f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;'
                    f'color:{T["t3"]};letter-spacing:1.5px;text-transform:uppercase;'
                    f'margin-bottom:5px;">{_ladder_title}</div>',
                    unsafe_allow_html=True)

                _sk_l, _dte_l, _mat_l = _build_dte_ladder(
                    _raw_hm, _src_col, n_dte=5, spot=spot_price, sym_ratio=_hm_ratio)

                if _sk_l is None:
                    st.info("Exposure data unavailable — market may be closed.")
                else:
                    _mat_l = np.array(_mat_l, dtype=float)
                    _vmax_l = float(np.nanpercentile(np.abs(_mat_l[np.isfinite(_mat_l)]), 97)) \
                              if np.any(np.isfinite(_mat_l)) else 1.0
                    _vmax_l = max(_vmax_l, 1e-12)

                    # Build annotated heatmap: value text inside each cell
                    def _fmt_cell(v):
                        av = abs(v)
                    ...
                    # Per-cell color: above spot = pos gradient green, below = red
                    # Use plotly Heatmap with diverging scale anchored at zmid=0
                    # Above spot row → green side, below → red side
                    _cs_ladder = (
                        [[0.0, "#FF3860"], [0.35, "#550015"],
                         [0.5,  T["bg"]], [0.65, "#003320"],
                         [1.0, "#00E5A0"]]
                        if _src_col == "vex_cell" else
                        [[0.0, "#FF3860"], [0.35, "#550015"],
                         [0.5,  T["bg"]], [0.65, "#003320"],
                         [1.0, "#00E5A0"]]
                    )

                    # Y-axis labels: Strike | Futures-equiv
                    _ylbls_l = [
                        f"${s:,.0f}  {equiv_label} ${s*_hm_ratio:,.0f}"
                        for s in _sk_l
                    ]

                    # Build DTE header labels
                    _dte_hdrs = []
                    for _jd in range(5):
                        _ed = _today + datetime.timedelta(days=_jd)
                        _dte_hdrs.append(f"{_jd}d*  {_ed.strftime('%m/%d/%y')}")
                    ...
                    fig_lad = go.Figure(go.Heatmap(
                        ...
                        text=_ann_text,
                        texttemplate="%{text}",
                        textfont=dict(size=7, family="JetBrains Mono",
                                     color="rgba(255,255,255,0.85)"),
                        colorbar=dict(
                            thickness=8, len=0.6,
                            tickfont=dict(size=7, color=T["t3"],
                                         family="JetBrains Mono"),
                        ),
                        hovertemplate=(
                            "<b>Strike:</b> %{y}<br>"
                            "<b>DTE:</b> %{x}<br>"
                            "<b>Exposure:</b> %{z:.4f}<extra></extra>"
                        ),
                        xgap=2, ygap=1,
                    ))

                    # Spot price line
                    _sp_near = min(_sk_l, key=lambda s: abs(s - spot_price))
                    _sp_lbl_l = f"${_sp_near:,.0f}  {equiv_label} ${_sp_near*_hm_ratio:,.0f}"
                    fig_lad.add_shape(
                        type="line", x0=-0.5, x1=4.5,
                        y0=_sp_lbl_l, y1=_sp_lbl_l,
                        line=dict(color=T["amber"], width=2.0, dash="dot"),
                    )
                    fig_lad.add_annotation(
                        x=4.6, y=_sp_lbl_l, xref="x", yref="y",
                        text=f" SPOT ${spot_price:,.2f}",
                        showarrow=False, xanchor="left",
                        font=dict(size=7, color=T["bg"], family="JetBrains Mono"),
                        bgcolor=T["amber"], borderpad=2, opacity=0.95,
                    )

                    _n_strikes = len(_sk_l)
                    _cell_h = max(16, min(22, int(560 / max(_n_strikes, 1))))
                    _fig_h  = max(380, min(700, _n_strikes * _cell_h + 80))
                    ...
                    fig_lad.update_layout(
                        ...
                        font=dict(family="JetBrains Mono, monospace",
                                  color=T["t2"], size=8),
                        margin=dict(t=10, r=60, b=50, l=180),
                        xaxis=dict(
                            tickfont=dict(size=7.5, color=T["t2"]),
                            side="top",
                            showgrid=False,
                            tickangle=0,
                        ),
        ...
        # Build signed GEX per row using raw_df with DTE as numeric axis
        _raw_df_3d = raw_df.copy()
        _raw_df_3d["gex_signed"] = _raw_df_3d["call_gex"] + _raw_df_3d["put_gex"]
        # Filter ±12% from spot, limit to 60 DTE
        _raw_df_3d = _raw_df_3d[
            (_raw_df_3d["strike"] >= spot_price * 0.88) &
            (_raw_df_3d["strike"] <= spot_price * 1.12) &
            (_raw_df_3d["dte"] <= 60)
        ].copy()

        if not _raw_df_3d.empty and "dte" in _raw_df_3d.columns:
            _raw_df_3d["_dte_int"] = _raw_df_3d["dte"].apply(
                lambda x: int(round(float(x))) if pd.notna(x) else -1)
            _raw_df_3d = _raw_df_3d[_raw_df_3d["_dte_int"] >= 0]
            _g3_agg = (_raw_df_3d.groupby(["strike", "_dte_int"])["gex_signed"]
                       .sum().reset_index())
            _g3_strikes = sorted(_g3_agg["strike"].unique().tolist())
            _g3_dtes    = sorted(_g3_agg["_dte_int"].unique().tolist())

            if _g3_strikes and _g3_dtes:
                _piv3 = _g3_agg.pivot(index="strike",
                                       columns="_dte_int",
                                       values="gex_signed").fillna(0)
                _piv3 = _piv3.reindex(index=_g3_strikes, columns=_g3_dtes,
                                      fill_value=0)
                _g_arr3 = _piv3.values  # shape: (n_strikes, n_dtes)

                _g_strikes_fut3 = [round(s * float(equiv_mult), 2)
                                   for s in _g3_strikes]
                _cd3 = np.array(_g_strikes_fut3 * len(_g3_dtes)).reshape(
                    len(_g3_dtes), len(_g3_strikes)).T

                _ax3g = dict(
                    backgroundcolor=T["bg1"], gridcolor=T["line2"],
                    gridwidth=1, showspikes=False, zeroline=False,
                    tickfont=dict(size=7, color=T["t3"],
                                  family="JetBrains Mono"))
                _gex_cs3 = [
                    [0.00, "#FF2244"], [0.18, "#CC0033"],
                    [0.35, "#660018"], [0.46, "#220008"],
                    [0.50, T["bg"]],
                    [0.54, "#001433"], [0.65, "#004488"],
                    [0.82, "#2266CC"], [1.00, "#88CCFF"],
                ]

                fig_gex3d = go.Figure(go.Surface(
                    x=_g3_strikes, y=_g3_dtes, z=_g_arr3.T,
                    colorscale=_gex_cs3, showscale=True, opacity=0.94,
                    customdata=_cd3,
                    colorbar=dict(
                        title=dict(
                            text="GEX ($B)",
                    ...
                    lighting=dict(ambient=0.55, diffuse=0.9,
                                  specular=0.20, roughness=0.50),
                ))
                # Spot & GammaFlip ridge lines
                for _lk, _lc, _ln in [
                    ...
                    fig_gex3d.add_trace(go.Scatter3d(
                        x=[_lk] * len(_g3_dtes), y=_g3_dtes, z=_zl3,
                        mode="lines", line=dict(color=_lc, width=3),
                        hoverinfo="skip",
                    ))

                fig_gex3d.update_layout(
                    paper_bgcolor=T["bg"], height=540,
                    margin=dict(l=0, r=0, b=0, t=30),
                    scene=dict(
                        xaxis=dict(
                            title=dict(
                        ...
                        camera=dict(
                            eye=dict(x=1.55, y=-1.65, z=0.85),
                            up=dict(x=0, y=0, z=1)),
                        aspectmode="manual",
                        aspectratio=dict(x=2.4, y=1.0, z=0.75),
                    ),
                    showlegend=False,
                    font=dict(family="JetBrains Mono, monospace",
                              color=T["t2"], size=9),
                    title=dict(
                        text=(
                            ...
                            f"Pos GEX above zero (stable) · Neg below (volatile)"
                            f"  ·  Hover = {equiv_label} price</span>"),
                        font=dict(size=10, color=T["t1"],
                                  family="JetBrains Mono"),
                        x=0.01),
                )
                st.plotly_chart(fig_gex3d, use_container_width=True,
                                config={
                                    "displayModeBar": True,
                                    "displaylogo": False,
                                    "scrollZoom": False,
                ...
                st.info("Insufficient GEX data for 3-D surface.")
        else:
            st.info("GEX surface data unavailable.")

        # ── MODEL 3: SpotGamma-style Absolute GEX 2-D Heatmap ────────────────
        # Strike Price (Y) × Expiration Date (X), color = signed GEX intensity
        # Includes: spot line, net gamma, max pos/neg gamma, gamma flip annotations
        st.markdown(
            f'<div style="font-family:JetBrains Mono,monospace;font-size:9px;'
            f'color:{T["t3"]};letter-spacing:2px;text-transform:uppercase;'
            f'padding:12px 0 6px 0;border-top:1px solid {T["line2"]};'
            f'margin-top:10px;">{asset_toggle}  ·  ABSOLUTE GEX HEATMAP  ·  '
        ...
        if "expiry" not in _hm2_raw.columns and "dte" in _hm2_raw.columns:
            _hm2_raw["expiry"] = _hm2_raw["dte"].apply(
                lambda x: (_today + datetime.timedelta(days=int(round(float(x))))).strftime("%Y-%m-%d")
                if pd.notna(x) else "unknown")
        ...
        if not _hm2_raw.empty and "expiry" in _hm2_raw.columns:
            ...
            try:
                _hm2_piv = _hm2_agg.pivot(
                    index="strike", columns="expiry", values="_gex_s").fillna(0)
                ...
                _hm2_Z   = _hm2_piv.values  # shape: (n_strikes, n_exps)
                ...
                # Net gamma per expiry column (bottom annotation)
                _net_by_exp = _hm2_piv.sum(axis=0).values

                # Y-labels: strike + futures equiv + signed net GEX
                _hm2_ylbls = []
                for _hs in _hm2_strikes:
                    _row_gex = float(_hm2_piv.loc[_hs].sum())
                    _es_eq   = _hs * _hm_ratio
                    _tag = (f"$ {abs(_row_gex)*1e9:,.0f}" if abs(_row_gex) > 1e-7
                            else "")
                    _hm2_ylbls.append(f"{_hs:.0f}   {equiv_label} {_es_eq:,.0f}   {_tag}")

                # Diverging colorscale: deep blue +GEX → black zero → deep red -GEX
                _hm2_cs = [
                    [0.0,  "#8800FF"], [0.20, "#4400AA"],
                    [0.38, "#220055"], [0.48, "#0A0010"],
                    [0.50, T["bg"]],
                    [0.52, "#150005"], [0.62, "#550015"],
                    [0.80, "#CC0033"], [1.0,  "#FF1144"],
                ]
                ...
                # Spot price line
                _hm2_sp_near = min(_hm2_strikes, key=lambda s: abs(s - spot_price))
                ...
                # Gamma flip line
                _hm2_gf_near = min(_hm2_strikes, key=lambda s: abs(s - gamma_flip))
                _hm2_gf_lbl  = _hm2_ylbls[_hm2_strikes.index(_hm2_gf_near)]
                fig_hm2.add_shape(
                    type="line", x0=-0.5, x1=len(_hm2_exp_lbls) - 0.5,
                    y0=_hm2_gf_lbl, y1=_hm2_gf_lbl,
                    line=dict(color=T["amber"], width=1.4, dash="dash"),
                )

                # Max positive & negative GEX strike markers
                _hm2_row_sum = _hm2_piv.sum(axis=1)
                _hm2_max_pos_sk = float(_hm2_row_sum.idxmax())
                _hm2_max_neg_sk = float(_hm2_row_sum.idxmin())
                for _msk, _mc, _ml in [
                    (_hm2_max_pos_sk, T["green"],  "Max +GEX"),
                    (_hm2_max_neg_sk, T["red"],    "Max -GEX"),
                ]:
                    _mlbl = _hm2_ylbls[_hm2_strikes.index(_msk)] \
                        if _msk in _hm2_strikes else None
                    if _mlbl:
                        fig_hm2.add_shape(
                            type="line",
                            x0=-0.5, x1=len(_hm2_exp_lbls) - 0.5,
                            y0=_mlbl, y1=_mlbl,
                            line=dict(color=_mc, width=1.2, dash="dot"),
                        )

                # Annotation legend box (top-right, SpotGamma style)
                _net_g_total = float(_hm2_piv.values.sum())
                _ann_box = (
                    f"Spot: {spot_price:,.2f}  ·  "
                    f"Net Γ: {_net_g_total:+.4f}B  ·  "
                ...
                fig_hm2.add_annotation(
                    ...
                    text=_ann_box, showarrow=False, xanchor="right",
                    yanchor="top",
                    font=dict(size=7.5, color=T["t1"],
                              family="JetBrains Mono"),
                    bgcolor=T["bg2"], bordercolor=T["line_bright"],
                    borderwidth=1, borderpad=5, opacity=0.92,
                )

                _hm2_nstrikes = len(_hm2_strikes)
                _hm2_height   = max(500, min(1100, _hm2_nstrikes * 14 + 100))
                ...
                fig_hm2.update_layout(
                    ...
                    font=dict(family="JetBrains Mono, monospace",
                              color=T["t2"], size=8),
                    margin=dict(t=28, r=80, b=60, l=220),
                    xaxis=dict(
                        tickfont=dict(size=7.5, color=T["t2"]),
                        side="bottom",
                        showgrid=False,
                    ...
                    yaxis=dict(
                        ...
                        showgrid=False,
                        title=dict(
                            text=f"Strike Price  /  {equiv_label}  /  Net GEX",
                            font=dict(size=8, color=T["t3"])),
                    ),
                    title=dict(
                        text=(
                            ...
                            f"<span style='color:{T['t3']};font-size:8px;'>"
                ...
                st.info(f"GEX heatmap build error: {_hm2_err}")
        else:
