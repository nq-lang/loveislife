...
def dashboard():
    ...
    if st.session_state.radar_mode == "GEX":
        ...
        # ── MODEL 1b: Vanna & Charm Exposure Heatmaps ────────────────────────
        st.markdown(
            f'<div style="font-family:JetBrains Mono,monospace;font-size:9px;'
            f'color:{T["t3"]};letter-spacing:2px;text-transform:uppercase;'
            f'padding:12px 0 6px 0;border-top:1px solid {T["line2"]};'
            f'margin-top:10px;">{asset_toggle}  ·  DEALER GREEK EXPOSURE  ·  '
            f'VANNA (vol×spot sensitivity) · CHARM (delta decay)  ·  '
            f'Strike × Expiry · {equiv_label} price shown per strike</div>',
            unsafe_allow_html=True)

        _hm_ratio = float(equiv_mult) if equiv_mult and float(equiv_mult) > 0 else 1.0

        # Build exposure-weighted columns: multiply greek by OI × 100
        _raw_hm = raw_df.copy()
        _raw_hm["vanna_exp"] = _raw_hm["vanna"] * _raw_hm["open_interest"] * 100
        _raw_hm["charm_exp"] = _raw_hm["charm"] * _raw_hm["open_interest"] * 100 / 1e6

        _hmc1, _hmc2 = st.columns(2)

        for _hw, _gk, _ht, _cs, _unit in [
            (_hmc1, "vanna_exp",
             f"VANNA EXPOSURE  ·  {asset_toggle}  ·  Vol×Spot Sensitivity  ·  OI-Weighted",
             [[0.0,"#9B7FFF"],[0.35,T["bg2"]],[0.5,T["bg"]],[0.65,T["bg2"]],[1.0,"#F5A623"]],
             "VannExp"),
            (_hmc2, "charm_exp",
             f"CHARM EXPOSURE  ·  {asset_toggle}  ·  Delta Decay Δ/day  ·  OI-Weighted ($M)",
             [[0.0,"#FF3860"],[0.35,T["bg2"]],[0.5,T["bg"]],[0.65,T["bg2"]],[1.0,"#00E5A0"]],
             "$M"),
        ]:
            _mx = _build_greek_matrix(_raw_hm, spot_price, _gk)
            with _hw:
                st.markdown(
                    f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;'
                    f'color:{T["t3"]};letter-spacing:1px;text-transform:uppercase;'
                    f'margin-bottom:4px;">{_ht}</div>',
                    unsafe_allow_html=True)
                if _mx[0] is not None:
                    _hm_s, _hm_e, _hm_Z = _mx
                    _hm_arr = np.array(_hm_Z, dtype=float)
                    # Y labels: asset strike + futures/ES equivalent
                    _hm_ylbls = [
                        f"${s:,.0f}  /  {equiv_label} ${s * _hm_ratio:,.0f}"
                        for s in _hm_s
                    ]
                    _hm_vmax = float(np.nanpercentile(np.abs(_hm_arr[np.isfinite(_hm_arr)]), 97)) \
                               if np.any(np.isfinite(_hm_arr)) else 1e-9
                    _hm_vmax = max(_hm_vmax, 1e-12)
                    fig_hm = go.Figure(go.Heatmap(
                        ...
                        colorscale=_cs,
                        zmid=0,
                        zmin=-_hm_vmax, zmax=_hm_vmax,
                        colorbar=dict(
                            thickness=10, len=0.7,
                            tickfont=dict(size=7, color=T["t3"], family="JetBrains Mono"),
                            title=dict(text=_unit,
                                       font=dict(size=8, color=T["t3"], family="JetBrains Mono")),
                        ),
                        hovertemplate=(
                            "<b>Strike:</b> %{y}<br>"
                            "<b>Expiry:</b> %{x}<br>"
                            "<b>Exposure:</b> %{z:.5f}<extra></extra>"
                        ),
                    ))
                    # Spot price marker — find closest strike label
                    _sp_idx = min(range(len(_hm_s)),
                                  key=lambda i: abs(_hm_s[i] - spot_price))
                    _sp_lbl = _hm_ylbls[_sp_idx]
                    fig_hm.add_shape(
                        type="line", x0=0, x1=1, xref="paper",
                        y0=_sp_lbl, y1=_sp_lbl, yref="y",
                        line=dict(color=T["t1"], width=1.6, dash="dash"),
                    )
                    fig_hm.add_annotation(
                        x=1.01, y=_sp_lbl, xref="paper", yref="y",
                        text=" SPOT ", showarrow=False, xanchor="left",
                        font=dict(size=7, color=T["bg"], family="JetBrains Mono"),
                        bgcolor=T["t1"], borderpad=2, opacity=0.9,
                    )
                    fig_hm.update_layout(
                        ...
                        margin=dict(t=8, r=90, b=40, l=170),
                        xaxis=dict(
                            tickfont=dict(size=7, color=T["t3"]),
                            showgrid=False,
                            title=dict(text="Expiry", font=dict(size=8, color=T["t3"])),
                        ),
                        yaxis=dict(
                            tickfont=dict(size=7, color=T["t3"]),
                            showgrid=False,
                            autorange="reversed",
                            title=dict(
                                text=f"{asset_toggle} Strike  /  {equiv_label} Equiv",
                    ...
                    st.info(f"{_gk.replace('_exp', '').capitalize()} exposure data unavailable.")
        ...
        # Build signed GEX per row (call_gex is positive, put_gex is negative)
        _raw_df_signed = raw_df.copy()
        _raw_df_signed["gex_net"] = _raw_df_signed["call_gex"] + _raw_df_signed["put_gex"]
        _gs = _build_greek_matrix(_raw_df_signed, spot_price, "gex_net")
