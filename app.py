...
def _fetch_spx_top_gex_levels(selected_asset: str = "SPX") -> dict:
    ...
    """
    # Asset chain map: (chain_ticker, futures_ticker, futures_label)
    # RATIO = futures_price / spot_native  (spot_native comes from the options chain fetch)
    # This ensures SPY strikes * ratio = correct ES price (~5600 range, not ~710 range)
    _ASSET_CHAIN_MAP = {
        "SPY": ("SPY",  "ES=F",  "ES Futures"),
        "SPX": ("SPX",  "ES=F",  "ES Futures"),
        "NDX": ("NDX",  "NQ=F",  "NQ Futures"),
        "GLD": ("GLD",  "GC=F",  "Gold/GC Futures"),
    }
    _chain_t, _fut_t, _fut_lbl_t = _ASSET_CHAIN_MAP.get(
        selected_asset, ("SPX", "ES=F", "ES Futures"))
    ...
    try:
        ...
        # CORRECT ratio: futures_price / option_chain_underlying_price (spot_native)
        # spot_native is the live price of whatever the options are written on
        # (SPY ~560, SPX ~5600, NDX ~19000, GLD ~230)
        # This gives: SPY strike 710 * (ES ~5600 / SPY ~560) = ~5680 ES  CORRECT
        try:
            _fut_px = _fetch_yahoo_price(_fut_t)
            ratio   = _fut_px / spot_native if spot_native > 0 else 1.0
        except Exception:
            ratio = 1.0
...
def render_es_chart_page():
    ...
    # ── Asset-aware futures conversion ratio for Models 2 & 3 ─────────────────
    # RULE: ratio = futures_price / option_chain_underlying_price
    #
    #   SPY -> ES=F / SPY   (SPY ~560, ES ~5600  -> ratio ~10.0)
    #   SPX -> ES=F / ^GSPC (SPX ~5600, ES ~5600 -> ratio ~1.0)
    #   NDX -> NQ=F / ^NDX  (NDX ~19000, NQ ~19000 -> ratio ~1.0)
    #   GLD -> GC=F / GLD   (GLD ~230, GC ~2300 -> ratio ~10.0)
    #
    # The denominator MUST be the price of the SAME instrument the options
    # are written on. SPY strike * (ES=F / SPY) = correct ES equivalent.
    # Using ^GSPC (SPX ~5600) as denominator for SPY gives ratio ~1.0 -- WRONG.
    _ASSET_FUT_MAP = {
        # (futures_ticker, option_underlying_ticker, futures_label)
        "SPY": ("ES=F",  "SPY",    "ES Futures"),
        "SPX": ("ES=F",  "^GSPC",  "ES Futures"),
        "NDX": ("NQ=F",  "^NDX",   "NQ Futures"),
        "GLD": ("GC=F",  "GLD",    "Gold/GC Futures"),
    }
    _fut_tk_m, _und_tk_m, _fut_lbl_m = _ASSET_FUT_MAP.get(
        _chain_asset, ("ES=F", "^GSPC", "ES Futures"))
    try:
        _fut_px_m = _fetch_yahoo_price(_fut_tk_m)
        # Use the live model spot (options chain underlying) as denominator.
        # Fall back to fetching the underlying ticker directly only if needed.
        _und_px_m = float(_model_spot) if (_model_spot and float(_model_spot) > 0) \
                    else _fetch_yahoo_price(_und_tk_m)
        _m23_ratio = _fut_px_m / _und_px_m if _und_px_m > 0 else 1.0
    except Exception:
        _m23_ratio = 1.0
    # Live futures spot — fetch directly so it's always accurate
    try:
        _fut_spot_m = _fetch_yahoo_price(_fut_tk_m)
    except Exception:
        _fut_spot_m = spot_es
    ...
    try:
        ...
        if _raw_es_data is not None and not _raw_es_data.empty:
            ...
            if not _gp2.empty:
                ...
                # Spot vertical line — yellow (at asset-native spot)
                ...
                fig_gp.add_annotation(
                    ...
                    text=f"  SPOT  ${_native_spot:,.2f}  |  {_fut_lbl_m} ${_fut_spot_m:,.2f}",
                ...
                # Key level annotations on chart
                _m2_key_levels = []
                if gex_lvls:
                    ...
                    _m2_key_levels = [
                        (_gflip_nat, T["amber"], "Zero Γ / Vol Trigger", "dot",   1.4),
                        (_cwall_nat, T["green"], "Call Wall",            "dash",  1.6),
                        (_pwall_nat, T["red"],   "Put Wall",             "dash",  1.6),
                        (_vtrig_nat, T["blue"],  "Vol Trigger",          "dashdot", 1.0),
                    ]
                for _lvnat, _lvc, _lvnm, _lvd, _lvw in _m2_key_levels:
                    if _lvnat <= 0:
                        continue
                    if _native_spot * 0.75 <= _lvnat <= _native_spot * 1.25:
                        ...
                        fig_gp.add_annotation(
                            x=_lvnat, y=0.98, xref="x", yref="paper",
                            text=f" {_lvnm} ${_lvnat:,.1f} | {_fut_lbl_m} ${_fut_at_lvl:,.1f} ",
                            showarrow=False, xanchor="center",
                ...
                fig_gp.update_layout(
                    ...
                    margin=dict(t=50, r=80, b=60, l=80),
                    showlegend=False,
                    xaxis=dict(
    ...
    st.markdown(
        ...
        f'GEX-Derived  ·  Live</div>',
        unsafe_allow_html=True)

    if gex_lvls:
        _ikl_gflip  = gex_lvls.get("gamma_flip",  None)
        _ikl_cwall  = gex_lvls.get("call_wall",   None)
        _ikl_pwall  = gex_lvls.get("put_wall",    None)
        _ikl_vtrig  = gex_lvls.get("vol_trigger", None)
        _ikl_spot   = float(_model_spot) if _model_spot else (spot_es / _m23_ratio)
        _ikl_ratio  = _m23_ratio

        # Compute futures-equivalent values
        def _f(v):
            return v * _ikl_ratio if v else None
        def _fs(v):
            return f"${_f(v):,.2f}" if v else "—"
        def _ns(v):
            return f"${v:,.2f}" if v else "—"

        # Regime score: bullish if spot > gamma_flip; score based on distance
        ...
        if _ikl_gflip and _ikl_cwall and _ikl_pwall:
            ...
            # proximity to call wall is bullish, proximity to put wall is bearish
        ...
        _rs_label = "BULLISH" if _regime_score >= 60 else ("BEARISH" if _regime_score < 40 else "NEUTRAL")

        # GEX-derived magnet strikes (top positive GEX = call magnet, top neg = put magnet)
        _top5p = gex_lvls.get("top5_pos_es", [])
        _top5n = gex_lvls.get("top5_neg_es", [])
        _magnet_call = _top5p[0]["strike_es"] if _top5p else (_f(_ikl_cwall) or "—")
        _magnet_put  = _top5n[0]["strike_es"] if _top5n else (_f(_ikl_pwall) or "—")

        # Gamma squeeze thresholds: breaking call wall → call side squeeze (upside burst)
        #                           breaking put wall  → put side squeeze  (downside burst)
        _squeeze_up   = _f(_ikl_cwall)
        _squeeze_down = _f(_ikl_pwall)
        ...
        <div style="display:flex;gap:10px;align-items:stretch;margin-bottom:12px;flex-wrap:wrap;">
          ...
          <div style="flex:1;background:{T['bg1']};border:1px solid {T['line2']};
                      border-top:3px solid {T['t3']};border-radius:4px;padding:10px 14px;">
            <div style="font-family:JetBrains Mono;font-size:8px;color:{T['t3']};
                        letter-spacing:1.5px;text-transform:uppercase;margin-bottom:6px;">
              Gamma Regime Context</div>
            <div style="font-family:JetBrains Mono;font-size:9px;color:{T['t1']};line-height:1.7;">
              {'🟢 ABOVE Zero Gamma → Long gamma regime. Dealers stabilise moves — expect mean reversion. Sell spikes, buy dips near key levels.' if _above_gflip else '🔴 BELOW Zero Gamma → Short gamma regime. Dealers amplify moves — expect trending/volatile price action. Let winners run, avoid fading.'}
            </div>
          </div>
        </div>""", unsafe_allow_html=True)
        ...
        _ikl_rows = [
            (T["amber"], "Zero Gamma / Flip",
             ...
             "Net dealer gamma = 0. The pivot between stabilising & amplifying regimes. "
             "Above = long gamma (fades moves). Below = short gamma (extends moves). "
             "Momentum/swing entries valid on acceptance through this level."),
            (T["blue"], "Volatility Trigger",
             _ns(_ikl_vtrig), _fs(_ikl_vtrig),
             "Strike of maximum absolute GEX. Crossing this level triggers intraday vol "
             "regime shift. Look for momentum holds or reversals as price approaches. "
             "Strong magnet effect — price may drift toward this level in low-volume periods."),
            (T["green"], "Call Wall",
             _ns(_ikl_cwall), _fs(_ikl_cwall),
             "Largest positive GEX cluster. Heavy dealer delta hedging creates strong "
             "resistance. Good area for rejections / short-term profit-taking. "
             "BREAKOUT ALERT: If price breaks and accepts above Call Wall → "
             "dealers forced to buy aggressively → potential gamma squeeze HIGHER."),
            (T["red"], "Put Wall",
             _ns(_ikl_pwall), _fs(_ikl_pwall),
             "Largest negative GEX cluster. Dealer hedging can create support / floor. "
             "Watch for bounces or high-volume rejections here. "
             "BREAKDOWN ALERT: If price breaks and accepts below Put Wall → "
             "dealers forced to sell aggressively → potential gamma squeeze LOWER."),
        ]
          ...
          <div style="display:flex;gap:24px;flex-wrap:wrap;">
            <div>
              ...
              <div style="font-family:'Barlow Condensed',sans-serif;font-size:15px;
                ...
                {'${:,.2f}'.format(_magnet_call) if isinstance(_magnet_call, float) else str(_magnet_call)} {_fut_lbl_m}</div>
            ...
            <div>
              ...
              <div style="font-family:'Barlow Condensed',sans-serif;font-size:15px;
                          font-weight:700;color:#FFFFFF;">
                {'${:,.2f}'.format(_magnet_put) if isinstance(_magnet_put, float) else str(_magnet_put)} {_fut_lbl_m}</div>
              <div style="font-family:JetBrains Mono;font-size:8px;color:{T['t3']};margin-top:2px;">
        ...
        <div style="background:{T['bg1']};border:1px solid {T['line2']};border-radius:4px;
          ...
          <div style="font-family:Barlow,sans-serif;font-size:9px;color:{T['t1']};
                      line-height:1.7;">
            <b style="color:{T['green']}">Call Wall ({_ns(_ikl_cwall)} | {_fs(_ikl_cwall)} {_fut_lbl_m}):</b>
            ...
            <b style="color:{T['red']}">Put Wall ({_ns(_ikl_pwall)} | {_fs(_ikl_pwall)} {_fut_lbl_m}):</b>
            ...
            <b style="color:{T['amber']}">Zero Gamma ({_ns(_ikl_gflip)} | {_fs(_ikl_gflip)} {_fut_lbl_m}):</b>
            ...
            <b style="color:{T['blue']}">Vol Trigger ({_ns(_ikl_vtrig)} | {_fs(_ikl_vtrig)} {_fut_lbl_m}):</b>
...
def _get_equiv_config(ticker: str, spot: float):
    if ticker == "SPY":
        ...
        # SPX → ES Futures: fetch live ratio
        try:
            es_px  = _fetch_yahoo_price("ES=F")
            spx_px = _fetch_yahoo_price("^GSPC")
            ratio  = es_px / spx_px if spx_px > 0 else 1.0
        except Exception:
            ratio = 1.0
        return "ES Equiv", ratio
    elif ticker == "QQQ":
        ...
        # NDX → NQ Futures: fetch live ratio
        try:
            nq_px  = _fetch_yahoo_price("NQ=F")
            ndx_px = _fetch_yahoo_price("^NDX")
            ratio  = nq_px / ndx_px if ndx_px > 0 else 1.0
        except Exception:
            ratio = 1.0
        return "NQ Equiv", ratio
    elif ticker == "GLD":
        # GLD → Gold Futures (GC=F): 1 GLD share ≈ 1/10 troy oz gold
        # Use live GC=F / GLD ratio for accuracy
        try:
            gc_px  = _fetch_yahoo_price("GC=F")
            gld_px = _fetch_yahoo_price("GLD")
            ratio  = gc_px / gld_px if gld_px > 0 else 10.0
        except Exception:
            ratio = 10.0
        return "Gold/GC Equiv", ratio
    elif ticker == "IWM":
        try:
