"""
GEX // QUANT TERMINAL — Unified Volatility Platform
════════════════════════════════════════════════════════════════════════════════
...
def _render_daily_levels(ticker: str, spot: float, df, raw_df, T: dict):
    ...
    st.markdown(
        ...
        f'margin-top:18px;">MARKET PULSE  ·  {ticker}  ·  OPTIONS-DERIVED SENTIMENT</div>',
        unsafe_allow_html=True)
    ...
    try:
        ...
        if spot > _gflip_0:
            ...
            _pulse_signals.append(("🟢", "ABOVE ZERO GAMMA", f"Spot ${spot:,.2f} is above the gamma flip at ${_gflip_0:,.2f}. Dealer hedging flows are directionally SUPPORTIVE — positive gamma means dealers buy dips and sell rips, dampening volatility and stabilising price.", T["green"]))
        else:
            _score_components.append(("Spot < Gamma Flip", -18, "bearish"))
            _pulse_signals.append(("🔴", "BELOW ZERO GAMMA", f"Spot ${spot:,.2f} is BELOW the gamma flip at ${_gflip_0:,.2f}. Dealers are short gamma — they sell dips and buy rips, AMPLIFYING moves. Volatility risk is elevated; larger swings expected.", T["red"]))
        ...
        if _cw_dist < _pw_dist:
            ...
            _pulse_signals.append(("🟡", "CALL WALL PROXIMITY", f"Spot is closer to the Call Wall (${_cw:,.2f}, {_cw_dist:.1f}% away) than the Put Wall (${_pw:,.2f}, {_pw_dist:.1f}% away). Heavy call OI nearby creates dealer SELLING pressure as they hedge — upside is capped near ${_cw:,.2f}.", T["amber"]))
        else:
            _score_components.append(("Closer to Put Wall", +12, "bullish"))
            _pulse_signals.append(("🟢", "PUT WALL CUSHION", f"Spot has more room to the Call Wall (${_cw:,.2f}, {_cw_dist:.1f}% away). Put Wall at ${_pw:,.2f} ({_pw_dist:.1f}% below) provides dealer BUY support. Upside continuation is structurally more open than downside.", T["green"]))
        ...
        if not df.empty and "gex_net" in df.columns:
            ...
            if _net_gex_total > 0:
                ...
                _pulse_signals.append(("🟢", "NET GEX POSITIVE", f"Total dealer net gamma is +{_net_gex_total:.4f}B. Dealers are collectively LONG gamma — they act as a stabilising force, suppressing vol and buying weakness. Short gamma squeeze risk is LOW.", T["green"]))
            else:
                _score_components.append(("Net GEX Negative", -20, "bearish"))
                _pulse_signals.append(("🔴", "NET GEX NEGATIVE", f"Total dealer net gamma is {_net_gex_total:.4f}B. Dealers are collectively SHORT gamma — directional moves get AMPLIFIED. Volatility risk elevated; be cautious of momentum overshoots.", T["red"]))
        ...
        if _fr is not None:
            ...
            if _fr >= 0.60:
                ...
                _pulse_signals.append(("🟢", "BULLISH FLOW", f"Options flow ratio is {_fr:.1%} — majority of dollar flow is BULLISH (call buys + put sells). Smart money positioning is skewed long.", T["green"]))
            elif _fr <= 0.40:
                _score_components.append(("Flow Bearish", -15, "bearish"))
                _pulse_signals.append(("🔴", "BEARISH FLOW", f"Options flow ratio is {_fr:.1%} — majority of dollar flow is BEARISH (put buys + call sells). Institutional hedging pressure is elevated.", T["red"]))
            else:
                _score_components.append(("Flow Neutral", +5, "neutral"))
                _pulse_signals.append(("🟡", "NEUTRAL FLOW", f"Options flow ratio is {_fr:.1%} — mixed signal. Neither bulls nor bears dominate current options activity. Wait for a clearer directional tilt.", T["amber"]))
        ...
        if spot > _range_mid:
            ...
            _pulse_signals.append(("🟢", "ABOVE EXPECTED RANGE MID", f"Spot at ${spot:,.2f} is trading ABOVE the day's expected range midpoint (${_range_mid:,.2f}). Bulls have range initiative; path of least resistance is toward ${_exp_hi_0:,.2f}.", T["green"]))
        else:
            _score_components.append(("Below Range Mid", -10, "bearish"))
            _pulse_signals.append(("🔴", "BELOW EXPECTED RANGE MID", f"Spot at ${spot:,.2f} is trading BELOW the day's expected range midpoint (${_range_mid:,.2f}). Bears have range initiative; ${_exp_lo_0:,.2f} put wall is the key downside level to watch.", T["red"]))
        ...
        if _vix_val is not None:
            ...
            if _vix_val > 25:
                ...
                _pulse_signals.append(("🔴", "VIX ELEVATED", f"VIX at {_vix_val:.1f} signals HIGH implied volatility. Dealers are pricing in larger moves. Options are expensive — premium sellers face risk; hedges are costly but warranted.", T["red"]))
            elif _vix_val < 16:
                _score_components.append(("VIX Compressed", +8, "bullish"))
                _pulse_signals.append(("🟢", "VIX COMPRESSED", f"VIX at {_vix_val:.1f} indicates LOW fear. Dealers are long gamma, vol is cheap, and the regime favors range-bound or slow grinding price action. Complacency risk: watch for sudden vol expansion.", T["green"]))
            ...
            <div style="display:flex;align-items:center;gap:14px;">
              ...
              <div style="border-left:1px solid {T['line2']};padding-left:14px;">
                ...
                <div style="font-family:JetBrains Mono,monospace;font-size:9px;
                  ...
                  {_bull_count} bull · {_bear_count} bear · of {_total_sigs} signals
                </div>
              </div>
            </div>
        ...
        _bull_prob = _score_100
        _bear_prob = 100 - _score_100
        _range_prob_adj = max(0, 50 - abs(_score_100 - 50))  # highest at neutral
        ...
        _scenarios = [
            (
                ...
                f"{_bull_prob}%",
                T["green"],
                (f"Price holds above ${_gf_s:,.2f} (Zero Gamma). Dealers continue buying dips as hedging flows remain supportive. "
                 f"Next resistance: Call Wall ${_cw_s:,.2f}. Break above opens path toward ${_hi_s:,.2f} (expected high). "
                 f"Vol remains compressed — gamma pinning keeps price stable near current levels." if spot > _gf_s
                 else f"Requires recapture of Zero Gamma at ${_gf_s:,.2f}. Bulls need to flip dealers long gamma for sustained upside. "
                      f"If ${_gf_s:,.2f} is cleared, path opens toward ${_cw_s:,.2f} Call Wall.")
            ),
            (
                "📉  BEARISH / REJECTION",
                f"{_bear_prob}%",
                T["red"],
                (f"Price fails to hold ${_gf_s:,.2f}. Dealer de-hedging adds sell pressure as spot moves into negative gamma territory. "
                 f"Initial support: ${_pw_s:,.2f} (Put Wall). Break of put wall removes structural support — vol expansion likely. "
                 f"Gamma squeeze risk increases below ${_pw_s:,.2f}; expect accelerated moves."
                 if spot > _gf_s
                 else f"Bearish structure intact below ${_gf_s:,.2f}. Dealers are short gamma — moves AMPLIFY directionally. "
                      f"Key downside: ${_pw_s:,.2f} Put Wall, then ${_lo_s:,.2f} (expected low). "
                      f"Vol expansion likely until price finds a new gamma anchor.")
            ),
            (
                "↔️  RANGE / PINNING",
                f"{_range_prob_adj + 15}%",
                T["amber"],
                (f"Price gravitates toward Max Pain and high OI strikes as expiration approaches. "
                 f"Dealer hedging creates a self-reinforcing pin near ${spot:,.2f}. "
                 f"Expected range: ${_lo_s:,.2f} – ${_hi_s:,.2f}. "
                 f"Breakout above ${_cw_s:,.2f} or below ${_pw_s:,.2f} required to escape pinning.")
            ),
        ]
...
def render_es_chart_page():
    ...
    <div style="display:flex;align-items:baseline;gap:14px;
      ...
      <span style="font-family:'JetBrains Mono',monospace;font-size:10px;
                   color:{T['t3']};letter-spacing:1px;">
        SPX-Derived GEX Levels (Fair-Value Converted) · Live 1-min · 3 Models
      </span>
    </div>""", unsafe_allow_html=True)
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # MODEL 1: Candlestick + GEX level overlays
    # ═══════════════════════════════════════════════════════════════════════════
    st.markdown(
        f'<div style="font-family:JetBrains Mono,monospace;font-size:9px;'
        f'color:{T["t3"]};letter-spacing:2px;text-transform:uppercase;'
        f'padding:10px 0 6px 0;border-top:1px solid {T["line2"]};margin-top:8px;">'
        f'MODEL 1  ·  {src_lbl}  ·  CANDLESTICK + GEX WALLS</div>',
        unsafe_allow_html=True)
    ...
    fig_es = make_subplots(
        rows=row_cnt, cols=1, shared_xaxes=True,
        vertical_spacing=0.015, row_heights=row_h, subplot_titles=row_ttl,
    )
    ...
    if show_gex and gex_lvls:
        ...
        fig_es.add_hline(y=spot_es, row=1, col=1,
                         line_dash="solid", line_color=SPOT_YELLOW, line_width=1.8)
        fig_es.add_annotation(
    ...
    vol_row = 3 if show_vol else None
    gex_row = 3 if show_vol else 2
    ...
    if show_gex and gex_lvls:
        ...
        if gex_strikes:
            ...
            fig_es.add_hline(y=0, row=gex_row, col=1, line_color=LN2, line_width=1)

    y_pad = (high_es - low_es) * 0.04
    total_h = 780 if show_vol else 680
    fig_es.update_layout(
        **PL, height=total_h,
        xaxis_rangeslider_visible=False, bargap=0.08,
    ...
    if show_vol:
        fig_es.update_xaxes(showgrid=False,
                             tickfont=dict(size=8, family="JetBrains Mono", color=TX3),
                             row=2, col=1)
        fig_es.update_yaxes(showgrid=False,
                             tickfont=dict(size=7.5, family="JetBrains Mono", color=TX3),
                             row=2, col=1)
    fig_es.update_xaxes(showgrid=False, tickangle=-35,
                         tickfont=dict(size=8, family="JetBrains Mono", color=TX3),
                         row=gex_row, col=1)
    fig_es.update_yaxes(
    ...
    st.plotly_chart(fig_es, use_container_width=True, config={
        ...
        "scrollZoom": True,
        "modeBarButtonsToRemove": ["sendDataToCloud","lasso2d","select2d"],
        "modeBarButtonsToAdd": ["drawline","eraseshape","zoom2d","pan2d","resetScale2d"],
    })

    # ═══════════════════════════════════════════════════════════════════════════
    # MODEL 2: GEX Profile — Net GEX bars + Call OI / Put OI spikes (Image 2 style)
    # Displays ALL strikes from options chain; X=strike, left-Y=GEX, right-Y=OI
    # Green bars = +GEX (dealers long gamma), Red bars = -GEX (dealers short gamma)
    # Magenta area = Put OI, Green area = Call OI; spot = yellow vertical line
    # ═══════════════════════════════════════════════════════════════════════════
    st.markdown(
        f'<div style="font-family:JetBrains Mono,monospace;font-size:9px;'
        f'color:{T["t3"]};letter-spacing:2px;text-transform:uppercase;'
        f'padding:14px 0 6px 0;border-top:1px solid {T["line2"]};margin-top:14px;">'
        f'MODEL 2  ·  NET GEX PROFILE  ·  Call/Put OI  ·  {asset_toggle}  ·  '
        f'Spot = Yellow  ·  Green = +GEX Dealers Long  ·  Red = −GEX Dealers Short  ·  '
        f'Scroll/Pan enabled</div>',
        unsafe_allow_html=True)

    try:
        # Fetch fresh options data for the selected asset
        _gex_lvls2 = _fetch_spx_top_gex_levels() if show_gex else {}
        _ratio2    = _gex_lvls2.get("ratio", 1.0) if _gex_lvls2 else 1.0

        # Always fetch fresh live options chain (60-second cache ensures per-minute updates)
        try:
            _chain_asset = st.session_state.get("asset_choice", "SPY")
            _result2 = fetch_options_data(_chain_asset, 4)
            _, _spot2, _raw_es_data = _result2 if len(_result2) == 3 else (None, spot_es / _ratio2, pd.DataFrame())
            # Persist to session state for other consumers
            if _raw_es_data is not None and not _raw_es_data.empty:
                st.session_state["_es_chart_raw_df"] = _raw_es_data
        except Exception:
            _raw_es_data = st.session_state.get("_es_chart_raw_df", pd.DataFrame())
            _spot2 = spot_es / _ratio2

        if _raw_es_data is not None and not _raw_es_data.empty:
            # Aggregate per strike
            _gp2 = _raw_es_data.groupby("strike").agg(
                net_gex=("call_gex", "sum"),
                put_gex_sum=("put_gex", "sum"),
                call_oi=("open_interest", lambda x: _raw_es_data.loc[x.index][_raw_es_data.loc[x.index,"flag"]=="C"]["open_interest"].sum() if "flag" in _raw_es_data.columns else x.sum() / 2),
                put_oi=("open_interest",  lambda x: _raw_es_data.loc[x.index][_raw_es_data.loc[x.index,"flag"]=="P"]["open_interest"].sum() if "flag" in _raw_es_data.columns else x.sum() / 2),
            ).reset_index()
            _gp2["net_gex"] = _gp2["net_gex"] + _gp2["put_gex_sum"]

            # Re-aggregate call/put OI cleanly
            if "flag" in _raw_es_data.columns:
                _call_oi_s = _raw_es_data[_raw_es_data["flag"]=="C"].groupby("strike")["open_interest"].sum().rename("call_oi")
                _put_oi_s  = _raw_es_data[_raw_es_data["flag"]=="P"].groupby("strike")["open_interest"].sum().rename("put_oi")
                _gp2 = _raw_es_data.groupby("strike").agg(
                    net_gex_c=("call_gex","sum"), net_gex_p=("put_gex","sum")
                ).reset_index()
                _gp2["net_gex"] = _gp2["net_gex_c"] + _gp2["net_gex_p"]
                _gp2 = _gp2.join(_call_oi_s, on="strike").join(_put_oi_s, on="strike")
                _gp2[["call_oi","put_oi"]] = _gp2[["call_oi","put_oi"]].fillna(0)
            else:
                _gp2["call_oi"] = _gp2.get("call_oi", 0)
                _gp2["put_oi"]  = _gp2.get("put_oi", 0)

            # Convert strikes to futures equivalent
            _gp2["strike_fut"] = _gp2["strike"] * _ratio2
            _gp2 = _gp2.sort_values("strike_fut").reset_index(drop=True)

            # Filter ±20% from spot
            _sp_fut = spot_es
            _gp2 = _gp2[
                (_gp2["strike_fut"] >= _sp_fut * 0.80) &
                (_gp2["strike_fut"] <= _sp_fut * 1.20)
            ].reset_index(drop=True)

            if not _gp2.empty:
                _strikes_p = _gp2["strike_fut"].tolist()
                _net_gex_p = _gp2["net_gex"].tolist()
                _call_oi_p = _gp2["call_oi"].tolist()
                _put_oi_p  = _gp2["put_oi"].tolist()

                _gex_colors2 = [T["green"] if v >= 0 else T["red"] for v in _net_gex_p]
                _gex_max2 = max(abs(v) for v in _net_gex_p) or 1e-9
                _oi_max2  = max(max(_call_oi_p or [0]), max(_put_oi_p or [0])) or 1.0

                fig_gp = go.Figure()

                # Put OI — filled area (negative direction, magenta/red)
                fig_gp.add_trace(go.Scatter(
                    x=_strikes_p, y=[-v for v in _put_oi_p],
                    mode="lines", fill="tozeroy",
                    line=dict(color="rgba(220,50,50,0.9)", width=1.2),
                    fillcolor="rgba(220,50,50,0.18)",
                    name="Put OI", yaxis="y2",
                    hovertemplate="Strike %{x:,.1f}<br>Put OI: %{customdata:,.0f}<extra></extra>",
                    customdata=_put_oi_p,
                ))
                # Call OI — filled area (positive direction, green)
                fig_gp.add_trace(go.Scatter(
                    x=_strikes_p, y=_call_oi_p,
                    mode="lines", fill="tozeroy",
                    line=dict(color="rgba(0,200,120,0.9)", width=1.2),
                    fillcolor="rgba(0,200,120,0.14)",
                    name="Call OI", yaxis="y2",
                    hovertemplate="Strike %{x:,.1f}<br>Call OI: %{customdata:,.0f}<extra></extra>",
                    customdata=_call_oi_p,
                ))
                # Net GEX bars — green pos, red neg
                fig_gp.add_trace(go.Bar(
                    ...
                    name="Net GEX",
                    hovertemplate=(
                        f"<b>{asset_toggle} Strike:</b> %{{x:,.1f}}<br>"
                        "<b>Net GEX:</b> %{y:.5f}B<br>"
                        "<i>Positive = dealers long gamma (stabilising)</i><br>"
                        "<i>Negative = dealers short gamma (amplifying)</i><extra></extra>"
                    ),
                    yaxis="y",
                ))
                # Spot vertical line — yellow
                fig_gp.add_vline(x=_sp_fut, line_color=SPOT_YELLOW,
                                  line_width=2.5, line_dash="solid")
                fig_gp.add_annotation(
                    x=_sp_fut, y=1.02, xref="x", yref="paper",
                    text=f"Spot: {SPOT_YELLOW and _sp_fut:,.2f}",
                    showarrow=False, xanchor="center",
                    font=dict(size=9, color=SPOT_YELLOW, family="JetBrains Mono", weight=700),
                    bgcolor=T["bg1"], bordercolor=SPOT_YELLOW, borderwidth=1, borderpad=3,
                )
                # Zero line
                ...
                fig_gp.update_layout(
                    ...
                    margin=dict(t=40, r=80, b=60, l=80),
                    showlegend=True,
                    legend=dict(
                    ...
                    xaxis=dict(
                        title=dict(text="STRIKE", font=dict(size=9, color=TX3)),
                    ...
                    yaxis=dict(
                        title=dict(text="NET GEX ($B)", font=dict(size=9, color=TX3),
                                   standoff=8),
                        showgrid=True, gridcolor=LN, gridwidth=1,
                        tickfont=dict(size=8.5, family="JetBrains Mono", color=TX3),
                    ...
                    yaxis2=dict(
                        title=dict(text="OPEN INTEREST", font=dict(size=9, color=TX3),
                                   standoff=8),
                        showgrid=False,
                        tickfont=dict(size=8.5, family="JetBrains Mono", color=TX3),
                        zeroline=True, zerolinecolor=LN2,
                        side="right", overlaying="y",
                        range=[-_oi_max2 * 1.3, _oi_max2 * 1.3],
                    ),
                    hoverlabel=dict(
                        bgcolor=T["chart_hover"], bordercolor=T["chart_hover_border"],
                        font=dict(family="JetBrains Mono", size=10, color=T["chart_hover_text"]),
                    ...
                    title=dict(
                        text=(f"<b>{asset_toggle} Net GEX Profile</b>"
                              ...
                              f"  <span style='color:rgba(0,200,120,0.9)'>▬ Call OI</span>"
                              f"  <span style='color:rgba(220,50,50,0.9)'>▬ Put OI</span>"),
                        font=dict(size=11, color=TX2, family="JetBrains Mono"), x=0.01),
                    uirevision=f"gex_profile_{st.session_state.get('asset_choice','SPY')}",
                )
                ...
                st.info("GEX profile: no strikes in range.")
        else:
            st.info("GEX profile: options chain not loaded. Navigate to OI GEX tab first.")

    except Exception as _gp_err:
        st.warning(f"GEX profile error: {_gp_err}")

    # ═══════════════════════════════════════════════════════════════════════════
    # MODEL 3: Horizontal MM Position Breakdown by Strike (Image 3 style)
    # Shows Calls (green), Puts (red), Net MM position (blue/cyan) per strike
    # Y-axis = strike price, X-axis = net market maker position count
    # Spot price highlighted with orange box
    # ═══════════════════════════════════════════════════════════════════════════
    st.markdown(
        f'<div style="font-family:JetBrains Mono,monospace;font-size:9px;'
        f'color:{T["t3"]};letter-spacing:2px;text-transform:uppercase;'
        ...
        f'Calls = Green  ·  Puts = Red  ·  Net MM Position = Cyan  ·  Scroll/Pan enabled</div>',
        unsafe_allow_html=True)

    try:
        _chain_asset3 = st.session_state.get("asset_choice", "SPY")
        try:
            _result3 = fetch_options_data(_chain_asset3, 4)
            _, _spot3_raw, _raw3_df = _result3 if len(_result3) == 3 else (None, spot_es / _ratio2, pd.DataFrame())
        except Exception:
            _raw3_df = pd.DataFrame()
            _spot3_raw = spot_es / _ratio2

        _ratio3 = _ratio2

        if _raw3_df is not None and not _raw3_df.empty and "flag" in _raw3_df.columns:
            # Aggregate call OI and put OI per strike
            ...
            # Net MM position: dealers short calls (negative) and short puts (negative)
            # Convention: dealer is short calls (+OI call = dealer sold calls = neg for dealer delta)
            # and short puts (+OI put = dealer sold puts = positive delta hedge)
            # Net position ≈ put_oi - call_oi (dealer net delta exposure per strike)
            _mm["net_mm"]  = _mm["put_oi"] - _mm["call_oi"]
            _mm["call_bar"] = -_mm["call_oi"]  # calls shown as negative (dealers short)
            _mm["strike_fut"] = _mm["strike"] * _ratio3
            _mm = _mm.sort_values("strike_fut", ascending=True).reset_index(drop=True)

            # Filter ±15% from spot
            _sp3 = spot_es
            _mm = _mm[
                (_mm["strike_fut"] >= _sp3 * 0.85) &
                (_mm["strike_fut"] <= _sp3 * 1.15)
            ].reset_index(drop=True)

            if not _mm.empty:
                _yst = _mm["strike_fut"].tolist()
                ...
                _sp3_near = min(_yst, key=lambda s: abs(s - _sp3))

                fig_mm = go.Figure()

                # Put OI bars — red (positive x direction, dealers sold puts = long delta hedge)
                fig_mm.add_trace(go.Bar(
                    ...
                    hovertemplate="Strike %{y}<br>Put OI: <b>%{x:,.0f}</b><extra></extra>",
                    width=0.55,
                ))
                # Call OI bars — green (negative x direction, dealers sold calls = short delta)
                fig_mm.add_trace(go.Bar(
                    ...
                    marker=dict(color="rgba(0,200,120,0.85)",
                                line=dict(width=0.3, color="rgba(255,255,255,0.15)")),
                    hovertemplate="Strike %{y}<br>Call OI: <b>%{customdata:,.0f}</b><extra></extra>",
                    customdata=_mm["call_oi"].tolist(),
                    width=0.55,
                ))
                ...
                fig_mm.add_trace(go.Bar(
                    ...
                    hovertemplate="Strike %{y}<br>Net MM: <b>%{x:,.0f}</b><extra></extra>",
                    width=0.25,
                    offset=0,
                ))
                ...
                fig_mm.add_shape(
                    type="rect",
                    x0=-abs(_mm["call_bar"].min() or 1) * 1.4,
                    x1=abs(_mm["put_oi"].max() or 1) * 1.4,
                ...
                fig_mm.add_annotation(
                    x=abs(_mm["put_oi"].max() or 1) * 1.4,
                    y=_sp3_lbl, xref="x", yref="y",
                    text=f"  ▶ SPOT  {_sp3:,.2f}",
                    showarrow=False, xanchor="left",
                    font=dict(size=9, color="#000000", family="JetBrains Mono", weight=700),
                ...
                fig_mm.update_layout(
                    ...
                    yaxis=dict(
                        title=dict(text="Strike Price", font=dict(size=9, color=TX3)),
                        showgrid=False,
                        tickfont=dict(size=8.5, family="JetBrains Mono", color=TX3),
                        autorange=True,
                    ),
                    ...
                    title=dict(
                        ...
                        font=dict(size=11, color=TX2, family="JetBrains Mono"), x=0.01),
                    uirevision=f"mm_pos_{_chain_asset3}",
                )
                ...
                st.info("MM position: no strikes in ±15% range.")
        else:
            st.info("MM position: options chain not loaded. Navigate to OI GEX tab first to load data.")
    ...
    if show_gex and gex_lvls:
        st.markdown(
            f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;'
            f'color:{T["t3"]};letter-spacing:2px;text-transform:uppercase;'
            f'margin:14px 0 6px 0;padding-top:10px;border-top:1px solid {T["line2"]};">'
            f'ACTIVE GEX LEVELS — ES  '
...
def dashboard():
    ...
    if st.session_state.radar_mode == "GEX":
        ...
        def _build_dte_ladder(raw_src, col, n_dte=5, spot=None, sym_ratio=1.0):
            ...
            # Build Mon-Fri column headers
            _week_cols = _get_week_trading_cols(_today, n_dte)
            dte_labels = [f"{wd.strftime('%a').upper()}  {wd.strftime('%m/%d')}"
                          for wd in _week_cols]
        ...
        def _render_greek_ladder(src_col, ladder_title,
            ...
            st.markdown(
                ...
                f'margin-bottom:6px;padding:4px 0;">{ladder_title}</div>',
                unsafe_allow_html=True)
            _sk_l, _dte_l, _mat_l = _build_dte_ladder(
                _raw_hm, src_col, n_dte=5,
                spot=spot_price, sym_ratio=_hm_ratio)
            if _sk_l is None:
                st.info("Exposure data unavailable.")
                return
            ...
            _ann_text = [[_fmt_cell(_mat_l[i, j]) for j in range(5)]
                         for i in range(len(_sk_l))]

            # Full-spectrum diverging colorscale from neg → zero → pos
            ...
            # Y-labels: Strike price | ES Equiv (no "Strike:" prefix)
            _ylbls_l = [
                f"{s:>8,.0f}  │  {equiv_label} {s * _hm_ratio:>8,.0f}"
                for s in _sk_l
            ]
            ...
            # Use _dte_l from _build_dte_ladder (already weekday-labeled)
            ...
            # Spot price line — bright PRICE YELLOW highlight
            _sp_lbl_l = f"{_sp_near_l:>8,.0f}  │  {equiv_label} {_sp_near_l * _hm_ratio:>8,.0f}"
            fig_lad.add_shape(
                type="rect", x0=-0.5, x1=4.5,
                y0=_sp_lbl_l, y1=_sp_lbl_l,
                line=dict(color=SPOT_YELLOW, width=3),
            )
            fig_lad.add_annotation(
                x=4.55, y=_sp_lbl_l, xref="x", yref="y",
            ...
            # Max +GEX wall line (green)
            _mp_lbl_l = f"{_max_pos_sk_l:>8,.0f}  │  {equiv_label} {_max_pos_sk_l * _hm_ratio:>8,.0f}"
            fig_lad.add_shape(
                type="line", x0=-0.5, x1=4.5,
                y0=_mp_lbl_l, y1=_mp_lbl_l,
                line=dict(color=cs_pos, width=2, dash="dot"),
            )
            fig_lad.add_annotation(
                x=4.55, y=_mp_lbl_l, xref="x", yref="y",
                text="  + WALL",
                showarrow=False, xanchor="left",
                font=dict(size=8, color=cs_pos, family="JetBrains Mono"),
                bgcolor="rgba(0,0,0,0.5)", borderpad=2,
            )

            # Max -GEX wall line (red)
            _mn_lbl_l = f"{_max_neg_sk_l:>8,.0f}  │  {equiv_label} {_max_neg_sk_l * _hm_ratio:>8,.0f}"
            fig_lad.add_shape(
                type="line", x0=-0.5, x1=4.5,
                y0=_mn_lbl_l, y1=_mn_lbl_l,
                line=dict(color=cs_neg, width=2, dash="dot"),
            )
            fig_lad.add_annotation(
                x=4.55, y=_mn_lbl_l, xref="x", yref="y",
                text="  − WALL",
                showarrow=False, xanchor="left",
                font=dict(size=8, color=cs_neg, family="JetBrains Mono"),
                bgcolor="rgba(0,0,0,0.5)", borderpad=2,
            )

            _n_strikes = len(_sk_l)
            _cell_h = max(16, min(22, int(500 / max(_n_strikes, 1))))
            _fig_h  = max(380, min(720, _n_strikes * _cell_h + 90))
            ...
            fig_lad.update_layout(
                ...
                margin=dict(t=44, r=120, b=30, l=200),
                xaxis=dict(
        ...
        # ── TODAY-ONLY: Vanna / Charm / OI single-column heatmaps ─────────────
        # Styled like the Absolute GEX heatmap — single "TODAY" column per strike
        # Shows only current day's exposure snapshot (DTE <= 1, all strikes +-18%)
        st.markdown(
            f'<div style="font-family:JetBrains Mono,monospace;font-size:10px;'
            f'font-weight:600;color:#FFFFFF;letter-spacing:2px;'
            f'text-transform:uppercase;padding:8px 0 5px 0;">'
            f'{asset_toggle}  ·  DEALER GREEK EXPOSURE  ·  TODAY  ·  '
            f'VANNA · CHARM · OI  ·  Current Day Snapshot</div>',
            unsafe_allow_html=True)

        def _build_today_greek_col(raw_src, col, spot=None, sym_ratio=1.0):
            d = raw_src.copy()
            if "dte" in d.columns:
                d_today = d[d["dte"].apply(
                    lambda x: float(x) <= 1 if pd.notna(x) else False)].copy()
                if d_today.empty:
                    d_today = d.copy()
            else:
                d_today = d.copy()
            if spot:
                d_today = d_today[
                    (d_today["strike"] >= spot * 0.82) &
                    (d_today["strike"] <= spot * 1.18)
                ]
            if d_today.empty:
                return None, None
            agg = d_today.groupby("strike")[col].sum().reset_index()
            agg = agg.sort_values("strike").reset_index(drop=True)
            return agg["strike"].tolist(), agg[col].tolist()

        def _render_today_heatmap(src_col, title, cs_pos, cs_neg, val_label):
            st.markdown(
                f'<div style="font-family:JetBrains Mono,monospace;'
                f'font-size:10px;font-weight:600;color:#FFFFFF;'
                f'letter-spacing:1.5px;text-transform:uppercase;'
                f'margin-bottom:6px;padding:4px 0;">{title}</div>',
                unsafe_allow_html=True)

            _sk_t, _vals_t = _build_today_greek_col(
                _raw_hm, src_col, spot=spot_price, sym_ratio=_hm_ratio)
            if _sk_t is None:
                st.info("Exposure data unavailable.")
                return

            _vals_arr = np.array(_vals_t, dtype=float)
            _z_t = _vals_arr.reshape(-1, 1)
            _vmax_t = float(np.nanpercentile(
                np.abs(_vals_arr[np.isfinite(_vals_arr)]), 97)) \
                if np.any(np.isfinite(_vals_arr)) else 1.0
            _vmax_t = max(_vmax_t, 1e-12)
            ...
            _ann_t = [[_fmt_cell_t(_vals_arr[i])] for i in range(len(_sk_t))]

            def _hex_to_rgba_t(hex_color, alpha=0.8):
                h = hex_color.lstrip("#")
                r, g, b = int(h[0:2],16), int(h[2:4],16), int(h[4:6],16)
                return f"rgba({r},{g},{b},{alpha})"

            _cs_t = [
                [0.00, cs_neg], [0.10, _hex_to_rgba_t(cs_neg, 0.8)],
                [0.25, "#440022"], [0.40, "#220011"],
                [0.50, T["bg"]],
                [0.60, "#003322"], [0.75, "#004433"],
                [0.90, _hex_to_rgba_t(cs_pos, 0.8)], [1.00, cs_pos],
            ]
            ...
            _sp_near_t   = min(_sk_t, key=lambda s: abs(s - spot_price))
            _max_pos_idx = int(np.argmax(_vals_arr))
            _max_neg_idx = int(np.argmin(_vals_arr))
            _today_lbl   = datetime.date.today().strftime("TODAY  %b %d")
            ...
            fig_t = go.Figure(go.Heatmap(
                ...
                colorbar=dict(
                    ...
                    title=dict(text=val_label,
                               font=dict(size=9, color="#FFFFFF", family="JetBrains Mono")),
                ),
                hovertemplate=(
                    "%{y}<br>"
                    f"<b>{val_label}:</b> %{{z:.4f}}<extra></extra>"
            ...
            _sp_lbl_t = f"{_sp_near_t:>8,.0f}  |  {equiv_label} {_sp_near_t * _hm_ratio:>8,.0f}"
            fig_t.add_shape(
                type="rect", x0=-0.5, x1=0.5,
                y0=_sp_lbl_t, y1=_sp_lbl_t,
                line=dict(color=SPOT_YELLOW, width=3),
            )
            fig_t.add_annotation(
                x=0.55, y=_sp_lbl_t, xref="x", yref="y",
                text=f"  > SPOT  ${spot_price:,.2f}",
                showarrow=False, xanchor="left",
                font=dict(size=9, color="#000000", family="JetBrains Mono", weight=700),
                bgcolor=SPOT_YELLOW, borderpad=3, opacity=0.97,
            )
            _mp_lbl_t = f"{_sk_t[_max_pos_idx]:>8,.0f}  |  {equiv_label} {_sk_t[_max_pos_idx] * _hm_ratio:>8,.0f}"
            fig_t.add_shape(
                type="line", x0=-0.5, x1=0.5,
                y0=_mp_lbl_t, y1=_mp_lbl_t,
                line=dict(color=cs_pos, width=2, dash="dot"),
            )
            fig_t.add_annotation(
                x=0.55, y=_mp_lbl_t, xref="x", yref="y", text="  + WALL",
                showarrow=False, xanchor="left",
                font=dict(size=8, color=cs_pos, family="JetBrains Mono"),
                bgcolor="rgba(0,0,0,0.5)", borderpad=2,
            )
            _mn_lbl_t = f"{_sk_t[_max_neg_idx]:>8,.0f}  |  {equiv_label} {_sk_t[_max_neg_idx] * _hm_ratio:>8,.0f}"
            fig_t.add_shape(
                type="line", x0=-0.5, x1=0.5,
                y0=_mn_lbl_t, y1=_mn_lbl_t,
                line=dict(color=cs_neg, width=2, dash="dot"),
            )
            fig_t.add_annotation(
                x=0.55, y=_mn_lbl_t, xref="x", yref="y", text="  - WALL",
                showarrow=False, xanchor="left",
                font=dict(size=8, color=cs_neg, family="JetBrains Mono"),
                bgcolor="rgba(0,0,0,0.5)", borderpad=2,
            )
            ...
            _n_t     = len(_sk_t)
            _cell_h  = max(16, min(22, int(500 / max(_n_t, 1))))
            _fig_h_t = max(380, min(900, _n_t * _cell_h + 90))
            ...
            fig_t.update_layout(
                ...
                margin=dict(t=44, r=160, b=30, l=210),
                xaxis=dict(
                    tickfont=dict(size=11, color="#FFFFFF", family="JetBrains Mono"),
                    side="top", showgrid=False, tickangle=0,
                ),
                yaxis=dict(
                    tickfont=dict(size=10, color="#FFFFFF", family="JetBrains Mono"),
                    showgrid=False, autorange="reversed",
                    title=dict(
                        text=f"Price  |  {equiv_label}",
                        font=dict(size=10, color="#FFFFFF")),
                ...
                hoverlabel=dict(
                    bgcolor=T["bg2"], bordercolor=T["line_bright"],
                    font=dict(family="JetBrains Mono", size=10, color="#FFFFFF")),
                uirevision=f"greek_today_{asset_toggle}_{src_col}",
            )
            st.plotly_chart(fig_t, use_container_width=True, config={
                "displayModeBar": True, "displaylogo": False,
                "scrollZoom": True,
                "modeBarButtonsToRemove": ["sendDataToCloud","lasso2d","select2d","autoScale2d"],
                "modeBarButtonsToAdd": ["zoom2d","pan2d","resetScale2d"],
        ...
        _render_today_heatmap(
            "vex_cell",
            f"VANNA EXPOSURE  ·  {asset_toggle}  ·  TODAY  ·  Pos = Call Vanna, Neg = Put Vanna",
            "#00E5A0", "#FF3860", "VEX $")

        _render_today_heatmap(
            "cex_cell",
            f"CHARM EXPOSURE  ·  {asset_toggle}  ·  TODAY  ·  Pos = Call Charm, Neg = Put Charm",
            "#4A9FFF", "#FF3860", "CEX $M")

        # OI heatmap today-only
        st.markdown(
            f'<div style="font-family:JetBrains Mono,monospace;'
            f'font-size:10px;font-weight:600;color:#FFFFFF;'
            f'letter-spacing:1.5px;text-transform:uppercase;'
            f'margin-bottom:6px;padding:4px 0;">'
            f'OPEN INTEREST  ·  {asset_toggle}  ·  TODAY  ·  Highest OI = Resistance/Support Walls</div>',
            unsafe_allow_html=True)

        _raw_oi = raw_df.copy()
        if "dte" in _raw_oi.columns and "open_interest" in _raw_oi.columns:
            _raw_oi_today = _raw_oi[_raw_oi["dte"].apply(
                lambda x: float(x) <= 1 if pd.notna(x) else False)].copy()
            if _raw_oi_today.empty:
                _raw_oi_today = _raw_oi.copy()

            _oi_strikes = sorted([s for s in _raw_oi_today["strike"].unique()
                                  if spot_price * 0.82 <= s <= spot_price * 1.18])
            if _oi_strikes:
                _oi_agg_today = (_raw_oi_today[_raw_oi_today["strike"].isin(_oi_strikes)]
                                 .groupby("strike")["open_interest"].sum().reset_index())
                _oi_agg_today = _oi_agg_today.sort_values("strike").reset_index(drop=True)
                _oi_vals  = _oi_agg_today["open_interest"].tolist()
                _oi_z     = np.array(_oi_vals, dtype=float).reshape(-1, 1)
                _oi_vmax  = float(np.nanpercentile(_oi_z[_oi_z > 0], 97)) \
                    if np.any(_oi_z > 0) else 1.0
                ...
                _oi_ann   = [[_fmt_oi_t(_oi_vals[i])] for i in range(len(_oi_strikes))]
                _oi_ylbls = [
                    f"{s:>8,.0f}  |  {equiv_label} {s * _hm_ratio:>8,.0f}"
                    for s in _oi_strikes]
                _oi_cs = [
                    [0.0,  T["bg"]],
                    [0.25, "#002244"], [0.50, "#004488"],
                    [0.72, "#0066BB"], [0.88, "#3399DD"],
                    [1.0,  "#66BBFF"],
                ]
                _today_lbl_oi = datetime.date.today().strftime("TODAY  %b %d")
                ...
                _sp_oi_lbl = f"{_sp_oi:>8,.0f}  |  {equiv_label} {_sp_oi * _hm_ratio:>8,.0f}"
                fig_oi.add_shape(
                    type="rect", x0=-0.5, x1=0.5,
                ...
                _oi_sorted_idx = np.argsort(np.array(_oi_vals))[::-1]
                for _oi_wall_sk, _oi_wall_color, _oi_wall_lbl_str in [
                    (_oi_strikes[_oi_sorted_idx[0]], "#66BBFF", "+ OI WALL"),
                    (_oi_strikes[_oi_sorted_idx[1]] if len(_oi_sorted_idx) > 1 else None,
                     "#3399DD", "OI WALL 2"),
                ]:
                    if _oi_wall_sk is None:
                        continue
                    _ow_lbl = f"{_oi_wall_sk:>8,.0f}  |  {equiv_label} {_oi_wall_sk * _hm_ratio:>8,.0f}"
                    fig_oi.add_shape(
                        type="line", x0=-0.5, x1=0.5,
                        y0=_ow_lbl, y1=_ow_lbl,
                        line=dict(color=_oi_wall_color, width=2, dash="dot"),
                    )
                    fig_oi.add_annotation(
                        x=0.55, y=_ow_lbl, xref="x", yref="y",
                        text=f"  {_oi_wall_lbl_str}",
                        showarrow=False, xanchor="left",
                ...
                _cell_h_oi = max(16, min(22, int(500 / max(_n_oi, 1))))
                _fig_h_oi  = max(380, min(900, _n_oi * _cell_h_oi + 90))
                fig_oi.update_layout(
                    ...
                    hoverlabel=dict(
                        bgcolor=T["bg2"], bordercolor=T["line_bright"],
                        font=dict(family="JetBrains Mono", size=10, color="#FFFFFF")),
                    uirevision=f"oi_today_{asset_toggle}",
                )
        ...
        if _3d_ok:
            ...
            _gex_cs3_vivid = [
                [0.00, "#FF0000"], [0.08, "#DD0000"],
                [0.18, "#AA0000"], [0.30, "#660000"],
                [0.40, "#330000"], [0.46, "#1A0000"],
                [0.50, "#050505"],
                [0.54, "#001A00"], [0.60, "#003300"],
                [0.70, "#006600"], [0.82, "#00AA00"],
                [0.92, "#00DD00"], [1.00, "#00FF00"],
            ]
            _ax3s = dict(
            ...
            fig_gex3d = go.Figure(go.Surface(
                ...
                lighting=dict(ambient=0.70, diffuse=0.95, specular=0.40,
                              roughness=0.30, fresnel=0.10),
            ...
            fig_gex3d.update_layout(
                ...
                scene=dict(
                    ...
                    aspectmode="manual",
                    aspectratio=dict(x=2.8, y=1.2, z=1.0),
                    # turntable = default Plotly 3D drag mode (left-click rotates freely)
                    dragmode="turntable",
                ),
                showlegend=False,
                font=dict(family="JetBrains Mono, monospace", color="#FFFFFF", size=10),
        ...
        if not _hm2_raw.empty and "expiry" in _hm2_raw.columns:
            ...
            try:
                ...
                # Y-labels: Price | ES equiv | net GEX amount (no "Strike:" prefix)
                _hm2_ylbls = []
                for _hs in _hm2_strikes:
                    _row_gex  = float(_hm2_piv.loc[_hs].sum())
                    _es_eq    = _hs * _hm_ratio
                    _gex_str  = (f" {'+' if _row_gex >= 0 else ''}{_row_gex*1e9:,.0f}$"
                                 if abs(_row_gex) > 1e-9 else "")
                    _hm2_ylbls.append(
                        f"{_hs:>8.0f}  │  {equiv_label} {_es_eq:>8,.0f}{_gex_str}")
                ...
                _hm2_height   = max(600, min(1400,
                                             _hm2_nstrikes * 17 + 130))
                ...
                fig_hm2.update_layout(
                    ...
                    font=dict(family="JetBrains Mono, monospace",
                              color="#FFFFFF", size=10),
                    margin=dict(t=36, r=140, b=60, l=260),
                    xaxis=dict(
                        tickfont=dict(size=10, color="#FFFFFF",
                                      family="JetBrains Mono"),
                    ...
                    yaxis=dict(
                        ...
                        showgrid=False,
                        title=dict(
                            text=f"Price  │  {equiv_label}  │  Net GEX",
                            font=dict(size=10, color="#FFFFFF")),
                    ),
                    title=dict(
                        text=(
                            ...
                            f"Red=-GEX (dealers short, volatile)  "
                            f"Yellow=Spot  Green=+Wall  Red=−Wall</span>"),
