...
def render_es_chart_page():
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # MODEL 2: GEX Profile — Net GEX bars + Call OI / Put OI areas (Image 2 style)
    ...
    try:
        _gex_lvls2 = gex_lvls  # reuse already-fetched levels
        ...
        if _raw_es_data is not None and not _raw_es_data.empty:
            # Aggregate per strike — cleanly handle flag column
            if "flag" in _raw_es_data.columns:
                _call_oi_s = (_raw_es_data[_raw_es_data["flag"]=="C"]
                              .groupby("strike")["open_interest"].sum().rename("call_oi"))
                _put_oi_s  = (_raw_es_data[_raw_es_data["flag"]=="P"]
                              .groupby("strike")["open_interest"].sum().rename("put_oi"))
                ...
                _gp2 = (_gp2_base
                        .join(_call_oi_s, on="strike")
                        .join(_put_oi_s,  on="strike"))
                _gp2[["call_oi","put_oi"]] = _gp2[["call_oi","put_oi"]].fillna(0)
            else:
                # agg already has call_oi / put_oi columns
                _gp2 = _cached_agg[["strike","net_gex","gex_net","call_oi","put_oi"]].copy()
                _gp2 = _gp2.rename(columns={"gex_net":"net_gex"}) if "gex_net" in _gp2.columns and "net_gex" not in _gp2.columns else _gp2
            ...
            if not _gp2.empty:
                ...
                fig_gp.add_trace(go.Scatter(
                    ...
                    hovertemplate=(
                        f"<b>Strike:</b> %{{x:,.1f}}<br>"
                        f"<b>Underlying Spot:</b> {_sp_fut:,.2f}<br>"
                        "<b>Put OI:</b> %{customdata:,.0f}<extra></extra>"
                    ),
                ...
                fig_gp.add_trace(go.Scatter(
                    ...
                    hovertemplate=(
                        f"<b>Strike:</b> %{{x:,.1f}}<br>"
                        f"<b>Underlying Spot:</b> {_sp_fut:,.2f}<br>"
                        "<b>Call OI:</b> %{customdata:,.0f}<extra></extra>"
                    ),
                ...
                fig_gp.add_trace(go.Bar(
                    ...
                    name="Net GEX",
                    customdata=list(zip(_call_oi_p, _put_oi_p)),
                    hovertemplate=(
                        f"<b>Strike:</b> %{{x:,.1f}}<br>"
                        f"<b>Underlying Spot:</b> {_sp_fut:,.2f}<br>"
                        "<b>Net GEX:</b> %{y:.5f}B<br>"
                        "<b>Call OI:</b> %{customdata[0]:,.0f}<br>"
                        "<b>Put OI:</b> %{customdata[1]:,.0f}<br>"
    ...
    try:
        # Reuse shared cached data — no redundant fetch
        ...
        if _raw3_df is not None and not _raw3_df.empty and "flag" in _raw3_df.columns:
            ...
            if not _mm.empty:
                ...
                fig_mm.add_trace(go.Bar(
                    ...
                    hovertemplate=(
                        f"<b>Strike:</b> %{{y}}<br>"
                        f"<b>Underlying Spot:</b> {_sp3:,.2f}<br>"
                ...
                fig_mm.add_trace(go.Bar(
                    ...
                    hovertemplate=(
                        f"<b>Strike:</b> %{{y}}<br>"
                        f"<b>Underlying Spot:</b> {_sp3:,.2f}<br>"
                        "<b>Net MM Position:</b> %{x:,.0f}<extra></extra>"
                    ),
                    width=0.25,
                    offset=0,
                ))
