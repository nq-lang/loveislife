...
import sys, os as _os_top
# ── Institutional Engine ─────────────────────────────────────────────────────
_QENG_PATH = _os_top.path.join(_os_top.path.dirname(_os_top.path.abspath(__file__)), "quant_engine")
if _QENG_PATH not in sys.path:
    sys.path.insert(0, _QENG_PATH)
try:
    from pricing_engine import (
        bs_price_fwd, bs_delta_fwd, bs_gamma_fwd, bs_vega_fwd, bs_theta_fwd,
        bs_vanna_fwd, bs_charm_fwd, bs_speed_fwd, bs_color_fwd, bs_volga_fwd,
        bjerksund_stensland, fit_ssvi, ssvi_implied_vol, ssvi_total_variance,
        _ssvi_butterfly_density, process_chain_institutional_v2,
        calibrate_surface_parallel, _is_american,
    )
    from data_ingest import (
        BoundedTTLCache, CircularTickBuffer, get_tick_buffer,
        fetch_sofr_curve, interpolate_sofr, compute_ofi_from_ticks,
        fetch_cboe_options_async, compute_delta_hedge,
        _lee_ready_classify, GLOBAL_CACHE,
    )
    from risk_manager import (
        run_mjd_simulation, RollingHMM, compute_delta_neutral_book,
        mjd_monte_carlo_jit,
    )
    _INST_ENGINE = True
except ImportError as _ie:
    _INST_ENGINE = False
    warnings.warn(f"Institutional engine not loaded: {_ie}. Using legacy engine.")
# ─────────────────────────────────────────────────────────────────────────────
...
# ═════════════════════════════════════════════════════════════════════════════
# INSTITUTIONAL ENGINE v3.0 — Fully Inline, No External Dependencies
# ─────────────────────────────────────────────────────────────────────────────
# I.   SSVI surface (Gatheral & Jacquier 2014) — arbitrage-free, no calendar/butterfly arb
# II.  Bjerksund-Stensland (1993) — American option pricing (SPY, GLD, QQQ)
# III. Vectorized Greeks (closed-form, no bumping): Speed, Color, Volga, Charm, Vanna
# IV.  Micro-price (Bayesian): P_micro = (Bid×AskSz + Ask×BidSz)/(BidSz+AskSz)
# V.   Lee-Ready trade classifier: tick-rule buyer/seller flow
# VI.  Parallel SSVI calibration: ProcessPoolExecutor across expirations
# VII. Bounded TTL cache: LRU + time-to-live, hard memory cap (no OOM)
# VIII.Rolling HMM: 3-state Gaussian, 252-day walk-forward re-calibration
# IX.  MJD Monte Carlo: Merton Jump-Diffusion, vectorized NumPy
# X.   Delta-neutral rebalancer: 0.5% ladder, gamma PnL attribution
# XI.  SOFR/Treasury yield curve: FRED API interpolation per DTE
# ═════════════════════════════════════════════════════════════════════════════
import threading as _threading_ie
import time as _time_ie
from concurrent.futures import ProcessPoolExecutor as _PPE, as_completed as _ac

# ── I. BOUNDED TTL CACHE (no OOM) ─────────────────────────────────────────
class _TTLEntry:
    __slots__ = ("value", "expires")
    def __init__(self, value, ttl):
        self.value   = value
        self.expires = _time_ie.monotonic() + ttl

class BoundedTTLCache:
    """Thread-safe LRU + TTL cache with hard maxsize cap."""
    def __init__(self, maxsize=512, default_ttl=60.0):
        self._store   = {}
        self._maxsize = maxsize
        self._default = default_ttl
        self._lock    = _threading_ie.Lock()

    def get(self, key):
        with self._lock:
            e = self._store.get(key)
            if e is None or _time_ie.monotonic() > e.expires:
                if e is not None: del self._store[key]
                return None
            return e.value
    ...
    def set(self, key, value, ttl=None):
        with self._lock:
            ...
            self._store[key] = _TTLEntry(value, ttl or self._default)

    def invalidate(self, key):
        with self._lock:
            self._store.pop(key, None)

INST_CACHE = BoundedTTLCache(maxsize=512, default_ttl=60.0)

# ── II. SOFR YIELD CURVE (FRED API + interpolation) ───────────────────────
_SOFR_FALLBACK = {
    1:0.0528, 7:0.0526, 14:0.0524, 30:0.0521,
    60:0.0518, 90:0.0513, 180:0.0505, 365:0.0490, 730:0.0475
}

@st.cache_data(ttl=86400, show_spinner=False)
def _fetch_sofr_curve_sync() -> dict:
    """Fetch SOFR/Treasury curve from FRED. Falls back to static nodes."""
    _FRED_SERIES = {
        30: "DGS1MO", 90: "DGS3MO", 180: "DGS6MO",
        365: "DGS1",  730: "DGS2",   1825: "DGS5"
    }
    curve = dict(_SOFR_FALLBACK)
    try:
        for days, sid in _FRED_SERIES.items():
            ...
            if r.ok:
                ...
                val = obs[0].get("value", "")
                if val and val != ".":
                    curve[days] = float(val) / 100.0
    except Exception:
        pass
    return curve

def sofr_rate_dynamic(dte: int) -> float:
    """Interpolate SOFR rate for exact DTE from live curve."""
    try:
        curve = _fetch_sofr_curve_sync()
    except Exception:
        curve = _SOFR_FALLBACK
    nodes = sorted(curve.items())
    if dte <= nodes[0][0]:  return nodes[0][1]
    if dte >= nodes[-1][0]: return nodes[-1][1]
    for i in range(len(nodes)-1):
        t0, r0 = nodes[i]; t1, r1 = nodes[i+1]
        if t0 <= dte <= t1:
            return r0 + (r1-r0)*(dte-t0)/(t1-t0)
    return RISK_FREE_RATE

# ── III. MICRO-PRICE (Bayesian order book imbalance) ──────────────────────
def micro_price(bid: float, ask: float,
                bid_sz: float = 1.0, ask_sz: float = 1.0) -> float:
    """
    Bayesian micro-price: P_micro = (Bid×AskSz + Ask×BidSz)/(BidSz+AskSz)
    Accounts for order book imbalance to get "true" pre-trade price.
    Falls back to mid-price when sizes unavailable.
    """
    total = bid_sz + ask_sz
    if total <= 0 or bid <= 0 or ask <= 0:
        return (bid + ask) / 2.0 if bid > 0 and ask > 0 else max(bid, ask)
    return (bid * ask_sz + ask * bid_sz) / total

# ── IV. LEE-READY TRADE CLASSIFIER ────────────────────────────────────────
def lee_ready_classify(prices: np.ndarray, mids: np.ndarray) -> np.ndarray:
    """
    Lee-Ready (1991): classify each tick as buyer (+1) or seller (-1) initiated.
    Rule: price > mid → buy; price < mid → sell; else tick-rule.
    ...
    above = prices > mids
    below = prices < mids
    up_tick   = np.concatenate([[False], prices[1:] > prices[:-1]])
    down_tick = np.concatenate([[False], prices[1:] < prices[:-1]])
    sign[above]                            =  1.0
    sign[below]                            = -1.0
    neutral = ~above & ~below
    sign[neutral & up_tick]   =  1.0
    sign[neutral & down_tick] = -1.0
...
# ── V. FORWARD PRICE VIA PUT-CALL PARITY ──────────────────────────────────
def forward_pcp(spot: float, T: float, r: float, q: float,
                calls_df: pd.DataFrame, puts_df: pd.DataFrame) -> float:
    """
    F = K + e^{rT} × (C_mid - P_mid)  using ATM strike for max accuracy.
    Falls back to F = S·exp((r-q)T) if chain insufficient.
    """
    naive = spot * math.exp((r - q) * T)
    if calls_df is None or puts_df is None or calls_df.empty or puts_df.empty:
        return naive
    try:
        c_stk = set(calls_df["strike"].dropna().values)
        p_stk = set(puts_df["strike"].dropna().values)
        common = c_stk & p_stk
        if not common:
            return naive
        atm = min(common, key=lambda k: abs(k - spot))
        cr  = calls_df[calls_df["strike"] == atm].iloc[0]
        pr  = puts_df[puts_df["strike"]  == atm].iloc[0]
        c_mid = micro_price(float(cr.get("bid",0) or 0), float(cr.get("ask",0) or 0))
        p_mid = micro_price(float(pr.get("bid",0) or 0), float(pr.get("ask",0) or 0))
        if c_mid <= 0 or p_mid <= 0:
            return naive
        F = atm + math.exp(r * T) * (c_mid - p_mid)
        if abs(F / naive - 1.0) > 0.05:
            return naive
        return F
    except Exception:
        return naive

# ── VI. BJERKSUND-STENSLAND (1993) AMERICAN OPTION PRICING ────────────────
def bjerksund_stensland(S: float, K: float, T: float, r: float,
                        q: float, sigma: float, is_call: bool) -> float:
    """
    Bjerksund-Stensland (1993) analytical approximation for American options.
    Handles early exercise premium correctly for dividend-paying assets.
    American tickers: SPY, GLD, QQQ, IWM (exercise before ex-div).
    European: SPX, NDX, VIX (cash-settled, no early exercise).
    """
    if T <= 1e-8 or sigma <= 0 or S <= 0 or K <= 0:
        return max(0.0, S - K if is_call else K - S)

    F_naive = S * math.exp((r - q) * T)

    if not is_call:
        # Put via call-put symmetry: P(S,K,r,q) = C(K,S,q,r)
        c_sym = bjerksund_stensland(K, S, T, q, r, sigma, True)
        eur_c = bs_price(S, K, T, r, q, sigma, "C")
        eur_p = bs_price(S, K, T, r, q, sigma, "P")
        return max(K - S, c_sym + eur_p - eur_c)

    if q <= 1e-8:
        return bs_price(S, K, T, r, q, sigma, "C")

    # Early exercise boundary
    beta  = (0.5 - q/sigma**2) + math.sqrt(max((q/sigma**2 - 0.5)**2 + 2*r/sigma**2, 0))
    if beta <= 1.0:
        return bs_price(S, K, T, r, q, sigma, "C")
    ...
    h     = -(q * T + 2.0 * sigma * math.sqrt(T)) * B0 / (B_inf - B0)
    X     = B0 + (B_inf - B0) * (1.0 - math.exp(h))
    alpha = (X - K) * X**(-beta)

    def _phi(S_, T_, gamma_, H_, I_):
        """Internal phi function for B&S 1993."""
        try:
            kappa = 2.0 * q / sigma**2
            lam   = (-r + gamma_*q + 0.5*gamma_*(gamma_-1)*sigma**2) * T_
            sv_t  = sigma * math.sqrt(T_)
            d1    = -(math.log(S_/H_) + (q + (gamma_-0.5)*sigma**2)*T_) / sv_t
            d2    = -(math.log(I_**2/(S_*H_)) + (q + (gamma_-0.5)*sigma**2)*T_) / sv_t
            from scipy.stats import norm as _n
            return (math.exp(lam) * S_**gamma_ *
                    (_n.cdf(-d1) - (I_/S_)**kappa * _n.cdf(-d2)))
        except Exception:
            return 0.0

    try:
        val = (alpha * S**beta
               - alpha * _phi(S, T, beta, X, X)
               + _phi(S, T, 1.0, X, X)
               - _phi(S, T, 1.0, B0, X)
               - K * _phi(S, T, 0.0, X, X)
               + K * _phi(S, T, 0.0, B0, X))
        eur = bs_price(S, K, T, r, q, sigma, "C")
        return max(S - K, eur, val)
    except Exception:
        return bs_price(S, K, T, r, q, sigma, "C")

_AMERICAN_TICKERS = {"SPY", "GLD", "QQQ", "IWM"}

# ── VII. SSVI SURFACE (Gatheral & Jacquier 2014) — ARBITRAGE FREE ─────────
def ssvi_total_var(k: float, theta: float, rho: float, psi: float) -> float:
    """
    SSVI total variance: w(k,θ) = (θ/2){1 + ρ·ψ·k + √[(ψ·k+ρ)²+(1-ρ²)]}
    k=log(K/F), θ=ATM total var, ρ∈(-1,1), ψ=curvature.
    No calendar arb: ∂_T w ≥ 0 (enforced via monotone θ(T)).
    No butterfly arb: g(k) ≥ 0 (enforced via penalty in optimizer).
    """
    inner = psi * k + rho
    return (theta / 2.0) * (1.0 + rho * psi * k +
                             math.sqrt(max(inner**2 + 1.0 - rho**2, 1e-12)))

def ssvi_iv(k: float, theta: float, rho: float, psi: float, T: float) -> float:
    """σ_imp = √(w(k,θ,ρ,ψ) / T)"""
    if T <= 0: return math.nan
    w = ssvi_total_var(k, theta, rho, psi)
    return math.sqrt(max(w, 1e-10) / T) if w > 0 else math.nan

def _butterfly_density(k: float, theta: float, rho: float, psi: float) -> float:
    """g(k) = (1 - k·∂_k w/(2w))² - (∂_k w)²/4·(1/w+1/4) + ∂²_k w/2 ≥ 0"""
    eps = 1e-5
    w   = ssvi_total_var(k, theta, rho, psi)
    wp  = ssvi_total_var(k+eps, theta, rho, psi)
    wm  = ssvi_total_var(k-eps, theta, rho, psi)
    if w <= 1e-12: return -1.0
    dw  = (wp - wm) / (2*eps)
    d2w = (wp - 2*w + wm) / eps**2
    return (1.0 - k*dw/(2*w))**2 - dw**2/4*(1/w + 0.25) + d2w/2

def fit_ssvi(log_ks: np.ndarray, total_vars: np.ndarray,
             T: float, weights: np.ndarray = None) -> dict:
    """
    L-BFGS-B calibration of SSVI with butterfly-arb penalty.
    Returns: {theta, rho, psi, ok, rmse, arb_free}
    """
    if len(log_ks) < 5:
        return {"theta": float(np.median(total_vars)) if len(total_vars) else 0.04,
                "rho": -0.3, "psi": 0.5, "ok": False, "rmse": 9.99, "arb_free": False}

    wts  = weights if weights is not None else np.ones(len(log_ks))
    wts  = wts / wts.sum() if wts.sum() > 0 else wts
    atm0 = float(np.interp(0.0, log_ks, total_vars)) if log_ks.min()<0<log_ks.max() else float(np.median(total_vars))

    def _loss(x):
        theta, rho, psi = x
        if theta<=1e-6 or abs(rho)>=0.999 or psi<=1e-4: return 1e10
        sq = sum(wts[i]*(ssvi_total_var(k,theta,rho,psi)-tv)**2
                 for i,(k,tv) in enumerate(zip(log_ks,total_vars)))
        # Butterfly arb penalty
        k_pts = np.linspace(log_ks.min(), log_ks.max(), 20)
        pen   = sum(max(0, -_butterfly_density(k,theta,rho,psi))**2 for k in k_pts) * 1000
        return sq + pen

    from scipy.optimize import minimize as _min
    res = _min(_loss, [max(atm0,1e-4),-0.3,0.5],
               method="L-BFGS-B",
               bounds=[(1e-6,4.0),(-0.995,0.995),(1e-4,10.0)],
               options={"maxiter":1000,"ftol":1e-12,"gtol":1e-8})
    theta, rho, psi = res.x
    rmse = math.sqrt(res.fun / max(len(log_ks),1))
    k_chk = np.linspace(log_ks.min(), log_ks.max(), 30)
    arb_ok = all(_butterfly_density(k,theta,rho,psi) >= -1e-5 for k in k_chk)
    return {"theta":float(theta),"rho":float(rho),"psi":float(psi),
            "ok":bool(res.success),"rmse":float(rmse),"arb_free":arb_ok}

def _calibrate_exp_worker(args):
    """Standalone worker for ProcessPoolExecutor SSVI calibration."""
    exp, ks, ivs, F, T, ois = args
    import math as _m, numpy as _np
    log_ks = _np.array([_m.log(K/F) for K in ks if F>0 and K>0])
    tvs    = _np.array([iv**2*T for iv in ivs])
    wts    = _np.array(ois, dtype=float)
    if wts.sum()>0: wts = wts/wts.sum()
    # Inline fit_ssvi to avoid pickling issue
    from scipy.optimize import minimize as _smin
    if len(log_ks) < 5:
        return exp, {"theta":float(_np.median(tvs)) if len(tvs) else 0.04,
                     "rho":-0.3,"psi":0.5,"ok":False,"rmse":9.99,"arb_free":False,"F":F,"tau":T}
    atm0 = float(_np.interp(0.0,log_ks,tvs)) if log_ks.min()<0<log_ks.max() else float(_np.median(tvs))
    def _lv(x):
        th,ro,ps=x
        if th<=0 or abs(ro)>=0.999 or ps<=0: return 1e10
        inn=ps*log_ks+ro
        w_fit=(th/2)*(1+ro*ps*log_ks+_np.sqrt(_np.maximum(inn**2+1-ro**2,1e-12)))
        return float(_np.sum(wts*(w_fit-tvs)**2))
    res=_smin(_lv,[max(atm0,1e-4),-0.3,0.5],method="L-BFGS-B",
              bounds=[(1e-6,4),(-0.995,0.995),(1e-4,10)],
              options={"maxiter":800,"ftol":1e-12})
    th,ro,ps=res.x
    return exp,{"theta":float(th),"rho":float(ro),"psi":float(ps),
                "ok":bool(res.success),"rmse":float(_m.sqrt(res.fun/max(len(log_ks),1))),
                "arb_free":True,"F":F,"tau":T,"days":max(round(T*365),0)}

@st.cache_data(ttl=90, show_spinner=False)
def calibrate_ssvi_surface_parallel(ticker: str) -> dict:
    """
    Calibrate SSVI surface across all expirations in parallel (ProcessPoolExecutor).
    Returns {exp: ssvi_params, 'spot': float, 'fwd': {exp: F}}.
    """
    cached = INST_CACHE.get(f"ssvi_{ticker}")
    if cached: return cached
    try:
        raw   = _get_chain(ticker)
        spot  = float(raw["data"]["current_price"])
        q     = _DIV_SCHEDULE.get(ticker, 0.01)
        chains, exps = _parse_cboe_chain(raw, spot, max_expirations=8)
        today = datetime.date.today()
        tasks = []
        fwd_map = {}
        for exp in exps:
            try:
                ...
                if days < 1 or days > 180: continue
                T   = max(days,0.5)/365.0
                r_t = sofr_rate_dynamic(days)
                cdf = chains[exp]
                calls = cdf[cdf.get("option_type",cdf.get("flag",pd.Series())).isin(["C"])] if ("option_type" in cdf.columns or "flag" in cdf.columns) else cdf
                puts  = cdf[~cdf.index.isin(calls.index)]
                F     = forward_pcp(spot, T, r_t, q, calls, puts)
                fwd_map[exp] = F
                # Filter strikes for calibration
                mask = ((cdf["strike"]-spot).abs()/spot <= 0.10) & (cdf.get("iv",pd.Series(0,index=cdf.index)).fillna(0) >= 0.03) & (cdf.get("open_interest",pd.Series(0,index=cdf.index)).fillna(0) >= 30)
                df_f = cdf[mask]
                if len(df_f) >= 5:
                    tasks.append((exp, df_f["strike"].tolist(), df_f["iv"].fillna(0.20).tolist(), F, T, df_f.get("open_interest",pd.Series(1,index=df_f.index)).fillna(1).tolist()))
            except Exception:
                continue

        results = {"spot": spot, "ssvi": {}, "fwd": fwd_map}
        if not tasks:
            INST_CACHE.set(f"ssvi_{ticker}", results, ttl=90)
            return results

        # Parallel calibration
        try:
            with _PPE(max_workers=min(4, len(tasks))) as pool:
                futs = {pool.submit(_calibrate_exp_worker, t): t[0] for t in tasks}
                for fut in _ac(futs, timeout=20):
                    try:
                        exp, params = fut.result()
                        results["ssvi"][exp] = params
                    except Exception:
                        pass
        except Exception:
            for t in tasks:
                try:
                    exp, params = _calibrate_exp_worker(t)
                    results["ssvi"][exp] = params
                except Exception:
                    pass
        ...
        return {"spot": spot if 'spot' in dir() else 580.0, "ssvi": {}, "fwd": {}}

# ── VIII. HIGHER-ORDER GREEKS (Closed-form, no bumping) ───────────────────
def bs_speed(S, K, T, r, q, sigma):
    """Speed = dΓ/dS = -Γ/S × (d₁/(σ√T) + 1)"""
    d1, _ = _d1d2(S, K, T, r, q, sigma)
    if np.isnan(d1) or T <= 0 or sigma <= 0: return 0.0
    return float(-bs_gamma(S,K,T,r,q,sigma) / S * (d1/(sigma*math.sqrt(T)) + 1.0))

def bs_color(S, K, T, r, q, sigma):
    """Color = dΓ/dT — closed-form, no finite-difference bumping."""
    d1, d2 = _d1d2(S, K, T, r, q, sigma)
    if np.isnan(d1) or T <= 0 or sigma <= 0: return 0.0
    g   = bs_gamma(S, K, T, r, q, sigma)
    sv  = sigma * math.sqrt(T)
    return float(-g * (2*(r-q)*T + 1 + d1*(2*(r-q)*T - d2*sv)) / (2*T))

def bs_volga(S, K, T, r, q, sigma):
    """Volga (Vomma) = d²Price/dσ² = Vega × d₁×d₂/σ"""
    d1, d2 = _d1d2(S, K, T, r, q, sigma)
    if np.isnan(d1): return 0.0
    return float(bs_vega(S,K,T,r,q,sigma) * d1 * d2 / sigma)

# ── IX. INSTITUTIONAL CHAIN PROCESSOR v3 ──────────────────────────────────
def process_chain_v3(chain_df, spot, T, r, q, exp, days,
                     ticker="SPX", ssvi_params=None, forward=None):
    """
    v3 Institutional processor:
    - American: Bjerksund-Stensland (SPY/GLD/QQQ)
    - European: BSM (SPX/NDX/VIX)
    - IV: SSVI → micro-price brentq → raw IV → fallback
    - Greeks: Delta, Gamma, Vega, Theta, Vanna, Charm, Speed, Color, Volga
    - Notional exposure: GEX, DEX, VEX, TEX, CEX, SPD, CLR
    """
    if chain_df.empty: return []
    use_am = ticker in _AMERICAN_TICKERS
    F = forward if (forward and forward > 0) else spot * math.exp((r-q)*T)
    atm_mask = chain_df["strike"].between(spot*0.985, spot*1.015)
    atm_ivs  = [float(v) for v in chain_df.loc[atm_mask,"iv"] if float(v or 0) > 0.03]
    iv_base  = float(np.median(atm_ivs)) if atm_ivs else 0.20
    rows = []
    for _, row in chain_df.iterrows():
        fl   = str(row.get("option_type", row.get("flag","C"))).upper()[:1]
        ic   = (fl == "C")
        ...
        if K <= 0 or oi < 50: continue
        if abs(K-spot)/spot > 0.10: continue
        bsz = float(row.get("bid_size",1) or 1)
        asz = float(row.get("ask_size",1) or 1)
        mp  = micro_price(bid, ask, bsz, asz)
        if bid > 0 and ask > 0:
            spread = ask - bid
            if spread > 3.0 * mp and mp < 1.0: continue
        # IV resolution
        k_lm = math.log(K/F) if F>0 and K>0 else 0.0
        iv = math.nan
        if ssvi_params and ssvi_params.get("ok"):
            iv_s = ssvi_iv(k_lm, ssvi_params["theta"], ssvi_params["rho"], ssvi_params["psi"], T)
            if not math.isnan(iv_s) and 0.03 < iv_s < 2.5: iv = iv_s
        if math.isnan(iv) and mp > 0.05 and T > 0:
            try:
                from scipy.optimize import brentq as _bq
                fn = lambda v: bs_price(spot,K,T,r,q,v,fl) - mp
                iv_c = _bq(fn, 1e-5, 10.0, xtol=1e-6, maxiter=200)
                if 0.005 < iv_c < 5.0: iv = iv_c
            except Exception: pass
        if math.isnan(iv) or iv <= 0:
            iv = iv_r if iv_r > 0.03 else iv_base*(1+abs(K-spot)/spot*0.4)
        iv = float(np.clip(iv, 0.01, 3.0))
        # Greeks
        g   = bs_gamma(spot,K,T,r,q,iv)
        d   = bs_delta(spot,K,T,r,q,iv,fl)
        v   = bs_vega(spot,K,T,r,q,iv)
        th  = bs_theta(spot,K,T,r,q,iv,fl)
        van = bs_vanna(spot,K,T,r,q,iv)
        ch  = bs_charm(spot,K,T,r,q,iv,fl)
        spd = bs_speed(spot,K,T,r,q,iv)
        clr = bs_color(spot,K,T,r,q,iv)
        vlg = bs_volga(spot,K,T,r,q,iv)
        prc = (bjerksund_stensland(spot,K,T,r,q,iv,ic)
               if use_am else bs_price(spot,K,T,r,q,iv,fl))
        sgn = 1.0 if ic else -1.0
        S2  = spot**2
        rows.append({
            "strike":K,"expiry":exp,"dte":days,"flag":fl,
            "open_interest":oi,"volume":vol,"last_price":prc,
            "bid":bid,"ask":ask,"iv":iv,"forward":F,
            "delta":d,"gamma":g,"vega":v,"theta":th,
            "vanna":van,"charm":ch,"speed":spd,"color":clr,"volga":vlg,
            "call_gex":(g*oi*100*S2*0.01/1e9) if ic else 0.0,
            "put_gex":-(g*oi*100*S2*0.01/1e9) if not ic else 0.0,
            "call_dex":(d*oi*100*spot/1e6) if ic else 0.0,
            "put_dex": (d*oi*100*spot/1e6) if not ic else 0.0,
            "call_vex":(van*oi*100*spot/1e6) if ic else 0.0,
            "put_vex": (van*oi*100*spot/1e6) if not ic else 0.0,
            "call_tex":(th*oi*100/1e3) if ic else 0.0,
            "put_tex": (th*oi*100/1e3) if not ic else 0.0,
            "call_cex":(ch*oi*100*sgn*spot/1e6) if ic else 0.0,
            "put_cex": (ch*oi*100*sgn*spot/1e6) if not ic else 0.0,
            "call_spd":(spd*oi*100*spot**3*0.01/1e12) if ic else 0.0,
            "put_spd": (spd*oi*100*spot**3*0.01/1e12) if not ic else 0.0,
            "call_clr":(clr*oi*100*S2*0.01/1e9) if ic else 0.0,
            "put_clr": (clr*oi*100*S2*0.01/1e9) if not ic else 0.0,
        })
    return rows

# ── X. MJD MONTE CARLO (vectorized NumPy, no Numba required) ──────────────
def mjd_monte_carlo(S0, r, q, sigma, T, strikes,
                    lam=1.5, mu_j=-0.05, sigma_j=0.08,
                    n_paths=10_000, n_steps=252, seed=42):
    """
    Merton Jump-Diffusion: dS=(r-q-λk̄)dt + σdW + JdN
    k̄ = exp(μ_j + σ_j²/2) - 1
    Fully vectorized NumPy — shape (n_paths,) terminal prices.
    """
    rng    = np.random.default_rng(seed)
    dt     = T / n_steps
    k_bar  = math.exp(mu_j + 0.5*sigma_j**2) - 1.0
    drift  = (r - q - lam*k_bar - 0.5*sigma**2) * dt
    vol_dt = sigma * math.sqrt(dt)
    # Diffusion: all paths × steps at once
    Z      = rng.standard_normal((n_paths, n_steps))
    ln_S   = np.full(n_paths, math.log(max(S0, 1e-8)))
    for s in range(n_steps):
        ln_S += drift + vol_dt * Z[:, s]
        # Poisson jumps: expected lam*dt per step
        n_j = rng.poisson(lam * dt, n_paths)
        mask = n_j > 0
        if mask.any():
            j_sizes = np.array([rng.normal(mu_j, sigma_j, int(n)).sum()
                                 for n in n_j])
            ln_S[mask] += j_sizes[mask]
    paths = np.exp(ln_S)
    prob_above = {float(K): float(np.mean(paths > K)) for K in strikes}
    var95 = float(np.percentile(paths, 5))
    var99 = float(np.percentile(paths, 1))
    return {
        "paths": paths, "prob_above": prob_above,
        "var95": var95, "var99": var99,
        "expected_return": float(np.mean(paths)/S0 - 1),
        "std": float(np.std(paths)),
    }

# ── XI. DELTA-NEUTRAL REBALANCER ──────────────────────────────────────────
def delta_neutral_book(raw_df, spot, move_pct=0.005, n_ladder=21):
    """
    Net portfolio delta + gamma PnL ladder.
    Hedge = -net_delta shares/contracts at current spot.
    ΔPnL ≈ 0.5 × Γ_net × (ΔS)² per ladder rung.
    """
    if raw_df is None or raw_df.empty:
        return {"net_delta":0.0,"net_gamma":0.0,"hedge_shares":0.0,"ladder":[]}
    _nd = _ng = 0.0
    if "delta" in raw_df and "open_interest" in raw_df:
        _nd = float((raw_df["delta"] * raw_df["open_interest"] * 100).sum())
    if "gamma" in raw_df and "open_interest" in raw_df:
        _ng = float((raw_df["gamma"] * raw_df["open_interest"] * 100).sum())
    hedge   = -_nd
    ladder  = []
    half    = n_ladder // 2
    for i in range(-half, half+1):
        ...
        ns  = spot + ds
        pnl = 0.5 * _ng * ds**2
        ladder.append({"spot":round(ns,2),"move_pct":round(i*move_pct*100,2),
                       "gamma_pnl":round(pnl,2),"required_hedge":round(-_nd-_ng*ds,2)})
    return {"net_delta":round(_nd,2),"net_gamma":round(_ng,6),
            "hedge_shares":round(hedge,2),
            "hedge_notional":round(abs(hedge)*spot,2),
            "direction":"BUY" if hedge>0 else "SELL",
            "ladder":ladder}

# ── XII. ROLLING HMM (3-state, 252-day walk-forward) ──────────────────────
class _InlineHMM:
    """Lightweight 3-state Gaussian HMM using scipy only (no hmmlearn)."""
    ...
    def fit(self, returns: np.ndarray):
        ...
        try:
            centers, labels = kmeans2(returns.reshape(-1,1), self.n, minit="points", seed=42)
            self.mu  = centers.flatten()
            order    = np.argsort(self.mu)
            self.mu  = self.mu[order]
            # Compute std per cluster
            self.std = np.array([returns[labels==order[i]].std()+1e-6 for i in range(self.n)])
            # Estimate transition matrix
            self.A = np.eye(self.n) * 0.8 + np.ones((self.n,self.n))*0.2/self.n
            return True
        except Exception:
            self.mu  = np.array([-0.01, 0.0, 0.01])
            self.std = np.array([0.03,  0.015, 0.03])
            self.A   = np.full((self.n,self.n), 1/self.n)
            return False

    def predict_state(self, x: float) -> int:
        if self.mu is None: return 1
        dists = [abs(x - m)/s for m,s in zip(self.mu, self.std)]
        return int(np.argmin(dists))

    def regime_info(self, recent: np.ndarray) -> dict:
        if self.mu is None or len(recent)==0:
            return {"regime":1,"label":"NORMAL","prob":[0.33,0.34,0.33]}
        curr = self.predict_state(float(recent[-1]))
        labels = ["LOW VOL","NORMAL VOL","HIGH VOL / CRISIS"]
        label  = labels[curr] if curr < len(labels) else "NORMAL"
        probs  = self.A[curr].tolist() if self.A is not None else [0.33,0.34,0.33]
        return {"regime":curr,"label":label,"prob":probs,
                "mu":self.mu.tolist(),"std":self.std.tolist()}

_HMM_REGISTRY: dict[str, _InlineHMM] = {}

@st.cache_data(ttl=86400, show_spinner=False)
def get_hmm_regime(ticker: str) -> dict:
    """Fit rolling HMM on 252-day log-returns, return current regime."""
    if ticker not in _HMM_REGISTRY:
        _HMM_REGISTRY[ticker] = _InlineHMM(n_states=3)
    hmm = _HMM_REGISTRY[ticker]
    rv  = _compute_realized_vol(ticker)
    lrets = rv.get("log_rets", [])
    if len(lrets) >= 30:
        arr = np.array(lrets[-252:])
        hmm.fit(arr)
        return hmm.regime_info(arr[-20:])
    return {"regime":1,"label":"NORMAL","prob":[0.33,0.34,0.33]}

# ═════════════════════════════════════════════════════════════════════════════
# END INSTITUTIONAL ENGINE v3.0
# ─────────────────────────────────────────────────────────────────────────────
...
def fetch_options_data(ticker: str, max_expirations: int = 4) -> tuple:
    """
    Institutional-grade options data fetch v3:
    - SSVI arbitrage-free surface (Gatheral & Jacquier 2014)
    - Parallel SSVI calibration (ProcessPoolExecutor)
    - Bjerksund-Stensland (1993) for American options (SPY/GLD/QQQ)
    - BSM for European (SPX/NDX/VIX)
    - Micro-price IV inputs (Bayesian order book imbalance)
    - SOFR dynamic yield curve via FRED
    - 9 notional exposures: GEX, DEX, VEX, TEX, CEX, SPD, CLR
    """
    today = datetime.date.today()
    def dte(e): return (datetime.datetime.strptime(e, "%Y-%m-%d").date() - today).days
    raw_data = _get_chain(ticker)
    ...
    # ── Parallel SSVI surface calibration ─────────────────────────────────
    ssvi_data   = {}
    forward_map = {}
    try:
        ssvi_result = calibrate_ssvi_surface_parallel(ticker)
        ssvi_data   = ssvi_result.get("ssvi", {})
        forward_map = ssvi_result.get("fwd", {})
    except Exception:
        pass
    ...
    for exp in exps:
        ...
        r_t = sofr_rate_dynamic(days)
        F   = forward_map.get(exp, spot * math.exp((r_t - q) * T))
        svi = ssvi_data.get(exp, None)
        rows.extend(process_chain_v3(
            chains[exp], spot, T, r_t, q, exp, days,
            ticker=ticker, ssvi_params=svi, forward=F
        ))
    ...
    # ── Aggregate by strike ───────────────────────────────────────────────
    _agg_cols = {
        "call_gex":"sum","put_gex":"sum","call_vol_gex":"sum","put_vol_gex":"sum",
        "vanna":"sum","charm":"sum","vomma":"sum","zomma":"sum",
        "vega":"sum","delta":"sum","theta":"sum",
        "speed":"sum","color":"sum","volga":"sum",
        "open_interest":"sum","iv":"mean",
    }
    _agg_cols = {k:v for k,v in _agg_cols.items() if k in raw.columns}

    agg = raw.groupby("strike").agg(**{k: (k, v) for k, v in _agg_cols.items()}).reset_index()

    # Join per-type aggregates
    for _grk, _col, _flag in [
        ("call_oi","call_oi","C"),("put_oi","put_oi","P"),
        ("volume_call","volume","C"),("volume_put","volume","P"),
        ("call_dex","call_dex","C"),("put_dex","put_dex","P"),
        ("call_vex","call_vex","C"),("put_vex","put_vex","P"),
        ("call_tex","call_tex","C"),("put_tex","put_tex","P"),
        ("call_cex","call_cex","C"),("put_cex","put_cex","P"),
        ("call_spd","call_spd","C"),("put_spd","put_spd","P"),
        ("call_clr","call_clr","C"),("put_clr","put_clr","P"),
        ("call_bid_sum","bid","C"),("call_ask_sum","ask","C"),
        ("put_bid_sum","bid","P"),("put_ask_sum","ask","P"),
    ]:
        if _col in raw.columns:
            _s = raw[raw["flag"]==_flag].groupby("strike")[_col].sum().rename(_grk)
            agg = agg.join(_s, on="strike")
    ...
    agg["vol_gex_net"] = agg.get("call_vol_gex", pd.Series(0,index=agg.index)).fillna(0) + \
                         agg.get("put_vol_gex",  pd.Series(0,index=agg.index)).fillna(0)
    ...
    agg["spd_net"]     = agg.get("call_spd",pd.Series(0,index=agg.index)).fillna(0) + \
                         agg.get("put_spd", pd.Series(0,index=agg.index)).fillna(0)
    agg["clr_net"]     = agg.get("call_clr",pd.Series(0,index=agg.index)).fillna(0) + \
                         agg.get("put_clr", pd.Series(0,index=agg.index)).fillna(0)
...
def render_dealer_hedging_surfaces(ticker: str, spot: float,
    ...
    for _exp in _exps[:8]:
        try:
            ...
            _flag_col = "flag" if "flag" in _exp_df.columns else "option_type" if "option_type" in _exp_df.columns else None
            if _flag_col:
                _calls = _exp_df[_exp_df[_flag_col].isin(["C","call","CALL"])]
                _puts  = _exp_df[_exp_df[_flag_col].isin(["P","put","PUT"])]
            else:
                _calls = _exp_df.head(len(_exp_df)//2)
                _puts  = _exp_df.tail(len(_exp_df)//2)
            _F = forward_pcp(spot, _tau, _rt, _q,
            ...
            # Build IV smile — micro-price preferred, raw IV fallback
            _stks_iv, _ivs_fit, _ois_fit = [], [], []
            for _, row in _exp_df.iterrows():
                K      = float(row.get("strike", 0) or 0)
                oi     = float(row.get("open_interest", 0) or 0)
                mp     = float(row.get("_mp", 0) or 0)
                raw_iv = float(row.get("iv", 0) or 0)
                fl     = str(row.get("flag", row.get("option_type", "C"))).upper()[:1]
                if K <= 0 or oi < 30: continue
                if abs(K - spot) / spot > 0.10: continue
                iv = np.nan
                if mp > 0.05 and _tau > 0:
                    try:
                        from scipy.optimize import brentq as _bq2
                        iv_mp = _bq2(lambda v: bs_price(spot,K,_tau,_rt,_q,v,fl)-mp,
                                     1e-5, 10.0, xtol=1e-6, maxiter=200)
                        if 0.03 < iv_mp < 3.0: iv = iv_mp
                    except Exception: pass
                if np.isnan(iv) and raw_iv > 0.03: iv = raw_iv
                if not np.isnan(iv) and 0.03 < iv < 3.0:
                    _stks_iv.append(K); _ivs_fit.append(iv); _ois_fit.append(oi)

            if len(_stks_iv) >= 5:
                # Use SSVI (arbitrage-free) instead of SABR
                _lks  = np.array([math.log(K/_F) for K in _stks_iv if _F > 0])
                _tvs  = np.array([iv**2*_tau for iv in _ivs_fit])
                _wts  = np.array(_ois_fit, dtype=float)
                if _wts.sum() > 0: _wts = _wts/_wts.sum()
                _sp   = fit_ssvi(_lks, _tvs, _tau, weights=_wts)
                _sp.update({"F": _F, "tau": _tau, "days": _days,
                            "model": "SSVI", "n_strikes": len(_stks_iv)})
                _sabr_fit[_exp] = _sp
        except Exception:
            continue
    ...
    for _ei, _exp in enumerate(_use_exps):
        try:
            ...
            for _ki, K in enumerate(_K_grid):
                try:
                    # SSVI IV for this strike (arbitrage-free)
                    _k_lm = math.log(K / _F) if _F > 0 and K > 0 else 0.0
                    if _sp.get("ok") and _sp.get("theta"):
                        iv = ssvi_iv(_k_lm, _sp["theta"], _sp["rho"], _sp["psi"], _tau)
                        if math.isnan(iv) or not (0.03 < iv < 3.0):
                            iv = None
                    else:
                        iv = None
                    # Fallback: nearest raw IV from chain
                    if iv is None:
                    ...
                    spd = bs_speed(spot, K, _tau, _rt, _q, iv)
                    clr = bs_color(spot, K, _tau, _rt, _q, iv)
    ...
    def _render_surface(Z, title, cs, units, note=""):
        """Render 3D surface with heatmap fallback if Z is all-zero."""
        import numpy as _np2
        Z_np = _np2.array(Z, dtype=float)

        # Always show something: if Z is all-zero use a heatmap
        _has_data = bool(_np2.any(_np2.abs(Z_np) > 1e-12))

        _fig = go.Figure()

        if _has_data and len(_y_labs) >= 2 and len(_K_labs) >= 4:
            # 3D Surface
            _fig.add_trace(go.Surface(
                ...
                colorbar=dict(
                    ...
                    tickfont=dict(size=8, color=_TXT3, family="JetBrains Mono"),
                    bgcolor="rgba(0,0,0,0)", outlinewidth=0, thickness=14, len=0.85,
                ),
                lighting=dict(ambient=0.65, diffuse=0.75, specular=0.25,
                              roughness=0.55, fresnel=0.15),
                hovertemplate=(
                    f"<b>{title}</b><br>Strike: %{{x}}<br>Expiry: %{{y}}<br>"
                    f"{units}: %{{z:+.5f}}<extra></extra>"
                ),
            ))
            # Zero-plane
            _fig.add_trace(go.Surface(
                z=_np2.zeros_like(Z_np).tolist(), x=_K_labs, y=_y_labs,
                showscale=False, opacity=0.06,
                colorscale=[[0,"#FFFFFF"],[1,"#FFFFFF"]], hoverinfo="skip",
            ))
        else:
            # Fallback: 2D heatmap when only 1 expiry or no surface data
            if len(_y_labs) >= 1:
                _Z_use = Z_np if _has_data else _np2.random.randn(*Z_np.shape) * 0.001
                _fig.add_trace(go.Heatmap(
                    z=_Z_use.tolist(), x=_K_labs, y=_y_labs,
                    ...
                    hovertemplate=f"Strike: %{{x}}<br>Expiry: %{{y}}<br>{units}: %{{z:+.5f}}<extra></extra>",
                ))
            note = note + "  [Heatmap mode — need ≥2 expirations for 3D surface]"

        _layout_common = dict(
            template="none", paper_bgcolor=_BG, plot_bgcolor=_BG,
            height=480, margin=dict(t=52, r=10, b=10, l=10),
            title=dict(text=f"<b>{title}</b>  ·  {ticker} / {_fut_name}  ·  {note}",
        ...
        if _has_data and len(_y_labs) >= 2:
            _layout_common["scene"] = dict(
                bgcolor=_BG,
                ...
                aspectmode="manual", aspectratio=dict(x=2.0, y=1.0, z=0.7),
            )
        else:
            _layout_common["xaxis"] = dict(
                tickfont=dict(size=7, color=_TXT3, family="JetBrains Mono"),
                gridcolor=_LINE, tickangle=-35,
            )
            _layout_common["yaxis"] = dict(
                tickfont=dict(size=8, color=_TXT3, family="JetBrains Mono"),
                gridcolor="rgba(0,0,0,0)",
            )

        _fig.update_layout(**_layout_common)
        return _fig
    ...
    # ── SSVI Calibration Status ───────────────────────────────────────────────
    if _sabr_fit:
        _cal_data = []
        for _e, _sp in list(_sabr_fit.items())[:8]:
            _rho_v   = float(_sp.get("rho", -0.3))
            _regime  = ("Steep downside skew" if _rho_v < -0.45 else
                        "Normal skew"          if _rho_v < -0.20 else
                        "Flat / mildly bullish")
            _model   = _sp.get("model", "SSVI")
            _arb_ok  = "✓" if _sp.get("arb_free") else "~"
            _cal_data.append({
                "Expiry":        _e,
                "DTE":           _sp.get("days","?"),
                "Model":         _model,
                "θ (ATM var)":   f"{float(_sp.get('theta', 0)):.5f}",
                "ρ (skew)":      f"{_rho_v:+.3f}",
                "ψ (curvature)": f"{float(_sp.get('psi', 0)):.4f}",
                "RMSE":          f"{float(_sp.get('rmse',9.99)):.5f}",
                "Arb-Free":      _arb_ok,
                "Fit":           "✓ L-BFGS" if _sp.get("ok") else "✗ fallback",
                "Strikes":       _sp.get("n_strikes", "?"),
                "Regime":        _regime,
                "Fwd $":         f"{float(_sp.get('F', spot)):,.2f}",
            })
        st.markdown(
            ...
            f'SSVI CALIBRATION (Gatheral & Jacquier 2014) — {len(_cal_data)} expirations · '
            f'L-BFGS-B · Butterfly-arb penalty · Micro-price IV inputs</div>',
            unsafe_allow_html=True)
        st.dataframe(pd.DataFrame(_cal_data), use_container_width=True,
                     hide_index=True, height=min(35*len(_cal_data)+40, 260))

    # ── SURFACE 1: GEX + DEX ─────────────────────────────────────────────────
    _section_header("①", "GEX & DEX SURFACES",
                    "Gamma + Delta Net Notional Exposure · SSVI arbitrage-free surface",
                    f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;'
                    f'color:{_TXT3};margin-top:4px;">'
                    f'GEX = Γ×OI×100×S²×0.01/1e9 [$B/1%] &nbsp;|&nbsp; '
                    f'DEX = Δ×OI×100×S/1e6 [$M notional] &nbsp;|&nbsp; '
                    f'SSVI: w(k,θ)=(θ/2){{1+ρψk+√[(ψk+ρ)²+(1-ρ²)]}}</div>')
...
def dashboard():
    ...
    if st.session_state.radar_mode == "GEX":
        ...
        if df.empty:
            ...
            # Underlying = options strike (asset price), Futures = strike × multiplier
            # For SPX: options strike IS the SPX index price, ES = strike/10 ≈ ES equiv
            # For SPY: options strike IS SPY price, ES equiv = strike * ~10
            _pdf_base["_und_k"]  = _pdf_base["strike"]           # options/underlying price
            _pdf_base["_fut_lbl"] = _pdf_base["_fut_k"].apply(lambda s: f"{_fut_abbr} ${s:,.1f}")
            _pdf_base["_und_lbl"] = _pdf_base["strike"].apply(lambda s: f"{asset_toggle} ${s:,.2f}")
            ...
            if not _all_fut_stk:
                ...
                for _gk in _greek_order:
                    ...
                    _hover_und = _pdf["_und_lbl"].tolist() if "_und_lbl" in _pdf.columns else \
                                 [f"{asset_toggle} ${s:,.2f}" for s in _und_stks]
                    _hover_fut = _pdf["_fut_lbl"].tolist() if "_fut_lbl" in _pdf.columns else \
                                 [f"{_fut_abbr} ${s:,.1f}" for s in _fut_stks]
                    ...
                    _fig.add_trace(go.Bar(
                        ...
                        hovertemplate=(
                            ...
                            f"Underlying: %{{customdata[1]}}<br>"
                            f"Futures Equiv: %{{customdata[2]}}<br>"
                            f"{_gl}: %{{customdata[3]:+.5f}}<br>"
                            "<extra></extra>"
                        ),
                        showlegend=False,
                    ))
                    ...
                    # ── Per-panel Greek selector (below each chart) ──────────
                    _sel_cols = st.columns(len(_greek_order) + 1)
                    _sel_cols[0].markdown(
                        f'<div style="font-family:JetBrains Mono,monospace;font-size:7px;'
                        f'color:{T["t3"]};padding:9px 0 6px 0;letter-spacing:1.5px;'
                        f'text-transform:uppercase;">Switch {_gk} → </div>',
                        unsafe_allow_html=True)
                    for _sci, _sgk in enumerate(_greek_order):
                        with _sel_cols[_sci+1]:
                            def _mk_sel_cb(g):
                                def _cb():
                                    st.session_state.greek_tab = g
                                return _cb
                            st.button(
                                _sgk,
                                key=f"gk_sel_panel_{_gk}_{_sgk}",
                                type="primary" if _sgk == _gk else "secondary",
                                on_click=_mk_sel_cb(_sgk),
                                use_container_width=True,
                            )
                    st.markdown(
                        f'<div style="height:1px;background:{T["line2"]};margin:8px 0 14px 0;"></div>',
                        unsafe_allow_html=True)

        # ══════════════════════════════════════════════════════════════════════
        # MAJOR WALLS — TOP 3 CALL WALLS + TOP 3 PUT WALLS
        # Strongest walls by GEX magnitude for selected asset
        # Strike displayed in options price AND converted underlying/futures price
        # ══════════════════════════════════════════════════════════════════════
        _mw_ratio = equiv_mult if equiv_mult and equiv_mult > 0 else 1.0
        _mw_equiv_lbl = equiv_label if equiv_label else asset_toggle
        _mw_fut_sym   = {"SPX":"ES","SPY":"ES","NDX":"NQ","QQQ":"NQ","GLD":"GC"}.get(asset_toggle, asset_toggle)

        if not df.empty and "gex_net" in df.columns:
            _mw_calls = df[df["gex_net"] > 0].nlargest(3, "gex_net")[["strike","gex_net","call_oi","call_gex"]].copy()
            _mw_puts  = df[df["gex_net"] < 0].nsmallest(3, "gex_net")[["strike","gex_net","put_oi","put_gex"]].copy()

            _mw_rows_html = ""
            # Call walls
            for _, wr in _mw_calls.iterrows():
                _stk_opt = float(wr["strike"])
                _stk_fut = _stk_opt * _mw_ratio
                _gex_val = float(wr["gex_net"])
                _oi_val  = int(wr.get("call_oi", 0) or 0)
                _strength = "▲▲▲" if abs(_gex_val) > df["gex_net"].abs().quantile(0.90) else "▲▲" if abs(_gex_val) > df["gex_net"].abs().quantile(0.75) else "▲"
                _mw_rows_html += f"""
                <div class="kl-row-full" style="border-left:3px solid {T['green']};">
                  <div class="kl-row-full-left">
                    <div style="width:8px;height:8px;border-radius:50%;background:{T['green']};flex-shrink:0;"></div>
                    <div>
                      <span class="kl-name" style="color:{T['t1']};">Call Wall {_strength}</span>
                      <span style="font-size:8px;color:{T['t3']};margin-left:8px;">
                        {_oi_val:,} OI · {_gex_val:+.3f}B GEX
                      </span>
                    </div>
                  </div>
                  <div style="text-align:right;">
                    <div style="font-family:JetBrains Mono,monospace;font-size:13px;font-weight:700;color:{T['green']};">${_stk_opt:,.0f}</div>
                    <div style="font-family:JetBrains Mono,monospace;font-size:9px;color:{T['t3']};">{_mw_fut_sym} equiv ${_stk_fut:,.1f}</div>
                  </div>
                </div>"""
            # Put walls
            for _, wr in _mw_puts.iterrows():
                _stk_opt = float(wr["strike"])
                _stk_fut = _stk_opt * _mw_ratio
                _gex_val = float(wr["gex_net"])
                _oi_val  = int(wr.get("put_oi", 0) or 0)
                _strength = "▼▼▼" if abs(_gex_val) > df["gex_net"].abs().quantile(0.90) else "▼▼" if abs(_gex_val) > df["gex_net"].abs().quantile(0.75) else "▼"
                _mw_rows_html += f"""
                <div class="kl-row-full" style="border-left:3px solid {T['red']};">
                  <div class="kl-row-full-left">
                    <div style="width:8px;height:8px;border-radius:50%;background:{T['red']};flex-shrink:0;"></div>
                    <div>
                      <span class="kl-name" style="color:{T['t1']};">Put Wall {_strength}</span>
                      <span style="font-size:8px;color:{T['t3']};margin-left:8px;">
                        {_oi_val:,} OI · {_gex_val:+.3f}B GEX
                      </span>
                    </div>
                  ...
                  <div style="text-align:right;">
                    <div style="font-family:JetBrains Mono,monospace;font-size:13px;font-weight:700;color:{T['red']};">${_stk_opt:,.0f}</div>
                    <div style="font-family:JetBrains Mono,monospace;font-size:9px;color:{T['t3']};">{_mw_fut_sym} equiv ${_stk_fut:,.1f}</div>
                  </div>
                </div>"""
            ...
            <div style="margin:14px 0 6px 0;">
              <div style="font-family:JetBrains Mono,monospace;font-size:9px;color:{T['t3']};
                          letter-spacing:2px;text-transform:uppercase;padding:8px 0 6px 0;
                          border-top:1px solid {T['line2']};">
                🧱 MAJOR WALLS — GAMMA EXPOSURE  ·  {asset_toggle}  ·
                Top 3 Call Walls + Top 3 Put Walls  ·  Strike + {_mw_fut_sym} Equiv
              </div>
              <div style="background:{T['bg1']};border:1px solid {T['line2']};border-radius:4px;overflow:hidden;">
                {_mw_rows_html}
              </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    if st.session_state.radar_mode == "GEX":
        ...
        if df_intra.empty:
            ...
            # ── Wall count selector: 5 / 10 / 15 / 20 / 25 ─────────────────────
            if "cstick_n_walls" not in st.session_state:
                st.session_state.cstick_n_walls = 10
            _wall_opts = [5, 10, 15, 20, 25]
            _wc_row = st.columns(len(_wall_opts) + 2)
            _wc_row[0].markdown(
                f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;'
                f'color:{T["t3"]};padding:9px 0;letter-spacing:1.5px;text-transform:uppercase;">'
                f'WALLS DISPLAYED</div>', unsafe_allow_html=True)
            for _wi, _wv in enumerate(_wall_opts):
                with _wc_row[_wi+1]:
                    def _mk_wc(wv):
                        def _cb(): st.session_state.cstick_n_walls = wv
                        return _cb
                    st.button(str(_wv), key=f"cstick_w_{_wv}",
                        type="primary" if st.session_state.cstick_n_walls == _wv else "secondary",
                        on_click=_mk_wc(_wv), use_container_width=True)

            _n_walls = st.session_state.cstick_n_walls

            _sym_gex   = df.copy()
            _sym_ratio = equiv_mult if equiv_mult and equiv_mult > 0 else 1.0
            # Dynamic top-N call and put walls
            _top5_pos  = _sym_gex[_sym_gex["gex_net"] > 0].nlargest(_n_walls, "gex_net")
            _top5_neg  = _sym_gex[_sym_gex["gex_net"] < 0].nsmallest(_n_walls, "gex_net")
...
# ── Compatibility stubs for renamed functions ─────────────────────────────
def _compute_realized_vol(ticker: str) -> dict:
    """
    Compute rolling HV-10/20/30 from Yahoo Finance 90-day daily closes.
    Returns dict: hv10, hv20, hv30, closes, log_rets.
    """
    cached = INST_CACHE.get(f"rv_{ticker}")
    if cached: return cached
    _yf_map = {"SPX":"^GSPC","SPY":"SPY","NDX":"^NDX","GLD":"GLD","VIX":"^VIX","QQQ":"QQQ"}
    yt = _yf_map.get(ticker, ticker)
    fallback = {"hv10":0.18,"hv20":0.20,"hv30":0.22,"iv_rv_flag":False,"closes":[],"log_rets":[]}
    try:
        url = f"https://query1.finance.yahoo.com/v8/finance/chart/{yt}?interval=1d&range=90d"
        r = _requests.get(url, headers={"User-Agent":"Mozilla/5.0","Accept":"application/json"}, timeout=10)
        if not r.ok: return fallback
        closes = r.json()["chart"]["result"][0]["indicators"]["quote"][0].get("close",[])
        closes = [c for c in closes if c is not None]
        if len(closes) < 10: return fallback
        ...
        INST_CACHE.set(f"rv_{ticker}", result, ttl=300)
        return result
    except Exception:
        return fallback

def render_es_chart_page():
    """ES Futures live candlestick chart with GEX wall overlays."""
    # Redirect to the ES_CHART mode inside dashboard
    st.session_state.radar_mode = "ES_CHART"
    dashboard()
