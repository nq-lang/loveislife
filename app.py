...
def _fetch_spx_top_gex_levels(selected_asset: str = "SPX") -> dict:
    ...
    """
    # Asset → (chain ticker, futures ticker, index ticker for ratio denominator)
    _ASSET_CHAIN_MAP = {
        "SPY": ("SPY",  "ES=F", "^GSPC"),
        "SPX": ("SPX",  "ES=F", "^GSPC"),
        "NDX": ("NDX",  "NQ=F", "^NDX"),
    ...
    _chain_t, _fut_t, _idx_t = _ASSET_CHAIN_MAP.get(
        selected_asset, ("SPX", "ES=F", "^GSPC"))
    ...
    try:
        ...
        # Live ratio: futures price / index price
        try:
            _fut_px  = _fetch_yahoo_price(_fut_t)
            _idx_px  = _fetch_yahoo_price(_idx_t)
            ratio    = _fut_px / _idx_px if _idx_px > 0 else 1.0
        except Exception:
            ratio = 1.0
...
def render_es_chart_page():
    ...
    # This ratio converts each option STRIKE PRICE to the corresponding
    # underlying futures price so every hover shows the correct futures equiv.
    _ASSET_FUT_MAP = {
        "SPY": ("ES=F",  "^GSPC",  "ES Futures"),
        "SPX": ("ES=F",  "^GSPC",  "ES Futures"),
        "NDX": ("NQ=F",  "^NDX",   "NQ Futures"),
        "GLD": ("GC=F",  "GLD",    "Gold/GC Futures"),
    }
    _fut_tk_m,  _opt_tk_m,  _fut_lbl_m  = _ASSET_FUT_MAP.get(
        _chain_asset, ("ES=F", "^GSPC", "ES Futures"))
    try:
        _fut_px_m  = _fetch_yahoo_price(_fut_tk_m)
        _opt_px_m  = _fetch_yahoo_price(_opt_tk_m) if _opt_tk_m != _chain_asset \
                     else float(_model_spot) if _model_spot else 1.0
        _m23_ratio = _fut_px_m / _opt_px_m if _opt_px_m > 0 else 1.0
    except Exception:
        _m23_ratio = 1.0
    # Futures spot (for the live underlying futures price)
    _fut_spot_m = float(_model_spot) * _m23_ratio if _model_spot else spot_es
    ...
    # Key GEX level values in futures-equivalent space
    _m23_gflip = _to_fut(gex_lvls.get("gamma_flip", spot_es / _m23_ratio)) if gex_lvls else spot_es
    _m23_cwall = _to_fut(gex_lvls.get("call_wall",  spot_es / _m23_ratio)) if gex_lvls else spot_es * 1.01
    _m23_pwall = _to_fut(gex_lvls.get("put_wall",   spot_es / _m23_ratio)) if gex_lvls else spot_es * 0.99
    _m23_vtrig = _to_fut(gex_lvls.get("vol_trigger",spot_es / _m23_ratio)) if gex_lvls else spot_es
