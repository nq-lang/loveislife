    ...
    if st.session_state.radar_mode == "GEX":
        ...
        try:
            ...
            _raw3 = _raw3[
                (_raw3["_dte3"] >= 0) &
                (_raw3["_dte3"] <= 65) &
                (_raw3["strike"] >= spot_price * 0.82) &
                (_raw3["strike"] <= spot_price * 1.18)
            ].copy()
        ...
        if _3d_ok:
            ...
            _z3 = _g_arr3.T  # shape: (n_dtes, n_strikes)

            # Ensure we have at least 2 DTE rows to form a real surface
            if _z3.shape[0] < 2:
                _z3 = np.vstack([_z3, _z3 * 0.85])
                _g3_dtes = list(_g3_dtes) + [_g3_dtes[-1] + 1]

            _g_strikes_fut3 = [round(s * float(equiv_mult), 2)
            ...
            # Compute explicit cmin/cmax from actual data so surface is always visible
            ...
            _z3_cmin, _z3_cmax = -_z3_abs_max, _z3_abs_max
            # Reference-style colorscale: red (neg) → dark mid → green/blue (pos)
            # Matching the reference image (blue peaks = +GEX, red dips = -GEX)
            _gex_cs3 = [
                [0.00, "#CC0020"], [0.10, "#990015"],
                [0.20, "#660010"], [0.30, "#330008"],
                [0.38, "#1A0005"], [0.45, "#0D0002"],
                [0.50, "#050505"],
                [0.55, "#000D08"], [0.62, "#001A10"],
                [0.70, "#003320"], [0.80, "#006640"],
                [0.90, "#00CC77"], [1.00, "#00FF99"],
            ]

            fig_gex3d = go.Figure(go.Surface(
                x=_g3_strikes,
                y=_g3_dtes,
                z=_z3,                # z shape: (n_dtes, n_strikes)
                colorscale=_gex_cs3,
                cmin=_z3_cmin,
                cmax=_z3_cmax,
                showscale=True,
                ...
                lighting=dict(ambient=0.65, diffuse=0.90,
            ...
            # Build axis style dicts without conflicting keys for zaxis
            _ax3g_xy = dict(
                backgroundcolor=T["bg1"],
                gridcolor="rgba(255,255,255,0.18)",
                gridwidth=1, showspikes=True,
                zeroline=True,
