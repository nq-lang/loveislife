...
def render_es_chart_page():
    ...
    try:
        ...
        # Use the dashboard's raw_df if available via session state, else rebuild
        _raw_es_data = st.session_state.get("_es_chart_raw_df", None)

        if _raw_es_data is None or _raw_es_data.empty:
            # Rebuild from live chain
            try:
...
def dashboard():
    ...
    if st.session_state.radar_mode == "GEX":
        ...
        # ── MODEL 1: 3-D GAMMA EXPOSURE SURFACE ────────────────────────────────
        st.markdown(
            f'<div style="font-family:JetBrains Mono,monospace;font-size:10px;'
            f'font-weight:600;color:#FFFFFF;letter-spacing:2px;'
            f'text-transform:uppercase;padding:14px 0 6px 0;'
            f'border-top:1px solid {T["line2"]};margin-top:4px;">'
            f'{asset_toggle}  ·  GEX SURFACE  ·  Strike × DTE  ·  '
            f'Green = +GEX Dealers Long  ·  Red = −GEX Dealers Short  ·  '
            f'Spot — Zero Γ  |  Drag to rotate</div>',
            unsafe_allow_html=True)
        ...
        if _3d_ok:
            ...
            # ── Smooth the surface via bicubic interpolation (IV-surface style) ──
            _n_strikes_fine = min(max(len(_g3_strikes) * 3, 60), 120)
            _n_dtes_fine    = min(max(len(_g3_dtes) * 3, 30), 80)
            _strikes_fine   = np.linspace(_g3_strikes[0], _g3_strikes[-1], _n_strikes_fine)
            _dtes_fine      = np.linspace(_g3_dtes[0], _g3_dtes[-1], _n_dtes_fine)

            try:
                _interp_fn  = _sci_interp.RectBivariateSpline(
                    _g3_strikes, _g3_dtes, _g_arr3, kx=min(3, len(_g3_strikes)-1),
                    ky=min(3, len(_g3_dtes)-1))
                _z3_smooth  = _interp_fn(_strikes_fine, _dtes_fine)  # (n_strikes_fine, n_dtes_fine)
            except Exception:
                # Fall back to nearest-neighbour if spline fails
                _z3_smooth = _g_arr3
                _strikes_fine = _g3_strikes
                _dtes_fine    = _g3_dtes

            # z for plotly Surface: shape (n_dtes_fine, n_strikes_fine)
            _z3 = _z3_smooth.T

            _g_strikes_fut3 = [round(s * float(equiv_mult), 2) for s in _strikes_fine]
            _cd3 = np.tile(np.array(_g_strikes_fut3), (len(_dtes_fine), 1))
            ...
            # ── Colorscale: red → black → green (clean, like IV surface viridis) ──
            _gex_cs3_vivid = [
                [0.00, "#CC0000"], [0.10, "#990000"],
                [0.22, "#660000"], [0.35, "#330000"],
                [0.45, "#111111"],
                [0.50, "#050505"],
                [0.55, "#111111"],
                [0.65, "#003300"], [0.78, "#006600"],
                [0.90, "#009900"], [1.00, "#00CC00"],
            ]
            ...
            _ax3s = dict(
                ...
                tickfont=dict(size=9, color="#CCCCCC", family="JetBrains Mono"),
                tickcolor="#CCCCCC",
            )
            _ax3s_z = dict(
                backgroundcolor=T["bg1"],
                gridcolor="rgba(255,255,255,0.12)", gridwidth=1,
                showspikes=True, zeroline=True,
                zerolinecolor="#FFFFFF", zerolinewidth=2,
                tickfont=dict(size=9, color="#CCCCCC", family="JetBrains Mono"),
                tickcolor="#CCCCCC",
            )

            fig_gex3d = go.Figure(go.Surface(
                x=list(_strikes_fine),
                y=list(_dtes_fine),
                z=_z3,
                ...
                colorbar=dict(
                    title=dict(text="GEX ($B)",
                               font=dict(size=10, color="#CCCCCC", family="JetBrains Mono")),
                    tickfont=dict(size=9, color="#CCCCCC", family="JetBrains Mono"),
                    thickness=12, len=0.72, tickformat=".4f",
                    bgcolor="rgba(0,0,0,0)",
                ...
                lighting=dict(ambient=0.75, diffuse=0.90, specular=0.25,
                              roughness=0.45, fresnel=0.05),
                contours=dict(
                    z=dict(show=False),
                    x=dict(show=False),
                    y=dict(show=False),
            ...
            # Spot line
            _sp_k   = min(_strikes_fine, key=lambda k: abs(k - spot_price))
            _sp_idx = list(_strikes_fine).index(_sp_k)
            _sp_z3  = [float(_z3[i, _sp_idx]) for i in range(len(_dtes_fine))]
            fig_gex3d.add_trace(go.Scatter3d(
                x=[_sp_k]*len(_dtes_fine), y=list(_dtes_fine), z=_sp_z3,
                mode="lines+text", line=dict(color=SPOT_YELLOW, width=5),
                text=["SPOT" if i == 0 else "" for i in range(len(_dtes_fine))],
                textfont=dict(size=10, color=SPOT_YELLOW),
                hoverinfo="skip", showlegend=False,
            ))

            # Zero-gamma flip line
            _gf_k   = min(_strikes_fine, key=lambda k: abs(k - gamma_flip))
            _gf_idx = list(_strikes_fine).index(_gf_k)
            _gf_z3  = [float(_z3[i, _gf_idx]) for i in range(len(_dtes_fine))]
            fig_gex3d.add_trace(go.Scatter3d(
                x=[_gf_k]*len(_dtes_fine), y=list(_dtes_fine), z=_gf_z3,
                mode="lines+text", line=dict(color=T["amber"], width=3),
                text=["ZERO Γ" if i == 0 else "" for i in range(len(_dtes_fine))],
                textfont=dict(size=9, color=T["amber"]),
                hoverinfo="skip", showlegend=False,
            ))

            # Zero-plane
            _z_plane3 = np.zeros((len(_dtes_fine), len(_strikes_fine)))
            fig_gex3d.add_trace(go.Surface(
                x=list(_strikes_fine), y=list(_dtes_fine), z=_z_plane3,
                colorscale=[[0,"rgba(255,255,255,0.06)"],[1,"rgba(255,255,255,0.06)"]],
            ...
            fig_gex3d.update_layout(
                ...
                scene=dict(
                    ...
                    xaxis=dict(title=dict(text=f"{asset_toggle} Strike ($)",
                                          font=dict(color="#CCCCCC", size=11)), **_ax3s),
                    yaxis=dict(title=dict(text="DTE (days)",
                                          font=dict(color="#CCCCCC", size=11)), **_ax3s),
                    zaxis=dict(title=dict(text="GEX ($B)",
                                          font=dict(color="#CCCCCC", size=11)), **_ax3s_z),
                    camera=dict(
                        eye=dict(x=1.35, y=-1.70, z=0.65),
                        up=dict(x=0, y=0, z=1),
                        center=dict(x=0, y=0, z=-0.10),
                    ),
                    aspectmode="manual",
                    aspectratio=dict(x=2.6, y=1.2, z=1.0),
                    dragmode="turntable",
                ),
                showlegend=False,
                font=dict(family="JetBrains Mono, monospace", color="#CCCCCC", size=10),
                title=dict(
                    text=(f"<b>{asset_toggle}  GEX Surface · Strike × DTE</b>"
                          f"  ·  <span style='color:#00CC00;'>▲ Green = +GEX Dealers Long</span>"
                          f"  <span style='color:#CC0000;'>▼ Red = −GEX Dealers Short</span>"
                          f"  <span style='color:{SPOT_YELLOW};'>— Spot</span>"
                          f"  <span style='color:{T['amber']};'>— Zero Γ  |  Drag to rotate</span>"),
                    font=dict(size=11, color="#FFFFFF", family="JetBrains Mono"),
                    x=0.01),
                uirevision=f"gex3d_m1_{asset_toggle}",
            )
            ...
            st.warning(
                f"GEX surface: insufficient data "
        ...
        # OI ladder (full width, open_interest per strike × DTE bucket)
        st.markdown(
            f'<div style="font-family:JetBrains Mono,monospace;'
            f'font-size:10px;font-weight:600;color:#FFFFFF;'
            f'letter-spacing:1.5px;text-transform:uppercase;'
            f'margin-bottom:6px;padding:4px 0;">'
            f'OPEN INTEREST  ·  {asset_toggle}  ·  Highest OI = Resistance/Support Walls</div>',
            unsafe_allow_html=True)

        _raw_oi = raw_df.copy()
        if "dte" in _raw_oi.columns and "open_interest" in _raw_oi.columns:
            _raw_oi["_dte_int"] = _raw_oi["dte"].apply(
                lambda x: min(int(round(float(x))), 4)
                if pd.notna(x) else 5)
            _raw_oi = _raw_oi[_raw_oi["_dte_int"] < 5].copy()
            _oi_strikes = sorted([s for s in _raw_oi["strike"].unique()
                                  if spot_price * 0.82 <= s <= spot_price * 1.18])
            if _oi_strikes:
                # Weekday column headers
                _week_cols_oi = _get_week_trading_cols(_today, 5)
                _dte_hdrs_oi = [f"{wd.strftime('%a').upper()}  {wd.strftime('%m/%d')}"
                                for wd in _week_cols_oi]
                _oi_mat = np.zeros((len(_oi_strikes), 5), dtype=np.float64)
                for _jd in range(5):
                    _bk = _raw_oi[_raw_oi["_dte_int"] == _jd]
                    for _ii, _sk in enumerate(_oi_strikes):
                        _rws = _bk[_bk["strike"] == _sk]
                        if not _rws.empty:
                            _oi_mat[_ii, _jd] = float(
                                _rws["open_interest"].sum())
                ...
                _oi_max_idx = int(np.argmax(_oi_row_sums))
                _oi_min_idx = int(np.argmin(_oi_row_sums)) if len(_oi_row_sums) > 1 else 0
                _oi_max_sk  = _oi_strikes[_oi_max_idx]
                # Find top-2 OI strikes for call/put walls
                ...
                fig_oi.add_annotation(
                    x=4.55, y=_sp_oi_lbl, xref="x", yref="y",
                    text=f"  ▶ SPOT  ${spot_price:,.2f}",
                    showarrow=False, xanchor="left",
                    font=dict(size=9, color="#000000",
                              family="JetBrains Mono", weight=700),
                ...
                # Highest OI wall lines
                for _oi_wall_sk, _oi_wall_color, _oi_wall_lbl in [
                    (_oi_top2[0], "#66BBFF", "+ OI WALL"),
                    (_oi_top2[1] if len(_oi_top2) > 1 else None, "#3399DD", "OI WALL 2"),
                ]:
                    if _oi_wall_sk is None:
                        continue
                    _oi_wall_ylbl = f"{_oi_wall_sk:>8,.0f}  │  {equiv_label} {_oi_wall_sk * _hm_ratio:>8,.0f}"
                    fig_oi.add_shape(
                        type="line", x0=-0.5, x1=4.5,
                    ...
                    fig_oi.add_annotation(
                        x=4.55, y=_oi_wall_ylbl, xref="x", yref="y",
                        text=f"  {_oi_wall_lbl}",
                        showarrow=False, xanchor="left",
                ...
                _fig_h_oi  = max(380, min(720, _n_oi * _cell_h_oi + 90))
