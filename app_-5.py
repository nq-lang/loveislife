...
def render_es_chart_page():
    ...
    try:
        ...
        if _raw_es_data is not None and not _raw_es_data.empty:
            ...
            # Keep strikes in asset-native space (no ES ratio conversion)
            _gp2["strike_fut"] = _gp2["strike"].astype(float)
            _gp2 = _gp2.sort_values("strike_fut").reset_index(drop=True)

            # Filter ±20% from the asset's own spot (not ES spot)
            _sp_fut = float(_model_spot) if _model_spot else float(_spot2)
            if _sp_fut <= 0:
                _sp_fut = float(spot_es)
            _gp2 = _gp2[
    ...
    try:
        ...
        # Use asset-native spot for filtering (not ES spot)
        _sp3       = float(_model_spot) if _model_spot else float(spot_es)
        if _sp3 <= 0:
            _sp3 = float(spot_es)
        ...
        if _raw3_df is not None and not _raw3_df.empty and "flag" in _raw3_df.columns:
            ...
            # Keep strikes in asset-native space (no ES ratio conversion)
            _mm["strike_fut"] = _mm["strike"].astype(float)
            _mm = _mm.sort_values("strike_fut", ascending=True).reset_index(drop=True)

            # Filter ±15% from the asset's own spot
            _mm = _mm[
