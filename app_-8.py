...
def _render_daily_levels(ticker: str, spot: float, df, raw_df, T: dict):
    ...
    try:
        ...
        if spot > _gflip_0:
            ...
            _pulse_signals.append(("🟢", "ABOVE ZERO GAMMA",
                f"Spot {_pf(spot)} is above the gamma flip at {_pf(_gflip_0)}. "
                f"Dealer hedging flows are directionally SUPPORTIVE — positive gamma means dealers buy dips and sell rips, "
                f"dampening volatility and stabilising price.", T["green"]))
        else:
            ...
            _pulse_signals.append(("🔴", "BELOW ZERO GAMMA",
                ...
                f"Dealers are short gamma — they sell dips and buy rips, AMPLIFYING moves. "
                f"Volatility risk is elevated; larger swings expected.", T["red"]))
        ...
        if _cw_dist < _pw_dist:
            ...
            _pulse_signals.append(("🟡", "CALL WALL PROXIMITY",
                f"Spot is closer to the Call Wall ({_pf(_cw)}, {_cw_dist:.1f}% away) "
                f"than the Put Wall ({_pf(_pw)}, {_pw_dist:.1f}% away). "
                f"Heavy call OI nearby creates dealer SELLING pressure as they hedge — upside is capped near {_pf(_cw)}.", T["amber"]))
        else:
            _score_components.append(("Closer to Put Wall", +12, "bullish"))
            _pulse_signals.append(("🟢", "PUT WALL CUSHION",
                f"Spot has more room to the Call Wall ({_pf(_cw)}, {_cw_dist:.1f}% away). "
                f"Put Wall at {_pf(_pw)} ({_pw_dist:.1f}% below) provides dealer BUY support. "
                f"Upside continuation is structurally more open than downside.", T["green"]))
        ...
        if not df.empty and "gex_net" in df.columns:
            ...
            if _net_gex_total > 0:
                ...
                _pulse_signals.append(("🟢", "NET GEX POSITIVE",
                    f"Total dealer net gamma is +{_net_gex_total:.4f}B. Dealers are collectively LONG gamma — "
                    f"they act as a stabilising force, suppressing vol and buying weakness. Short gamma squeeze risk is LOW.", T["green"]))
            else:
                ...
                _pulse_signals.append(("🔴", "NET GEX NEGATIVE",
                    ...
                    f"directional moves get AMPLIFIED. Volatility risk elevated; be cautious of momentum overshoots.", T["red"]))
                ...
                for _lvnat, _lvc, _lvnm, _lvd, _lvw in _m2_key_levels:
                    ...
                    if _native_spot * 0.75 <= _lvnat <= _native_spot * 1.25:
                        ...
                        fig_gp.add_annotation(
                            ...
                            text=f" {_lvnm}  ${_lvnat:,.1f} → {_fut_lbl_m} ${_fut_at_lvl:,.0f} ",
                            showarrow=False, xanchor="center",
                            font=dict(size=7.5, color=_lvc, family="JetBrains Mono"),
                            bgcolor=T["bg1"], bordercolor=_lvc, borderwidth=1,
                            borderpad=2, opacity=0.93,
                        )
...
def dashboard():
    ...
    if st.session_state.radar_mode == "GEX":
        ...
        if df_intra.empty:
            ...
            for i_, (_, rg) in enumerate(_top5_pos.iterrows()):
                ...
                if y_lo_i*0.97 <= lvl <= y_hi_i*1.03:
                    ...
                    fig_ig.add_annotation(
                        ...
                        text=f" +{i_+1} ${_native_s:,.0f}→{lvl:,.0f}  {rg['gex_net']:+.3f}B ",
                        showarrow=False, xanchor="left",
            ...
            # Y-axis values are already in futures-equivalent space (_sym_ratio applied)
            # Label shows: level value (futures equiv) with index native price
            _ig_basis = _get_futures_basis(asset_toggle)
            _ig_ratio = _ig_basis["ratio"] if _ig_basis["ratio"] > 0 else 1.0
            _ig_flbl  = _ig_basis["label"]
            for lvl_v, lvl_c, lvl_dash, lvl_lw, lvl_nm in [
                ...
                if y_lo_i*0.97 <= lvl_v <= y_hi_i*1.03:
                    # For non-spot levels: lvl_v is already in futures space
                    # (strike * _sym_ratio). Show both index native and futures.
                    if lvl_nm != "LAST":
                        _native_strike = lvl_v / _ig_ratio if _ig_ratio > 0 else lvl_v
                        _ann_text = f"  {lvl_nm}  ${_native_strike:,.1f} → {_ig_flbl} ${lvl_v:,.1f} "
                    else:
                        _ann_text = f"  {lvl_nm}  {lvl_v:,.2f} "
            ...
            fig_ig.update_layout(
                ...
                margin=dict(t=20, r=160, b=36, l=66),
                hoverlabel=dict(
