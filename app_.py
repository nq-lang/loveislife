...
def dashboard():
    ...
    if st.session_state.radar_mode == "GEX":
        ...
        if df_intra.empty:
            ...
            # IMPORTANT: use _sym_ratio (already computed above from equiv_mult / basis engine)
            # for the back-calculation of the native strike — this guarantees the annotation
            # level displayed matches the line drawn, eliminating any cache-timing divergence
            # that would occur if we re-fetched the ratio via a second _get_futures_basis call.
            for lvl_v, lvl_c, lvl_dash, lvl_lw, lvl_nm in [
                ...
                if y_lo_i*0.97 <= lvl_v <= y_hi_i*1.03:
                    if lvl_nm != "LAST":
                        # Compact tag: show only the futures-equivalent price
                        # (same value the line is drawn at) — avoids text overflow
                        _ann_text = f" {lvl_nm} ${lvl_v:,.0f} "
                    else:
                        _ann_text = f" LAST {lvl_v:,.2f} "
            ...
            fig_ig.update_layout(
                ...
                margin=dict(t=20, r=185, b=36, l=66),
                hoverlabel=dict(
