...
def build_gex_landscape(df, spot_price):
    ...
    fig = go.Figure(go.Surface(
        ...
        contours=dict(z=dict(show=True, usecolormap=True, highlightcolor="white", highlightwidth=1.5)),
        hovertemplate=(
            "Price: $%{x:.2f}<br>"
            "Time: %{y}<br>"
            "Net GEX: %{z:.4f}B"
...
def render_volatility_lab_page():
    ...
    exp_days  = [int(e.replace("d","")) for e in exp_lbls]
    ...
    # IV Surface + Skew — same page, no tabs

    # ── IMAGE-MATCHING colorscale: cyan→blue→purple→magenta→red ─────────────
    _SURF_CS = [
        [0.00, "#050810"],   # floor
        [0.08, "#041530"],
        [0.18, "#062880"],
    ...
    if True:  # surface
        z3   = surf_arr.tolist()
        vmin = float(np.nanmin(surf_arr))
        vmax = float(np.nanmax(surf_arr))
        xlbl = [f"{m*100:.0f}%" for m in mono_g]

        fig3 = go.Figure(go.Surface(
            x=xlbl, y=exp_lbls, z=z3,
            colorscale=_SURF_CS, cmin=vmin, cmax=vmax,
            showscale=True, opacity=0.93,
            # contours omitted — unstable in older plotly versions
            colorbar=dict(
                title=dict(text="IV %",
            ...
            lighting=dict(ambient=0.65, diffuse=0.9, specular=0.25,
                          roughness=0.45, fresnel=0.0),
        ))
        # ATM spine
        ...
        fig3.add_trace(go.Scatter3d(
            x=["100%"]*len(exp_lbls), y=exp_lbls, z=atm_z,
            mode="lines+markers",
            line=dict(color="#FFFFFF", width=2.5),
            marker=dict(size=3, color="#FFFFFF"),
            hovertemplate="ATM %{y}  IV:%{z:.2f}%<extra>ATM</extra>",
        ))
        fig3.update_layout(
            paper_bgcolor=_BG, height=560,
            margin=dict(l=0, r=10, b=0, t=38),
            scene=dict(
                xaxis=dict(title=dict(text="Moneyness",
        ...
        # ── Grid table below surface ─────────────────────────────────────────
        st.markdown(
            f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;'
            f'color:{_TXT3};letter-spacing:2px;text-transform:uppercase;'
            f'margin:10px 0 5px 0;">IV GRID  ·  DTE × MONEYNESS  (Select cells to compare)</div>',
            unsafe_allow_html=True)
    ...
    if True:  # skew
        ...
        if c_str and p_str and c_iv and p_iv:
            ...
            _skew_data = [
                ...
                ("Feller Cond.",    f"{'✓' if fok else '✗'} 2κθ={2*kappa*theta:.3f}", "#44FF88" if fok else "#FF4422"),
            ]
            _sc = st.columns(5)
...
_INTRA_SYM_MAP = {
    ...
    "GLD": {"futures": "GC=F", "label": "Gold (GC Fut)", "opt": "GLD", "ratio_fn": "gld"},
}
...
def render_intraday_gex_page():
    ...
    if "volume" in df_c.columns:
        ...
        fig.add_trace(go.Bar(
            x=x_ts, y=df_c["volume"].fillna(0),
            marker=dict(color=vc, line=dict(width=0), opacity=0.6),
            name="Volume", hovertemplate="Vol: %{y:,.0f}<extra></extra>",
        ), row=2, col=1)
    ...
    fig.update_xaxes(showgrid=True, gridcolor=LINE, gridwidth=1,
                     ...
                     spikecolor=TXT3, spikedash="dot", row=1, col=1)
    fig.update_xaxes(showgrid=True, gridcolor=LINE,
                     tickfont=dict(size=8, family="JetBrains Mono", color=TXT3),
                     row=2, col=1)
    fig.update_yaxes(showgrid=True, gridcolor=LINE, gridwidth=1,
                     tickfont=dict(size=9, family="JetBrains Mono", color=TXT3),
                     range=[y_lo - y_pad, y_hi + y_pad], row=1, col=1)
...
def _fetch_fred_series(series_id: str, limit: int = 60) -> pd.Series:
    """Fetch from FRED API (no key required for public series)."""
    try:
...
def _compute_macro_regime() -> dict:
    ...
    Four-quadrant macro regime:
      Growth Z + Inflation Z → Goldilocks / Reflation / Stagflation / Deflation
    Uses rolling 5-yr Z-scores.  Returns dict with regime + all metrics.
    """
    result = {
    ...
    try:
        ...
        # HY OAS
        hy = _fetch_fred_series("BAMLH0A0HYM2", 5)
        if not hy.empty:
            result["hy_oas"] = float(hy.iloc[-1])

        # 10Y-2Y spread
        sp = _fetch_fred_series("T10Y2Y", 5)
        if not sp.empty:
            result["spread_10_2"] = float(sp.iloc[-1])

        # VRP from live VIX vs realized vol
        try:
        ...
        # VRP signal
        if result["vrp"] > 0.001:
            score += 2; result["signals"]["vrp"] = "POSITIVE (vol underpriced)"
        else:
            score -= 1; result["signals"]["vrp"] = "NEGATIVE (vol rich)"
        # Growth Z
        score += int(np.clip(gz * 2, -3, 3))
        # Inflation penalty
        score -= int(np.clip(iz * 1.5, 0, 3))
        # HY OAS (stress indicator)
        hy_v = result["hy_oas"]
        if hy_v > 5.0:
            score -= 2; result["signals"]["hy"] = f"ELEVATED {hy_v:.2f} (credit stress)"
        elif hy_v > 3.5:
            score -= 1; result["signals"]["hy"] = f"MODERATE {hy_v:.2f}"
        else:
            score += 1; result["signals"]["hy"] = f"TIGHT {hy_v:.2f} (benign)"
        # Yield curve
        sp_v = result["spread_10_2"]
        if sp_v < 0:
            score -= 2; result["signals"]["curve"] = "INVERTED (recession risk)"
        else:
            score += 1; result["signals"]["curve"] = f"NORMAL +{sp_v:.2f}%"

        result["composite"] = int(np.clip(score, -10, 10))

        # Thesis verdict
        if result["composite"] >= 3:
            result["thesis"] = "LEAN BULLISH"
        elif result["composite"] <= -3:
            result["thesis"] = "LEAN BEARISH"
        else:
            result["thesis"] = "NEUTRAL / WAIT"

        # Risk factors
        rf = []
        if iz > 0.5:
            rf.append("⚠️ Elevated inflation Z-score — hawkish Fed risk")
        if hy_v > 5.0:
            rf.append("⚠️ HY OAS elevated — credit market stress")
        if sp_v < 0:
            rf.append("⚠️ Inverted yield curve — recession probability elevated")
        if result["vrp"] < -0.001:
            rf.append("⚠️ Negative VRP — implied vol cheap vs realised")
...
def render_macro_shi_page():
    ...
    mc = st.columns(5)
    _mdata = [
        ("CPI YoY",       f"{macro['cpi_yoy']:.2f}%",    T["red"] if macro["cpi_yoy"] > 3 else T["amber"]),
        ("Unemployment",  f"{macro['unemployment']:.1f}%", T["green"] if macro["unemployment"] < 4.5 else T["red"]),
        ("HY OAS",        f"{macro['hy_oas']:.2f}",       T["red"] if macro["hy_oas"] > 5 else T["green"]),
        ("10Y-2Y Spread", f"{macro['spread_10_2']:+.2f}%", T["red"] if macro["spread_10_2"] < 0 else T["green"]),
        ("VRP",           f"{macro['vrp']:.4f}",           T["green"] if macro["vrp"] > 0 else T["red"]),
    ]
    for col_, (lbl, val, col) in zip(mc, _mdata):
        with col_:
            st.markdown(f"""
            <div style="background:{T['bg1']};border:1px solid {T['line2']};
                        border-top:2px solid {col};border-radius:3px;
    ...
    for sig_key, sig_val in macro["signals"].items():
        sig_col = T["green"] if any(w in sig_val for w in ["POSITIVE","TIGHT","NORMAL"]) else T["red"]
        st.markdown(f"""
    ...
    st.markdown(
        f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;'
        f'color:{T["t3"]};padding:8px 0;letter-spacing:1px;">'
        f'Data: FRED (UNRATE, CPIAUCSL, BAMLH0A0HYM2, T10Y2Y) · '
...
_asset_options = ["SPX", "NDX", "GLD"]
_cur = st.session_state.asset_choice if st.session_state.asset_choice in _asset_options else "SPX"
...
_PAGES = {
    "gex":              "📊  GEX DASHBOARD",
    "mispricings_macro":"🔬  MISPRICINGS & MACRO",
...
def dashboard():
    ...
    if asset_toggle not in ["SPX", "NDX", "GLD"]:
        asset_toggle = "SPX"
        st.session_state.asset_choice = "SPX"
    max_exp      = st.session_state.max_exp
    ...
    _modes_list  = ["GEX", "REPLAY"]
    _labels_list = ["OI GEX", "⏱ Replay"]
    ...
    if st.session_state.radar_mode not in ("GEX", "REPLAY"):
    ...
    if st.session_state.radar_mode == "GEX":
        _render_gex_four_models(
            asset_toggle, spot_price, df, raw_df,
            equiv_label, equiv_mult, gamma_flip, call_wall, put_wall,
            max_pain, vol_trigger, mom_wall, mom_val, T)
    ...
    # ── HEATMAP ───────────────────────────────────────────────────────────
        ...
        _label = f"{'☑' if st.session_state.heat_dual else '☐'}  SPX + NDX DUAL"
        ...
        # ── Gaussian 2-D GEX Distribution Heatmap ────────────────────────
        def make_heatmap_fig(raw, spot, lbl, gflip, c_wall=None, p_wall=None):
            from scipy.ndimage import gaussian_filter as _gf_2d
            strikes, expiries, z = build_heatmap_matrix(raw, spot, mode="oi")
            if strikes is None:
                f = go.Figure()
                f.add_annotation(text="No heatmap data available", showarrow=False,
            ...
            def _walls_from_raw(rdf, sp):
                if rdf is None or rdf.empty:
                    return sp * 1.01, sp * 0.99
                try:
                    ag = rdf.groupby("strike").agg(
                        call_gex=("call_gex","sum"), put_gex=("put_gex","sum")
                    ).reset_index()
                    ag["gex_net"] = ag["call_gex"] + ag["put_gex"]
                    p_ = ag[ag["gex_net"] > 0]; n_ = ag[ag["gex_net"] < 0]
                    cw = float(p_.loc[p_["gex_net"].idxmax(),"strike"]) if not p_.empty else sp*1.01
                    pw = float(n_.loc[n_["gex_net"].idxmin(),"strike"]) if not n_.empty else sp*0.99
                    return cw, pw
                except Exception:
                    return sp*1.01, sp*0.99
            ...
            # Normalise + Gaussian blur for smooth 2-D distribution look
            z_norm = z_arr.copy()
            col_abs = np.abs(z_norm).max(axis=0, keepdims=True)
            col_abs = np.where(col_abs < 1e-12, 1e-12, col_abs)
            z_norm  = z_norm / col_abs
            z_blur  = _gf_2d(z_norm, sigma=[1.8, 0.9])
            z_disp  = np.sign(z_blur) * np.power(np.abs(z_blur), 0.5)
            net_per_strike = z_arr.sum(axis=1)

            # Diverging red/green colorscale
            _bg = T["bg"]
            gauss_cs = [
                [0.000, "#FF4500"], [0.060, T["red"]],  [0.160, "#8B0000"],
                [0.300, "#3D0000"], [0.440, "#1A0000"],  [0.490, "#0A0000"],
                [0.500, _bg],
                [0.510, "#00100A"], [0.560, "#003318"],  [0.700, "#006B35"],
                [0.840, T["green"]],[0.940, "#00E5A0"],  [1.000, "#ADFFF0"],
            ]
            ...
            fig.add_trace(go.Heatmap(
                z=z_disp, x=exp_labels, y=strike_nums, customdata=z_arr,
                colorscale=gauss_cs, zmid=0, zmin=-1.0, zmax=1.0, zsmooth="best",
                colorbar=dict(
                    title=dict(text="GEX Intensity",
                    ...
                    thickness=10, len=0.70, x=0.825, xanchor="right",
                    outlinewidth=0, bgcolor=T["bg1"],
                ),
                hovertemplate=(
                    f"<b>{lbl}</b><br>Strike: <b>${{%{{y:.2f}}}}</b><br>"
                    "Expiry: %{x}<br>Net OI-GEX: <b>%{customdata:.4f}B</b><extra></extra>"
                ),
                xgap=1.5, ygap=0.8,
            ), row=1, col=1)

            _sp_col = [T["bar_pos"] if v >= 0 else T["bar_neg"] for v in net_per_strike]
            _sp_w = ([(strike_nums[i+1]-strike_nums[i])*0.78 if i+1<len(strike_nums)
                       else (strike_nums[-1]-strike_nums[-2])*0.78
                       for i in range(len(strike_nums))] if len(strike_nums)>1 else [1.0])
            fig.add_trace(go.Bar(
                x=net_per_strike, y=strike_nums, orientation="h",
                marker=dict(color=_sp_col, line=dict(width=0), opacity=0.90),
                width=_sp_w,
                hovertemplate="Strike $%{y:.2f}<br>Net GEX: <b>%{x:.4f}B</b><extra></extra>",
            ), row=1, col=2)

            _lvl_defs = [
                (spot,   T["t1"],    2.5, "solid", f"SPOT  ${spot:.2f}"),
                (gflip,  T["amber"], 1.3, "dot",   f"ZERO Γ  ${gflip:.2f}"),
                (c_wall, T["green"], 1.3, "dash",  f"CALL WALL  ${c_wall:.2f}"),
                (p_wall, T["red"],   1.3, "dash",  f"PUT WALL  ${p_wall:.2f}"),
            ]
            s_lo, s_hi = strike_nums[0], strike_nums[-1]
            for _pr, _cl, _lw, _ds, _al in _lvl_defs:
                if not (s_lo*0.995 <= _pr <= s_hi*1.005):
                    continue
                fig.add_shape(type="line", x0=0, x1=1, xref="paper",
                               y0=_pr, y1=_pr, yref="y",
                               line=dict(color=_cl, width=_lw, dash=_ds), layer="above")
                fig.add_annotation(x=1.01, y=_pr, xref="paper", yref="y",
                                   text=f" {_al} ", showarrow=False, xanchor="left",
                                   font=dict(size=8, color=_cl, family="JetBrains Mono"),
                                   bgcolor=T["bg1"], bordercolor=_cl,
                                   borderwidth=1, borderpad=4, opacity=0.95)

            fig.update_layout(
                **PLOTLY_BASE, barmode="relative", bargap=0.06,
                title=dict(
                    text=(f"<b>{lbl}</b>  ·  GEX  2-D GAUSSIAN DISTRIBUTION  "
                          f"<span style='font-size:9px;color:{TEXT3};'>"
                          f"Gaussian-smoothed  ·  red=short Γ  green=long Γ</span>"),
                    font=dict(size=11, color=TEXT2, family="JetBrains Mono"),
                    x=0.01, xanchor="left"),
                height=700, margin=dict(t=46, r=210, b=64, l=72),
                hoverlabel=dict(bgcolor=T["bg2"], bordercolor=T["line_bright"],
                                font=dict(family="JetBrains Mono", size=10, color=T["t1"])),
            )
            fig.update_xaxes(type="category", tickangle=-30,
                              tickfont=dict(size=10, family="JetBrains Mono", color=TEXT2),
                              showgrid=False, linecolor=T["line2"], linewidth=1,
            ...
            fig.update_yaxes(tickprefix="$",
                              ...
                              row=1, col=1)
            fig.update_xaxes(tickfont=dict(size=8, family="JetBrains Mono", color=TEXT3),
            ...
            fig.update_yaxes(tickprefix="$",
                              tickfont=dict(size=9, family="JetBrains Mono", color=TEXT2),
                              range=[spot*0.953, spot*1.047],
                              showgrid=True, gridcolor=T["line"], row=1, col=2)
            return fig
        # ── END make_heatmap_fig ──────────────────────────────────────────

        _hm_cfg = {"displayModeBar": True, "displaylogo": False,
                   "scrollZoom": False,
                   "modeBarButtonsToRemove": ["sendDataToCloud","zoom2d","lasso2d",
                                               "select2d","autoScale2d","zoomIn2d","zoomOut2d"]}
        dual = st.session_state.heat_dual
        if dual:
            with st.spinner("Fetching SPX + NDX heatmap data…"):
                spx_hm = fetch_options_data_heatmap("SPX")
                ndx_hm = fetch_options_data_heatmap("NDX")
            dc1, dc2 = st.columns(2)
            with dc1:
                _sd, _ss, _sr = fetch_options_data("SPX", 4)
                _gf2, _cw2, _pw2, _ = compute_key_levels(_sd, spx_hm[1], _sr) if not _sd.empty \
                    else (spx_hm[1], spx_hm[1]*1.01, spx_hm[1]*0.99, spx_hm[1])
                st.plotly_chart(make_heatmap_fig(spx_hm[2], spx_hm[1], "SPX", _gf2, _cw2, _pw2),
                                use_container_width=True, config=_hm_cfg)
            with dc2:
                _nd, _ns, _nr = fetch_options_data("NDX", 4)
                _gf3, _cw3, _pw3, _ = compute_key_levels(_nd, ndx_hm[1], _nr) if not _nd.empty \
                    else (ndx_hm[1], ndx_hm[1]*1.01, ndx_hm[1]*0.99, ndx_hm[1])
                st.plotly_chart(make_heatmap_fig(ndx_hm[2], ndx_hm[1], "NDX", _gf3, _cw3, _pw3),
                                use_container_width=True, config=_hm_cfg)
        else:
            with st.spinner("Fetching heatmap data…"):
                hm_data = fetch_options_data_heatmap(asset_toggle)
            st.plotly_chart(
                make_heatmap_fig(hm_data[2], hm_data[1], asset_toggle,
            ...
            # ── Gaussian-smooth → 2-D distribution ───────────────────────
            # Normalise each column, then apply 2-D Gaussian kernel
            # This produces smooth bell-curve-like intensity blobs per strike
            z_norm = z_arr.copy()
            col_abs = np.abs(z_norm).max(axis=0, keepdims=True)
            col_abs = np.where(col_abs < 1e-12, 1e-12, col_abs)
            z_norm  = z_norm / col_abs                    # [-1, 1]

            # Gaussian blur: sigma=(1.8 rows, 0.9 cols) → smooth Gaussian blobs
            z_blur  = _gf_2d(z_norm, sigma=[1.8, 0.9])
            # Power-compression to bring mid-intensity zones alive
            z_disp  = np.sign(z_blur) * np.power(np.abs(z_blur), 0.5)

            net_per_strike = z_arr.sum(axis=1)

            # ── Diverging red/green colorscale ────────────────────────────
            # Strong negative = vivid red, fading red = weak negative
            # Strong positive = vivid green, fading green = weak positive
            # Zero GEX = background (black/dark)
            _bg = T["bg"]
            gauss_cs = [
                [0.000, "#FF4500"],  # max negative → vivid red-orange
                [0.060, T["red"]],
                [0.160, "#8B0000"],
                [0.300, "#3D0000"],
                [0.440, "#1A0000"],
                [0.490, "#0A0000"],
                [0.500, _bg],        # zero GEX → background
                [0.510, "#00100A"],
                [0.560, "#003318"],
                [0.700, "#006B35"],
                [0.840, T["green"]],
                [0.940, "#00E5A0"],
                [1.000, "#ADFFF0"],  # max positive → bright cyan-green
            ]
            ...
            fig.add_trace(go.Heatmap(
                z=z_disp, x=exp_labels, y=strike_nums, customdata=z_arr,
                colorscale=gauss_cs, zmid=0, zmin=-1.0, zmax=1.0,
                zsmooth="best",   # smooth interpolation for Gaussian look
                colorbar=dict(
                    title=dict(text="GEX Intensity",
                               font=dict(size=8, color=TEXT3, family="JetBrains Mono"),
                               side="right"),
                ...
                hovertemplate=(
                    f"<b>{lbl}</b><br>Strike: <b>${{%{{y:.2f}}}}</b><br>"
                    "Expiry: %{x}<br>Net OI-GEX: <b>%{customdata:.4f}B</b>"
                    "<extra></extra>"
                ),
                xgap=1.5, ygap=0.8,
            ), row=1, col=1)

            # Side bar — net GEX per strike
            _sp_col = [T["bar_pos"] if v >= 0 else T["bar_neg"] for v in net_per_strike]
            _sp_w = ([(strike_nums[i+1]-strike_nums[i])*0.78 if i+1 < len(strike_nums)
                       else (strike_nums[-1]-strike_nums[-2])*0.78
            ...
            # Key level lines + pill annotations
            _lvl_defs = [
                (spot,   T["t1"],    2.5, "solid", f"SPOT  ${spot:.2f}"),
                (gflip,  T["amber"], 1.3, "dot",   f"ZERO Γ  ${gflip:.2f}"),
                (c_wall, T["green"], 1.3, "dash",  f"CALL WALL  ${c_wall:.2f}"),
            ...
            fig.update_layout(
                **PLOTLY_BASE, barmode="relative", bargap=0.06,
                title=dict(
                    text=(f"<b>{lbl}</b>  ·  GEX  2-D GAUSSIAN DISTRIBUTION  "
                          f"<span style='font-size:9px;color:{TEXT3};'>"
                          f"Gaussian-smoothed intensity  ·  red=short Γ  green=long Γ</span>"),
                    font=dict(size=11, color=TEXT2, family="JetBrains Mono"),
                    x=0.01, xanchor="left"),
                height=700, margin=dict(t=46, r=210, b=64, l=72),
                hoverlabel=dict(bgcolor=T["bg2"], bordercolor=T["line_bright"],
...
if _active == "gex":
    dashboard()
elif _active == "mispricings_macro":
    render_mispricings_macro_page()
...
if _active in ("gex","mispricings_macro") and _m:
...
# ═══════════════════════════════════════════════════════════════════════════════
# NEW: ASSET CONFIG + 4-MODEL GEX DASHBOARD
# ═══════════════════════════════════════════════════════════════════════════════
_ACFG = {
    "SPX": {"opt":"SPX","fut":"ES=F","label":"ES Futures","und":"ES"},
    "NDX": {"opt":"NDX","fut":"NQ=F","label":"NQ Futures","und":"NQ"},
    "GLD": {"opt":"GLD","fut":"GC=F","label":"GC Futures","und":"GC"},
}

def _asset_ratio(asset):
    try:
        c = _ACFG.get(asset, _ACFG["SPX"])
        fp = _fetch_yahoo_price(c["fut"])
        if asset=="SPX": op=_fetch_yahoo_price("^GSPC")
        elif asset=="NDX": op=_fetch_yahoo_price("^NDX")
        elif asset=="GLD": op=_fetch_yahoo_price("GLD")
        else: return 1.0
        return fp/op if op>0 else 1.0
    except: return {"SPX":1.0,"NDX":1.0,"GLD":10.0}.get(asset,1.0)

@st.cache_data(ttl=60,show_spinner=False)
def _candles(asset,interval="5m"):
    c=_ACFG.get(asset,_ACFG["SPX"])
    try:
        url=f"https://query1.finance.yahoo.com/v8/finance/chart/{c['fut']}?interval={interval}&range=1d"
        r=_requests.get(url,headers={"User-Agent":"Mozilla/5.0","Accept":"application/json"},timeout=12)
...
def _agex(asset):
    c=_ACFG.get(asset,_ACFG["SPX"])
    try:
        df_g,spot_g,raw_g=fetch_options_data(c["opt"],max_expirations=3)
        if df_g.empty: return {}
        gf,cw,pw,mp=compute_key_levels(df_g,spot_g,raw_g)
        vt,_,_=compute_intraday_levels(df_g,spot_g)
        ratio=_asset_ratio(asset)
        p5=df_g[df_g["gex_net"]>0].nlargest(5,"gex_net")["strike"].tolist()
        n5=df_g[df_g["gex_net"]<0].nsmallest(5,"gex_net")["strike"].tolist()
        cv=lambda v:round(v*ratio,2)
        return {"spot_opt":spot_g,"spot_fut":cv(spot_g),"gamma_flip":cv(gf),
                "call_wall":cv(cw),"put_wall":cv(pw),"max_pain":cv(mp),"vol_trigger":cv(vt),
                "pos5_fut":[cv(s) for s in p5],"neg5_fut":[cv(s) for s in n5],
                "pos5_opt":p5,"neg5_opt":n5,"ratio":ratio,"df_gex":df_g,"raw_df":raw_g}
    except: return {}

def _fig_heatmap_candle(asset,interval,T):
    c=_ACFG.get(asset,_ACFG["SPX"]); BG=T["bg"]; und=c["und"]
    dfc=_candles(asset,interval); lvls=_agex(asset)
    if dfc.empty:
        fig=go.Figure()
        fig.add_annotation(text=f"Awaiting {c['label']} data",showarrow=False,
            font=dict(color=T["t3"],size=13,family="JetBrains Mono"),xref="paper",yref="paper",x=0.5,y=0.5)
        fig.update_layout(template="none",paper_bgcolor=BG,plot_bgcolor=BG,height=480)
        return fig
    xts=dfc["ts"].dt.strftime("%H:%M").tolist()
    spot=float(dfc["close"].iloc[-1])
    ylo=float(dfc["low"].min())*0.993; yhi=float(dfc["high"].max())*1.007
    fig=make_subplots(rows=2,cols=1,shared_xaxes=True,vertical_spacing=0.02,row_heights=[0.80,0.20])
    if lvls:
        gf=lvls.get("gamma_flip",spot); cw=lvls.get("call_wall",spot*1.01); pw=lvls.get("put_wall",spot*0.99)
        fig.add_shape(type="rect",x0=0,x1=1,xref="paper",y0=gf,y1=min(cw*1.015,yhi),yref="y",
                      fillcolor="rgba(0,200,100,0.08)",line=dict(width=0),layer="below")
        fig.add_shape(type="rect",x0=0,x1=1,xref="paper",y0=max(pw*0.985,ylo),y1=gf,yref="y",
                      fillcolor="rgba(220,40,40,0.08)",line=dict(width=0),layer="below")
        for i,s in enumerate(lvls.get("pos5_fut",[])):
            if ylo*0.97<=s<=yhi*1.03:
                fig.add_shape(type="rect",x0=0,x1=1,xref="paper",y0=s-2,y1=s+2,yref="y",
                              fillcolor=f"rgba(0,229,160,{max(0.04,0.16-i*0.03):.2f})",line=dict(width=0),layer="below")
        for i,s in enumerate(lvls.get("neg5_fut",[])):
            if ylo*0.97<=s<=yhi*1.03:
                fig.add_shape(type="rect",x0=0,x1=1,xref="paper",y0=s-2,y1=s+2,yref="y",
                              fillcolor=f"rgba(255,56,96,{max(0.04,0.16-i*0.03):.2f})",line=dict(width=0),layer="below")
    fig.add_trace(go.Candlestick(x=xts,open=dfc["open"],high=dfc["high"],low=dfc["low"],close=dfc["close"],
        increasing_line_color=T["green"],decreasing_line_color=T["red"],
        increasing_fillcolor=T["green"],decreasing_fillcolor=T["red"],line_width=0.9,
        hovertext=[f"{und} O:{o:.2f} H:{h:.2f} L:{l:.2f} C:{cl:.2f}" for o,h,l,cl in zip(dfc["open"],dfc["high"],dfc["low"],dfc["close"])],
        hoverinfo="text+x",showlegend=False),row=1,col=1)
    if "volume" in dfc.columns:
        tp=(dfc["high"]+dfc["low"]+dfc["close"])/3; vol=dfc["volume"].fillna(0).clip(lower=0)
        cv2=vol.cumsum(); vwap=np.where(cv2>0,(tp*vol).cumsum()/cv2,tp)
        fig.add_trace(go.Scatter(x=xts,y=vwap,mode="lines",line=dict(color=T["amber"],width=1.3,dash="dot"),
            hovertemplate="VWAP %{y:.2f}<extra>VWAP</extra>",showlegend=False),row=1,col=1)
    if lvls:
        ratio=lvls.get("ratio",1.0)
        for key,col,dash,lw,lbl in [("call_wall",T["green"],"dash",1.8,"Call Wall"),
                                     ("put_wall",T["red"],"dash",1.8,"Put Wall"),
                                     ("gamma_flip",T["amber"],"dot",1.4,"Zero Γ"),
                                     ("vol_trigger",T["blue"],"dashdot",1.0,"Vol Trig")]:
            val=lvls.get(key)
            if val and ylo*0.97<=val<=yhi*1.03:
                ov=round(val/ratio,0) if ratio else val
                fig.add_hline(y=val,row=1,col=1,line_dash=dash,line_color=col,line_width=lw,
                    annotation_text=f" {lbl} {val:.0f}|{und}:{ov:.0f}",annotation_position="right",
                    annotation_font=dict(size=8,color=col,family="JetBrains Mono"))
        for i,(sf,so) in enumerate(zip(lvls.get("pos5_fut",[]),lvls.get("pos5_opt",[]))):
            if ylo*0.97<=sf<=yhi*1.03:
                fig.add_hline(y=sf,row=1,col=1,line_dash="dot",line_color=T["green"],line_width=0.7,
                    annotation_text=f" +Γ{i+1} {sf:.0f}|{und}:{so:.0f}",annotation_position="right",
                    annotation_font=dict(size=7.5,color=T["green"],family="JetBrains Mono"))
        for i,(sf,so) in enumerate(zip(lvls.get("neg5_fut",[]),lvls.get("neg5_opt",[]))):
            if ylo*0.97<=sf<=yhi*1.03:
                fig.add_hline(y=sf,row=1,col=1,line_dash="dot",line_color=T["red"],line_width=0.7,
                    annotation_text=f" -Γ{i+1} {sf:.0f}|{und}:{so:.0f}",annotation_position="right",
                    annotation_font=dict(size=7.5,color=T["red"],family="JetBrains Mono"))
        fig.add_hline(y=spot,row=1,col=1,line_dash="solid",line_color=T["t1"],line_width=2,
            annotation_text=f" LAST {spot:.2f}",annotation_position="right",
            annotation_font=dict(size=9,color=T["t1"],family="JetBrains Mono"))
    if "volume" in dfc.columns:
        vc=[T["green"] if cl>=o else T["red"] for cl,o in zip(dfc["close"],dfc["open"])]
        fig.add_trace(go.Bar(x=xts,y=dfc["volume"].fillna(0),marker=dict(color=vc,line=dict(width=0),opacity=0.55),
            hovertemplate="Vol: %{y:,.0f}<extra></extra>",showlegend=False),row=2,col=1)
    ratio_s=f"×{lvls.get('ratio',1.0):.4f}" if lvls else ""
    ypad=(yhi-ylo)*0.03
    fig.update_layout(template="none",paper_bgcolor=BG,plot_bgcolor=BG,
        font=dict(family="JetBrains Mono, monospace",color=T["chart_t2"],size=10),
        showlegend=False,height=500,dragmode="pan",margin=dict(t=38,r=160,b=36,l=68),
        xaxis_rangeslider_visible=False,
        title=dict(text=f"<b>{c['label']}</b>  ·  GEX Heatmap  ·  {interval}  ·  {c['opt']} {ratio_s}",
            font=dict(size=11,color=T["t1"],family="JetBrains Mono"),x=0.01,xanchor="left"),
        hoverlabel=dict(bgcolor=T["bg2"],bordercolor=T["line_bright"],
            font=dict(family="JetBrains Mono",size=10,color=T["t1"])))
    for row in [1,2]:
        fig.update_xaxes(showgrid=True,gridcolor=T["chart_line"],
            tickfont=dict(size=8,family="JetBrains Mono"),showspikes=True,
            spikemode="across",spikethickness=1,spikecolor=T["chart_t3"],spikedash="dot",row=row,col=1)
    fig.update_yaxes(showgrid=True,gridcolor=T["chart_line"],
        tickfont=dict(size=9,family="JetBrains Mono"),range=[ylo-ypad,yhi+ypad],row=1,col=1)
    fig.update_yaxes(showgrid=True,gridcolor=T["chart_line"],tickfont=dict(size=8),row=2,col=1)
    return fig

def _fig_gex_3d(asset,T):
    c=_ACFG.get(asset,_ACFG["SPX"]); BG=T["bg"]; und=c["und"]
    lvls=_agex(asset)
    if not lvls or "raw_df" not in lvls or lvls["raw_df"].empty:
        fig=go.Figure()
        fig.add_annotation(text="Loading 3D GEX surface...",showarrow=False,
            font=dict(color=T["t3"],size=12,family="JetBrains Mono"),xref="paper",yref="paper",x=0.5,y=0.5)
        fig.update_layout(template="none",paper_bgcolor=BG,plot_bgcolor=BG,height=500)
        return fig
    raw_df=lvls["raw_df"]; spot=lvls["spot_opt"]; ratio=lvls.get("ratio",1.0)
    d=raw_df.copy()
    d=d[(d["strike"]>=spot*0.91)&(d["strike"]<=spot*1.09)]
    if d.empty:
        fig=go.Figure(); fig.add_annotation(text="No data ±9%",showarrow=False,font=dict(color=T["t3"],size=12))
        fig.update_layout(template="none",paper_bgcolor=BG,height=480); return fig
    d["net_gex"]=d["call_gex"]+d["put_gex"]
    piv=d.groupby(["strike","expiry"])["net_gex"].sum().reset_index()
    mat=piv.pivot(index="strike",columns="expiry",values="net_gex").fillna(0).sort_index()
    strikes=mat.index.tolist(); expiries=[str(e)[-5:] for e in mat.columns.tolist()]; Z=mat.values
    sfut=[round(s*ratio,2) for s in strikes]
    htexts=[]
    for si,s in enumerate(strikes):
        row=[]
        for ei,e in enumerate(expiries):
            gv=float(Z[si,ei]) if si<Z.shape[0] and ei<Z.shape[1] else 0.0
            row.append(f"Strike: ${s:.0f}<br>{und}: {sfut[si]:.1f}<br>Expiry: {e}<br>GEX: {gv:.4f}B<br>{'▲ Long Γ' if gv>0.0001 else ('▼ Short Γ' if gv<-0.0001 else '◆ Neutral')}")
        htexts.append(row)
    gex_cs=[[0.00,"#FF0040"],[0.12,"#CC0030"],[0.25,"#660015"],[0.40,"#1A0006"],[0.48,"#080002"],
             [0.50,BG],[0.52,"#001505"],[0.60,"#004020"],[0.75,"#007040"],[0.88,"#00CC77"],[1.00,"#00FF99"]]
    ax3d=dict(backgroundcolor=T["bg1"],gridcolor=T["line2"],gridwidth=1,showspikes=False,zeroline=False,
              tickfont=dict(size=7.5,color=T["chart_t3"],family="JetBrains Mono"))
    atm=min(range(len(strikes)),key=lambda i:abs(strikes[i]-spot))
    atmz=[float(Z[atm,ei]) if atm<Z.shape[0] else 0.0 for ei in range(len(expiries))]
    fig=go.Figure(go.Surface(x=strikes,y=expiries,z=Z,colorscale=gex_cs,zmid=0,showscale=True,opacity=0.94,
        text=htexts,hovertemplate="%{text}<extra></extra>",
        colorbar=dict(title=dict(text="GEX $B",font=dict(size=9,color=T["chart_t3"])),
                      tickfont=dict(size=8,color=T["chart_t3"],family="JetBrains Mono"),thickness=10,len=0.65),
        lighting=dict(ambient=0.65,diffuse=0.9,specular=0.15,roughness=0.5),
        contours=dict(z=dict(show=True,usecolormap=True,highlightcolor="rgba(255,255,255,0.35)",highlightwidth=1.5))))
    fig.add_trace(go.Scatter3d(x=[strikes[atm]]*len(expiries),y=expiries,z=atmz,mode="lines+markers",
        line=dict(color="#FFD700",width=4),marker=dict(size=3,color="#FFD700"),
        hovertemplate=f"SPOT ${spot:.0f}→{und}:{spot*ratio:.0f}<br>%{{y}}<br>GEX:%{{z:.4f}}B<extra>ATM</extra>"))
    fig.update_layout(paper_bgcolor=BG,height=520,margin=dict(l=0,r=10,b=0,t=38),
        scene=dict(
            xaxis=dict(title=dict(text=f"Strike ({und} on hover)",font=dict(color=T["chart_t3"],size=9)),**ax3d),
            yaxis=dict(title=dict(text="Expiry",font=dict(color=T["chart_t3"],size=9)),**ax3d),
            zaxis=dict(title=dict(text="Net GEX ($B)",font=dict(color=T["chart_t3"],size=9)),
                       zerolinecolor=T["line2"],zerolinewidth=1,**ax3d),
            camera=dict(eye=dict(x=-1.6,y=-1.5,z=0.9),up=dict(x=0,y=0,z=1)),
            dragmode="orbit",aspectmode="manual",aspectratio=dict(x=2.2,y=0.9,z=0.7),bgcolor=T["bg1"]),
        showlegend=False,font=dict(family="JetBrains Mono, monospace",color=T["chart_t2"],size=9),
        title=dict(text=f"<b>{c['opt']}→{und}</b>  ·  3D GEX Surface  ·  Strike × Expiry",
            font=dict(size=11,color=T["t1"],family="JetBrains Mono"),x=0.01,xanchor="left"))
    return fig

def _fig_greek_heatmap(asset,greek_col,greek_label,cs,T):
    c=_ACFG.get(asset,_ACFG["SPX"]); BG=T["bg"]; und=c["und"]
    lvls=_agex(asset)
    if not lvls or "raw_df" not in lvls or lvls["raw_df"].empty:
        fig=go.Figure(); fig.add_annotation(text=f"No {greek_label} data",showarrow=False,font=dict(color=T["t3"],size=12))
        fig.update_layout(template="none",paper_bgcolor=BG,height=420); return fig
    raw_df=lvls["raw_df"]; spot=lvls["spot_opt"]; ratio=lvls.get("ratio",1.0)
    d=raw_df.copy(); d["dte_int"]=d["dte"].fillna(0).astype(int)
    d=d[d["dte_int"]<=4]; d=d[(d["strike"]>=spot*0.91)&(d["strike"]<=spot*1.09)]
    if d.empty:
        fig=go.Figure(); fig.add_annotation(text=f"No 0-4DTE {greek_label} data",showarrow=False,font=dict(color=T["t3"],size=12))
        fig.update_layout(template="none",paper_bgcolor=BG,height=380); return fig
    d["_v"]=d[greek_col]*d["open_interest"]*100
    piv=d.groupby(["strike","dte_int"])["_v"].sum().reset_index()
    mat=piv.pivot(index="strike",columns="dte_int",values="_v").fillna(0)
    mat=mat.reindex(columns=sorted(mat.columns)).sort_index()
    strikes=mat.index.tolist(); dte_cols=mat.columns.tolist(); Z=mat.values
    sfut=[round(s*ratio,2) for s in strikes]; clbls=[f"{d}DTE" for d in dte_cols]
    ht=[]
    for si in range(len(strikes)):
        row=[]
        for ei in range(len(dte_cols)):
            gv=float(Z[si,ei]) if si<Z.shape[0] and ei<Z.shape[1] else 0.0
            row.append(f"Strike: ${strikes[si]:.0f}<br>{und}: {sfut[si]:.1f}<br>DTE: {dte_cols[ei]}<br>{greek_label}: {gv:.5f}")
        ht.append(row)
    fig=go.Figure(go.Heatmap(z=Z,x=clbls,y=strikes,colorscale=cs,zmid=0,zsmooth="best",
        text=ht,hovertemplate="%{text}<extra></extra>",
        colorbar=dict(title=dict(text=greek_label,font=dict(size=9,color=T["chart_t3"])),
                      tickfont=dict(size=8,color=T["chart_t3"],family="JetBrains Mono"),thickness=10,len=0.70),
        xgap=2,ygap=0.5))
    fig.add_hline(y=spot,line_dash="solid",line_color=T["t1"],line_width=2,
        annotation_text=f"  SPOT ${spot:.0f}|{und} {spot*ratio:.1f}",
        annotation_font=dict(size=9,color=T["t1"],family="JetBrains Mono"),annotation_position="right")
    fig.update_layout(template="none",paper_bgcolor=BG,plot_bgcolor=BG,
        font=dict(family="JetBrains Mono, monospace",color=T["chart_t2"],size=10),height=460,
        margin=dict(t=38,r=130,b=50,l=80),
        title=dict(text=f"<b>{c['opt']}</b>  ·  {greek_label} Exposure  ·  0–4 DTE  ·  Strike + {und} Value",
            font=dict(size=11,color=T["t1"],family="JetBrains Mono"),x=0.01,xanchor="left"),
        xaxis=dict(title=dict(text="DTE",font=dict(size=9,color=T["chart_t3"])),
                   tickfont=dict(size=10,family="JetBrains Mono"),side="top"),
        yaxis=dict(title=dict(text=f"Strike (opt) | {und} value",font=dict(size=9,color=T["chart_t3"])),
                   tickfont=dict(size=9,family="JetBrains Mono"),tickformat="$.0f",range=[spot*0.91,spot*1.09]),
        hoverlabel=dict(bgcolor=T["bg2"],bordercolor=T["line_bright"],font=dict(family="JetBrains Mono",size=10,color=T["t1"])))
    return fig

def _render_gex_four_models(asset,spot_price,df,raw_df,equiv_label,equiv_mult,
                             gamma_flip,call_wall,put_wall,max_pain,vol_trigger,mom_wall,mom_val,T):
    c=_ACFG.get(asset,_ACFG["SPX"]); und=c["und"]
    lvls=_agex(asset); ratio=lvls.get("ratio",1.0) if lvls else 1.0
    spot_fut=lvls.get("spot_fut",round(spot_price*ratio,2)) if lvls else spot_price

    kl=[
        (f"{und} LAST",f"{spot_fut:,.2f}",T["green"]),
        ("Zero Γ",f"{lvls.get('gamma_flip',round(gamma_flip*ratio,2)):,.1f}" if lvls else f"{gamma_flip:.1f}",T["amber"]),
        ("Call Wall",f"{lvls.get('call_wall',round(call_wall*ratio,2)):,.1f}" if lvls else f"{call_wall:.1f}",T["green"]),
        ("Put Wall",f"{lvls.get('put_wall',round(put_wall*ratio,2)):,.1f}" if lvls else f"{put_wall:.1f}",T["red"]),
        ("Max Pain",f"{lvls.get('max_pain',round(max_pain*ratio,2)):,.1f}" if lvls else f"{max_pain:.1f}",T["violet"]),
        ("Vol Trig",f"{lvls.get('vol_trigger',round(vol_trigger*ratio,2)):,.1f}" if lvls else f"{vol_trigger:.1f}",T["blue"]),
    ]
    cards="".join([f'<div style="flex:1;min-width:90px;background:{T["bg1"]};border:1px solid {T["line2"]};border-top:2px solid {col};border-radius:4px;padding:8px 12px;"><div style="font-size:7.5px;color:{T["t3"]};letter-spacing:1.5px;text-transform:uppercase;margin-bottom:3px;">{lbl}</div><div style="font-family:JetBrains Mono,monospace;font-size:14px;font-weight:600;color:{col};">{val}</div></div>' for lbl,val,col in kl])
    st.markdown(f'<div style="display:flex;gap:6px;margin-bottom:14px;flex-wrap:wrap;">{cards}</div>',unsafe_allow_html=True)

    lsz_c1,lsz_c2,lsz_c3,_=st.columns([1,1,1,5])
    cur_lsz=st.session_state.get("gex_label_size","Big")
    for _lc,_lsz in zip([lsz_c1,lsz_c2,lsz_c3],["Big","Bigger","Biggest"]):
        with _lc:
            if st.button(_lsz,key=f"lsz_{_lsz}_{asset}",type="primary" if cur_lsz==_lsz else "secondary",use_container_width=True):
                st.session_state["gex_label_size"]=_lsz

    # MODEL A
    st.markdown(f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;color:{T["t3"]};letter-spacing:2px;text-transform:uppercase;padding:10px 0 6px 0;border-bottom:1px solid {T["line2"]};margin-bottom:10px;">📊 A  ·  LIVE GEX HEATMAP + {c["label"]}  ·  Options: {c["opt"]}</div>',unsafe_allow_html=True)
    tf_c1,tf_c2,tf_c3=st.columns([2,2,4])
    with tf_c1: tf=st.selectbox("TF",["1m","2m","5m","30m","1h","4h"],index=2,key=f"tf_{asset}",label_visibility="collapsed")
    with tf_c2: st.markdown(f'<div style="font-family:JetBrains Mono,monospace;font-size:9px;color:{T["t3"]};padding:8px 0;">ratio ×{ratio:.4f}  ·  {c["opt"]}→{und}</div>',unsafe_allow_html=True)
    with tf_c3:
        if st.button("⟳  Refresh",key=f"ref_{asset}"): st.cache_data.clear()
    st.plotly_chart(_fig_heatmap_candle(asset,tf,T),use_container_width=True,config={
        "displayModeBar":True,"displaylogo":False,"scrollZoom":False,
        "modeBarButtonsToRemove":["sendDataToCloud","lasso2d","select2d","toImage","zoom2d","zoomIn2d","zoomOut2d","autoScale2d"]})

    # MODEL B
    st.markdown(f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;color:{T["t3"]};letter-spacing:2px;text-transform:uppercase;padding:10px 0 6px 0;border-bottom:1px solid {T["line2"]};margin-bottom:10px;">🌐 B  ·  3D GAMMA EXPOSURE SURFACE  ·  {c["opt"]}→{und}  ·  Strike × Expiry</div>',unsafe_allow_html=True)
    st.plotly_chart(_fig_gex_3d(asset,T),use_container_width=True,config={
        "displayModeBar":True,"displaylogo":False,"scrollZoom":False,
        "modeBarButtonsToRemove":["sendDataToCloud","toImage","zoom3d","pan3d","resetCameraLastSave3d"],
        "modeBarButtonsToAdd":["orbitRotation","resetCameraDefault3d"]})

    # MODEL C — Vanna
    st.markdown(f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;color:{T["t3"]};letter-spacing:2px;text-transform:uppercase;padding:10px 0 6px 0;border-bottom:1px solid {T["line2"]};margin-bottom:10px;">🔷 C  ·  VANNA EXPOSURE  ·  0–4 DTE  ·  Strike + {und} Value</div>',unsafe_allow_html=True)
    vcs=[[0.00,"#CC1100"],[0.20,"#882200"],[0.38,"#331100"],[0.46,"#0A0400"],[0.50,T["bg"]],[0.54,"#001130"],[0.62,"#002266"],[0.80,"#0055CC"],[1.00,"#44AAFF"]]
    st.plotly_chart(_fig_greek_heatmap(asset,"vanna","Vanna",vcs,T),use_container_width=True,config={
        "displayModeBar":True,"displaylogo":False,"modeBarButtonsToRemove":["sendDataToCloud","lasso2d","select2d","zoom2d"]})

    # MODEL D — Charm
    st.markdown(f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;color:{T["t3"]};letter-spacing:2px;text-transform:uppercase;padding:10px 0 6px 0;border-bottom:1px solid {T["line2"]};margin-bottom:10px;">🔶 D  ·  CHARM EXPOSURE  ·  0–4 DTE  ·  Strike + {und} Value</div>',unsafe_allow_html=True)
    ccs=[[0.00,"#FF0040"],[0.15,"#CC0030"],[0.30,"#660015"],[0.43,"#2A0008"],[0.49,"#050001"],[0.50,T["bg"]],[0.51,"#001505"],[0.57,"#003315"],[0.70,"#006633"],[0.85,"#00CC77"],[1.00,"#00FF99"]]
    st.plotly_chart(_fig_greek_heatmap(asset,"charm","Charm",ccs,T),use_container_width=True,config={
        "displayModeBar":True,"displaylogo":False,"modeBarButtonsToRemove":["sendDataToCloud","lasso2d","select2d","zoom2d"]})

    # Topology
    st.markdown(f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;color:{T["t3"]};letter-spacing:2px;text-transform:uppercase;padding:10px 0 6px 0;border-bottom:1px solid {T["line2"]};margin-bottom:10px;">🗺️  GEX TOPOLOGY  ·  Intraday Decay</div>',unsafe_allow_html=True)
    lf=build_gex_landscape(df,spot_price)
    if lf: st.plotly_chart(lf,use_container_width=True,config={"displayModeBar":True,"displaylogo":False,"scrollZoom":False,"modeBarButtonsToRemove":["sendDataToCloud","toImage","zoom3d","pan3d","resetCameraLastSave3d"],"modeBarButtonsToAdd":["orbitRotation","resetCameraDefault3d"]})


# ═══════════════════════════════════════════════════════════════════════════════
# NEW: MISPRICINGS & MACRO PAGE
# ═══════════════════════════════════════════════════════════════════════════════
@st.cache_data(ttl=3600,show_spinner=False)
def _fred(series_id,limit=30):
    try:
        ...
        if r.ok:
            ...
            s=pd.Series({o["date"]:float(o["value"]) for o in obs if o["value"]!="."},dtype=float)
            return s.sort_index().tail(limit)
    except: pass
    try:
        from io import StringIO
        r2=_requests.get(f"https://fred.stlouisfed.org/graph/fredgraph.csv?id={series_id}",headers={"User-Agent":"Mozilla/5.0"},timeout=10)
        if r2.ok:
            df=pd.read_csv(StringIO(r2.text),parse_dates=["DATE"],index_col="DATE")
            return pd.to_numeric(df[df.columns[0]],errors="coerce").dropna().tail(limit)
    except: pass
    return pd.Series(dtype=float)
...
def _sofr():
    try:
        r=_requests.get("https://markets.newyorkfed.org/api/rates/sofr/last/1.json",headers={"User-Agent":"Mozilla/5.0"},timeout=8)
        if r.ok: return float(r.json()["refRates"][0]["percentRate"])
    except: pass
    return None

@st.cache_data(ttl=3600,show_spinner=False)
def _macro_engine(asset="SPX"):
    out={"regime":"UNKNOWN","growth_z":0.0,"infl_z":0.0,"cpi_yoy":0.0,"unemployment":0.0,
         "hy_oas":0.0,"spread_10_2":0.0,"vrp":0.0,"composite":0,"thesis":"NEUTRAL",
         "risk_factors":[],"signals":{},"cpi_forecast":None,
         "directional":{"ES":50,"NQ":50,"GC":50,"DXY":50},"regime_desc":"","strategies":[],
         "sofr":None,"nfci":None}
    try:
        un=_fred("UNRATE",65)
        if not un.empty:
            mu=un.tail(60).mean(); sg=un.tail(60).std() or 1.0
            out["growth_z"]=float(-(un.iloc[-1]-mu)/sg); out["unemployment"]=float(un.iloc[-1])
        cp=_fred("CPIAUCSL",65)
        if not cp.empty:
            yoy=cp.pct_change(12).dropna()*100
            if not yoy.empty:
                mu_c=yoy.tail(60).mean(); sg_c=yoy.tail(60).std() or 1.0
                out["infl_z"]=float((yoy.iloc[-1]-mu_c)/sg_c); out["cpi_yoy"]=float(yoy.iloc[-1])
                if len(yoy)>=3:
                    slope=float(np.polyfit(range(min(6,len(yoy))),yoy.tail(6).values,1)[0])
                    out["cpi_forecast"]=round(float(yoy.iloc[-1])+slope,2)
        hy=_fred("BAMLH0A0HYM2",5)
        if not hy.empty: out["hy_oas"]=float(hy.iloc[-1])
        sp=_fred("T10Y2Y",5)
        if not sp.empty: out["spread_10_2"]=float(sp.iloc[-1])
        nf=_fred("NFCI",5)
        if not nf.empty: out["nfci"]=float(nf.iloc[-1])
        out["sofr"]=_sofr()
        try:
            vix=_fetch_yahoo_price("^VIX")
            rh=_requests.get("https://query1.finance.yahoo.com/v8/finance/chart/%5EGSPC?interval=1d&range=2mo",headers={"User-Agent":"Mozilla/5.0","Accept":"application/json"},timeout=10)
            rv21=18.0
            if rh.ok:
                cls=[x for x in rh.json()["chart"]["result"][0]["indicators"]["quote"][0].get("close",[]) if x]
                if len(cls)>=22: rv21=float(np.std(np.diff(np.log(cls[-22:])))*np.sqrt(252)*100)
            out["vrp"]=float((vix/100)**2-(rv21/100)**2)
            out["signals"]["vix"]=round(vix,2); out["signals"]["rv21"]=round(rv21,2)
        except: out["vrp"]=0.01
        gz=out["growth_z"]; iz=out["infl_z"]
        if   gz>0.3  and iz<0.3:  out["regime"]="GOLDILOCKS"
        elif gz>0.3  and iz>=0.3: out["regime"]="REFLATION"
        elif gz<=0.0 and iz>=0.3: out["regime"]="STAGFLATION"
        else:                       out["regime"]="DEFLATION / NEUTRAL"
        score=0
        score+=(2 if out["vrp"]>0.001 else -1)
        out["signals"]["vrp"]=("POSITIVE — vol overpriced" if out["vrp"]>0.001 else "NEGATIVE — RV>IV")
        score+=int(np.clip(gz*2,-3,3)); score-=int(np.clip(iz*1.5,0,3))
        hy_v=out["hy_oas"]
        if   hy_v>5.0: score-=2; out["signals"]["hy"]=f"ELEVATED {hy_v:.2f}bps — stress"
        elif hy_v>3.5: score-=1; out["signals"]["hy"]=f"MODERATE {hy_v:.2f}bps"
        else:          score+=1; out["signals"]["hy"]=f"TIGHT {hy_v:.2f}bps — benign"
        sp_v=out["spread_10_2"]
        if   sp_v<0:    score-=2; out["signals"]["curve"]="INVERTED — recession signal"
        elif sp_v<0.25: score-=1; out["signals"]["curve"]=f"FLAT +{sp_v:.2f}%"
        else:           score+=1; out["signals"]["curve"]=f"NORMAL +{sp_v:.2f}%"
        if out["nfci"] and out["nfci"]>0.1: score-=1; out["signals"]["nfci"]=f"TIGHT {out['nfci']:.3f}"
        elif out["nfci"] and out["nfci"]<-0.3: score+=1; out["signals"]["nfci"]=f"LOOSE {out['nfci']:.3f}"
        out["composite"]=int(np.clip(score,-10,10)); c2=out["composite"]
        if   c2>= 4: out["thesis"]="STRONG BULLISH"
        elif c2>= 2: out["thesis"]="LEAN BULLISH"
        elif c2<=-4: out["thesis"]="STRONG BEARISH"
        elif c2<=-2: out["thesis"]="LEAN BEARISH"
        else:        out["thesis"]="NEUTRAL / WAIT"
        b=float(np.clip(50+c2*3.5,5,95))
        if   asset=="SPX": out["directional"]={"ES":round(np.clip(b+gz*5-iz*3,5,95),1),"NQ":round(np.clip(b+gz*6-iz*2,5,95),1),"GC":round(np.clip(50+iz*8-gz*3,5,95),1),"DXY":round(np.clip(50+iz*5-gz*3,5,95),1)}
        elif asset=="NDX": out["directional"]={"NQ":round(np.clip(b+gz*6-iz*2,5,95),1),"ES":round(np.clip(b+gz*5-iz*3,5,95),1),"GC":round(np.clip(50+iz*8-gz*3,5,95),1),"DXY":round(np.clip(50+iz*5-gz*3,5,95),1)}
        elif asset=="GLD": out["directional"]={"GC":round(np.clip(50+iz*8-gz*4+(sp_v<0)*10,5,95),1),"ES":round(b,1),"NQ":round(np.clip(b+1,5,95),1),"DXY":round(np.clip(50+iz*5-gz*3,5,95),1)}
        rd={"GOLDILOCKS":"Growth above trend, inflation contained. Dealer long-gamma stabilises. VRP positive — premium selling has edge. Pullbacks bought.","REFLATION":"Growth + inflation rising. Commodities outperform. Fed tightening risk elevates. Monitor GEX flip levels.","STAGFLATION":"Dangerous — high inflation + slowing growth. Bearish equities, bullish gold. Dealer short-gamma amplifies moves. Hedge tail risk.","DEFLATION / NEUTRAL":"Muted backdrop. Bonds benefit. GEX flip levels critical. Wait for directional confirmation."}
        st_map={"GOLDILOCKS":["✅ Buy dips ES/NQ — dealer absorption supports","✅ Sell premium in compressed-vol regime","⚠️ Respect Call Wall resistance"],"REFLATION":["✅ Long GC/GLD inflation hedge","✅ ES/NQ longs at Zero-Gamma or Put Wall","⚠️ Reduce short-vol — RV rising"],"STAGFLATION":["✅ Long GC at GEX support","✅ Long puts / long vol — negative VRP","❌ No naked ES/NQ upside","❌ No naked premium selling"],"DEFLATION / NEUTRAL":["✅ Range-trade ES between walls","✅ Theta if VRP positive + long-gamma","⚠️ Monitor NFCI weekly"]}
        out["regime_desc"]=rd.get(out["regime"],""); out["strategies"]=st_map.get(out["regime"],[])
        rf=[]
        if iz>0.5: rf.append("⚠️ Elevated CPI Z — hawkish Fed tail")
        if hy_v>5.0: rf.append("⚠️ HY OAS elevated — credit stress")
        if sp_v<0: rf.append("⚠️ Inverted curve — recession signal")
        if out["vrp"]<-0.001: rf.append("⚠️ Negative VRP — vol cheap vs realized")
        if out["nfci"] and out["nfci"]>0.2: rf.append("⚠️ NFCI positive — conditions tightening")
        out["risk_factors"]=rf
    except Exception as e: out["signals"]["error"]=str(e)
    return out

def render_mispricings_macro_page():
    _load_calibrated(); T=get_theme()
    _BG="#050810"; _GRID="rgba(80,120,200,0.14)"; _TXT="#C0D8F0"; _TXT3="#3A5070"
    _PH=dict(template="none",paper_bgcolor=_BG,plot_bgcolor=_BG,font=dict(family="JetBrains Mono, monospace",color=_TXT,size=10),showlegend=False,hovermode="closest")
    st.markdown(f'<div style="display:flex;align-items:baseline;gap:14px;padding:14px 0 10px 0;border-bottom:1px solid {T["line2"]};margin-bottom:14px;"><span style="font-family:Barlow Condensed,sans-serif;font-size:22px;font-weight:700;letter-spacing:3px;text-transform:uppercase;color:{T["t1"]};">🔬 MISPRICINGS &amp; MACRO</span><span style="font-family:JetBrains Mono,monospace;font-size:10px;color:{T["t3"]};letter-spacing:1px;">IV Surface · Vol Skew · Predictive Engine · FRED · VRP</span></div>',unsafe_allow_html=True)
    ma_cols=st.columns([1,1,1,5]); macro_asset=st.session_state.get("macro_asset","SPX")
    for _mc,_ma in zip(ma_cols[:3],["SPX","NDX","GLD"]):
        with _mc:
            if st.button(_ma,key=f"ma_{_ma}",type="primary" if macro_asset==_ma else "secondary",use_container_width=True):
                st.session_state["macro_asset"]=_ma; macro_asset=_ma
    macro=_macro_engine(macro_asset)
    rc_map={"GOLDILOCKS":T["green"],"REFLATION":T["amber"],"STAGFLATION":T["red"],"DEFLATION / NEUTRAL":T["blue"],"UNKNOWN":T["t3"]}
    rc=rc_map.get(macro["regime"],T["t1"])
    th_col=T["green"] if "BULL" in macro["thesis"] else T["red"] if "BEAR" in macro["thesis"] else T["amber"]
    sc_col=T["green"] if macro["composite"]>0 else T["red"] if macro["composite"]<0 else T["amber"]
    st.markdown(f'<div style="background:{T["bg1"]};border:1px solid {T["line_bright"]};border-top:3px solid {T["amber"]};border-radius:6px;padding:12px 18px;margin-bottom:14px;"><div style="font-family:JetBrains Mono,monospace;font-size:8px;font-weight:700;color:{T["amber"]};letter-spacing:2.5px;text-transform:uppercase;margin-bottom:10px;">⚡ REAL-TIME MACRO HUD  ·  {macro_asset}</div></div>',unsafe_allow_html=True)
    hud=st.columns([2,2,1,1,1,1,1])
    with hud[0]: st.markdown(f'<div style="text-align:center"><div style="font-size:7.5px;color:{T["t3"]};letter-spacing:1.5px;text-transform:uppercase;">Regime</div><div style="font-family:Barlow Condensed,sans-serif;font-size:18px;font-weight:700;color:{rc};">{macro["regime"]}</div></div>',unsafe_allow_html=True)
    with hud[1]: st.markdown(f'<div style="text-align:center"><div style="font-size:7.5px;color:{T["t3"]};letter-spacing:1.5px;text-transform:uppercase;">Thesis</div><div style="font-family:Barlow Condensed,sans-serif;font-size:18px;font-weight:700;color:{th_col};">{macro["thesis"]}</div></div>',unsafe_allow_html=True)
    with hud[2]: st.markdown(f'<div style="text-align:center"><div style="font-size:7.5px;color:{T["t3"]};letter-spacing:1px;text-transform:uppercase;">Score</div><div style="font-family:JetBrains Mono,monospace;font-size:22px;font-weight:600;color:{sc_col};">{macro["composite"]:+d}</div></div>',unsafe_allow_html=True)
    for hi,(ins,lbl) in zip(hud[3:],[("ES","ES"),("NQ","NQ"),("GC","GC"),("DXY","DXY")]):
        p=macro["directional"].get(ins,50); col=T["green"] if p>=55 else T["red"] if p<=45 else T["amber"]
        dlbl="BULL" if p>=55 else "BEAR" if p<=45 else "NEUT"
        with hi: st.markdown(f'<div style="text-align:center"><div style="font-size:7.5px;color:{T["t3"]};letter-spacing:1px;text-transform:uppercase;">{lbl}</div><div style="font-family:JetBrains Mono,monospace;font-size:16px;font-weight:600;color:{col};">{dlbl}</div><div style="font-size:8px;color:{col};">{p:.0f}%</div></div>',unsafe_allow_html=True)
    dc=st.columns(5)
    dcards=[("CPI Forecast",f"{macro['cpi_forecast']:.2f}%" if macro['cpi_forecast'] else "—",T["red"] if (macro['cpi_forecast'] or 0)>3.0 else T["green"]),("SOFR",f"{macro['sofr']:.3f}%" if macro['sofr'] else "—",T["blue"]),("NFCI",f"{macro['nfci']:.3f}" if macro['nfci'] else "—",T["red"] if (macro['nfci'] or 0)>0 else T["green"]),("VRP",f"{macro['vrp']*100:+.3f}%",T["green"] if macro['vrp']>0 else T["red"]),("HY OAS",f"{macro['hy_oas']:.2f}bps",T["red"] if macro['hy_oas']>5 else T["green"])]
    for _dc,(lbl,val,col) in zip(dc,dcards):
        with _dc: st.markdown(f'<div style="background:{T["bg1"]};border:1px solid {T["line2"]};border-top:2px solid {col};border-radius:3px;padding:7px 10px;margin-bottom:8px;"><div style="font-size:7.5px;color:{T["t3"]};letter-spacing:1.5px;text-transform:uppercase;margin-bottom:3px;">{lbl}</div><div style="font-family:Barlow Condensed,sans-serif;font-size:20px;font-weight:700;color:{col};">{val}</div></div>',unsafe_allow_html=True)
    if macro["regime_desc"] or macro["strategies"]:
        st.markdown(f'<div style="background:{T["bg1"]};border:1px solid {T["line2"]};border-left:3px solid {rc};border-radius:4px;padding:10px 14px;margin-bottom:12px;"><div style="font-family:JetBrains Mono,monospace;font-size:7.5px;font-weight:700;color:{rc};letter-spacing:2px;text-transform:uppercase;margin-bottom:7px;">STRATEGIES  ·  {macro_asset}  ·  {macro["regime"]}</div><div style="font-size:8.5px;color:{T["t3"]};line-height:1.6;margin-bottom:8px;">{macro["regime_desc"]}</div>'+"".join([f'<div style="font-family:Barlow,sans-serif;font-size:9px;color:{T["t2"]};padding:2px 0;">{s}</div>' for s in macro["strategies"]])+'</div>',unsafe_allow_html=True)
    for rf in macro.get("risk_factors",[]):
        st.markdown(f'<div style="padding:5px 12px;background:{T["bg1"]};border:1px solid {T["line2"]};border-left:3px solid {T["amber"]};border-radius:0 3px 3px 0;margin-bottom:4px;font-family:Barlow,sans-serif;font-size:9px;color:{T["t2"]};">{rf}</div>',unsafe_allow_html=True)
    if macro["signals"]:
        st.markdown(f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;color:{T["t3"]};letter-spacing:2px;text-transform:uppercase;margin:12px 0 6px 0;">SIGNAL BREAKDOWN</div>',unsafe_allow_html=True)
        for sk,sv in macro["signals"].items():
            if sk=="error": continue
            scol=T["green"] if any(w in str(sv).upper() for w in ["POSITIVE","TIGHT","NORMAL","LOOSE"]) else T["red"]
            st.markdown(f'<div style="display:flex;align-items:center;gap:8px;padding:5px 10px;border-left:3px solid {scol};margin-bottom:4px;background:{T["bg1"]};border-radius:0 3px 3px 0;"><span style="font-family:JetBrains Mono,monospace;font-size:9px;color:{T["t3"]};width:70px;flex-shrink:0;text-transform:uppercase;">{sk.upper()}</span><span style="font-family:JetBrains Mono,monospace;font-size:9px;color:{scol};">{sv}</span></div>',unsafe_allow_html=True)
    if st.button("⟳  Refresh Macro",key="macro_ref"): st.cache_data.clear()
    st.markdown("<div style='height:20px'></div>",unsafe_allow_html=True)

    # VOL LAB SECTION
    st.markdown(f'<div style="display:flex;align-items:baseline;gap:14px;padding:12px 0 8px 0;border-bottom:1px solid #0D2040;margin-bottom:12px;"><span style="font-family:Barlow Condensed,sans-serif;font-size:18px;font-weight:700;letter-spacing:3px;text-transform:uppercase;color:#44AAFF;">VOLATILITY LAB</span><span style="font-family:JetBrains Mono,monospace;font-size:9px;color:{_TXT3};letter-spacing:1px;">Heston IV Surface · Vol Skew · Auto-calibrated</span></div>',unsafe_allow_html=True)
    cc1,cc2,cc3=st.columns([2,2,2])
    _vl_syms=["SPX","NDX","GLD"]; _vl_def=_vl_syms.index(macro_asset) if macro_asset in _vl_syms else 0
    with cc1: vl_sym=st.selectbox("Symbol",_vl_syms,index=_vl_def,key="vl_sym_mm",label_visibility="collapsed")
    with cc2: vl_dte=st.slider("Skew DTE",1,60,7,key="vl_dte_mm",format="%d DTE")
    with cc3:
        if st.button("⟳  Recalibrate",key="vl_recal_mm"): st.cache_data.clear()
    hd=_vol_lab_calibrate(vl_sym)
    if not hd or "params" not in hd: st.warning("Calibration loading…"); return
    params=hd["params"]; mono_g=hd.get("moneyness",[round(0.75+i*0.025,3) for i in range(22)])
    exp_lbls=hd.get("expiries",["7d","14d","30d","45d","60d","90d","120d","180d"])
    exp_days=[int(e.replace("d","")) for e in exp_lbls]
    surf_arr=np.array(hd.get("surface",[]),dtype=float)
    spot_h=float(hd.get("spot",6800.0))
    v0,kappa,theta,xi,rho=params["v0"],params["kappa"],params["theta"],params["xi"],params["rho"]
    fok=feller_condition(kappa,theta,xi)
    if surf_arr.ndim!=2 or surf_arr.shape[0]!=len(exp_lbls): surf_arr=_heston_iv_grid(spot_h,params,mono_g,exp_days)
    atm_idx=min(range(len(mono_g)),key=lambda i:abs(mono_g[i]-1.0))
    for _col,(lbl,val,col) in zip(st.columns(5),[("v₀ init var",f"{v0:.4f}","#44AAFF"),("κ mean-rev",f"{kappa:.3f}","#44AAFF"),("θ long-run",f"{theta:.4f}","#44AAFF"),("ξ vol-of-vol",f"{xi:.3f}","#FFB800"),("ρ spot-vol",f"{rho:.3f}","#FF6644")]):
        with _col: st.markdown(f'<div style="background:{_BG};border:1px solid #0D2040;border-top:2px solid {col};border-radius:3px;padding:7px 10px;margin-bottom:8px;"><div style="font-size:8px;color:{_TXT3};letter-spacing:1.5px;text-transform:uppercase;margin-bottom:3px;">{lbl}</div><div style="font-family:Barlow Condensed,sans-serif;font-size:20px;font-weight:700;color:{col};">{val}</div></div>',unsafe_allow_html=True)

    # IV Surface - Strike × DTE, no tabs
    st.markdown(f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;color:{_TXT3};letter-spacing:2px;text-transform:uppercase;padding:10px 0 8px 0;border-bottom:1px solid #0D2040;margin-bottom:12px;">🌐 IMPLIED VOLATILITY SURFACE  ·  {vl_sym}  ·  Strike × DTE</div>',unsafe_allow_html=True)
    strike_lbls=[f"${round(spot_h*m,0):.0f}" for m in mono_g]
    _SURF_CS=[[0.00,"#050810"],[0.08,"#041530"],[0.18,"#062880"],[0.30,"#0844CC"],[0.42,"#2266FF"],[0.54,"#6644DD"],[0.65,"#AA22BB"],[0.76,"#DD1188"],[0.87,"#FF2244"],[0.94,"#FF5522"],[1.00,"#FFE040"]]
    _ax3=dict(backgroundcolor=_BG,gridcolor=_GRID,gridwidth=1,showspikes=False,zeroline=False,tickfont=dict(size=7.5,color=_TXT3,family="JetBrains Mono"))
    z3=surf_arr.tolist(); vmin=float(np.nanmin(surf_arr)) if not np.all(np.isnan(surf_arr)) else 0.0; vmax=float(np.nanmax(surf_arr)) if not np.all(np.isnan(surf_arr)) else 50.0
    fig3=go.Figure(go.Surface(x=strike_lbls,y=exp_lbls,z=z3,colorscale=_SURF_CS,cmin=vmin,cmax=vmax,showscale=True,opacity=0.93,
        colorbar=dict(title=dict(text="IV %",font=dict(size=9,color=_TXT3,family="JetBrains Mono")),tickfont=dict(size=8,color=_TXT3,family="JetBrains Mono"),bgcolor=_BG,outlinewidth=0,thickness=10,len=0.72,x=1.01),
        hovertemplate=f"<b>{vl_sym}</b><br>Strike: <b>%{{x}}</b><br>Expiry: <b>%{{y}}</b><br>IV: <b>%{{z:.2f}}%</b><extra></extra>",
        lighting=dict(ambient=0.65,diffuse=0.9,specular=0.25,roughness=0.45),
        contours=dict(z=dict(show=True,usecolormap=True,highlightcolor="rgba(255,255,255,0.45)",highlightwidth=2))))
    atmz=[float(surf_arr[i,atm_idx]) if i<surf_arr.shape[0] and not np.isnan(surf_arr[i,atm_idx]) else 0.0 for i in range(len(exp_lbls))]
    fig3.add_trace(go.Scatter3d(x=[strike_lbls[atm_idx]]*len(exp_lbls),y=exp_lbls,z=atmz,mode="lines+markers",line=dict(color="#FFFFFF",width=2.5),marker=dict(size=3,color="#FFFFFF"),hovertemplate="ATM %{x}  DTE:%{y}  IV:%{z:.2f}%<extra>ATM</extra>"))
    fig3.update_layout(paper_bgcolor=_BG,height=580,margin=dict(l=0,r=10,b=0,t=38),
        scene=dict(xaxis=dict(title=dict(text="Strike Price ($)",font=dict(color=_TXT3,size=9)),**_ax3),yaxis=dict(title=dict(text="DTE",font=dict(color=_TXT3,size=9)),**_ax3),zaxis=dict(title=dict(text="IV %",font=dict(color=_TXT3,size=9)),**_ax3),camera=dict(eye=dict(x=-1.55,y=-1.35,z=0.85),up=dict(x=0,y=0,z=1)),aspectmode="manual",aspectratio=dict(x=2.0,y=1.0,z=0.72),bgcolor=_BG),
        title=dict(text=f"<b>{vl_sym} IV SURFACE</b>  ·  Spot ${spot_h:,.0f}  ·  {params.get('calibrated_on','—')}  ·  Feller {'✓' if fok else '✗'}",font=dict(size=11,color="#44AAFF",family="JetBrains Mono"),x=0.01,xanchor="left"),
        font=dict(family="JetBrains Mono, monospace",color=_TXT,size=9),showlegend=False)
    st.plotly_chart(fig3,use_container_width=True,config={"displayModeBar":True,"displaylogo":False,"scrollZoom":False,"modeBarButtonsToRemove":["sendDataToCloud","toImage","zoom3d","pan3d","resetCameraDefault3d"],"modeBarButtonsToAdd":["orbitRotation","resetCameraDefault3d"]})

    # IV Grid
    st.markdown(f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;color:{_TXT3};letter-spacing:2px;text-transform:uppercase;margin:10px 0 5px 0;">IV GRID  ·  DTE × STRIKE</div>',unsafe_allow_html=True)
    _show_m=[0.85,0.90,0.95,1.00,1.05,1.10,1.15]; _midxs=[min(range(len(mono_g)),key=lambda i:abs(mono_g[i]-m)) for m in _show_m]
    _tbl={}
    for di,dlbl in enumerate(exp_lbls):
        if di>=surf_arr.shape[0]: continue
        row={}
        for mv,mx in zip(_show_m,_midxs):
            v=(float(surf_arr[di,mx]) if mx<surf_arr.shape[1] and not np.isnan(surf_arr[di,mx]) else None)
            row[f"${round(spot_h*mv,0):.0f}"]=f"{v:.1f}%" if v else "—"
        _tbl[dlbl]=row
    if _tbl:
        _tdf=pd.DataFrame(_tbl).T; _tdf.index.name="DTE"
        st.dataframe(_tdf,use_container_width=True,height=max(36*len(_tdf)+40,100))

    # Vol Skew - same page below surface
    st.markdown("<div style='height:16px'></div>",unsafe_allow_html=True)
    st.markdown(f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;color:{_TXT3};letter-spacing:2px;text-transform:uppercase;padding:10px 0 8px 0;border-bottom:1px solid #0D2040;margin-bottom:12px;">📐 VOLATILITY SKEW  ·  {vl_sym}  ·  Market vs Heston</div>',unsafe_allow_html=True)
    c_str,c_iv,p_str,p_iv,sk_spot=_vol_lab_live_skew(vl_sym,vl_dte)
    T_skew=max(vl_dte,1)/365.0; _model_m=[round(0.75+i*0.02,3) for i in range(26)]; _model_ivs=[]
    for m in _model_m:
        K_=(sk_spot if sk_spot>0 else spot_h)*m; S_=sk_spot if sk_spot>0 else spot_h
        try:
            pr=heston_call_price(S_,K_,T_skew,params["r"],v0,kappa,theta,xi,rho)
            iv_=implied_vol(pr,S_,K_,T_skew,params["r"],0.0,"C"); _model_ivs.append((iv_ or 0.0)*100.0)
        except: _model_ivs.append(None)
    S_ref=sk_spot if sk_spot>0 else spot_h
    fig_sk=go.Figure()
    if c_str: fig_sk.add_trace(go.Scatter(x=c_str,y=c_iv,mode="markers",marker=dict(color="#44AAFF",size=5,opacity=0.8),name="Call IV (market)",hovertemplate="Strike $%{x:,.0f} · Call IV: %{y:.2f}%<extra></extra>"))
    if p_str: fig_sk.add_trace(go.Scatter(x=p_str,y=p_iv,mode="markers",marker=dict(color="#FF6644",size=5,opacity=0.8),name="Put IV (market)",hovertemplate="Strike $%{x:,.0f} · Put IV: %{y:.2f}%<extra></extra>"))
    fig_sk.add_trace(go.Scatter(x=[S_ref*m for m in _model_m],y=_model_ivs,mode="lines",name="Heston Model",line=dict(color="#FFB800",width=2.0),connectgaps=True,hovertemplate="Strike $%{x:,.0f} · Model IV: %{y:.2f}%<extra>Heston</extra>"))
    if S_ref>0: fig_sk.add_vline(x=S_ref,line_dash="dot",line_color=_TXT,line_width=1.2,annotation_text=f" Spot ${S_ref:,.0f}",annotation_font=dict(size=9,color=_TXT))
    fig_sk.update_layout(**_PH,height=420,showlegend=True,legend=dict(font=dict(size=9,family="JetBrains Mono",color=_TXT),bgcolor="rgba(0,0,0,0)",orientation="h",y=1.06),xaxis=dict(title=dict(text="Strike Price",font=dict(size=9,color=_TXT3)),gridcolor=_GRID,tickfont=dict(size=9),tickprefix="$"),yaxis=dict(title=dict(text="Implied Volatility %",font=dict(size=9,color=_TXT3)),gridcolor=_GRID,tickfont=dict(size=9)),margin=dict(t=48,r=18,b=50,l=58),title=dict(text=f"<b>{vl_sym} VOLATILITY SKEW</b>  ·  {vl_dte} DTE  ·  Spot ${S_ref:,.0f}  ·  Market vs Heston",font=dict(size=11,color="#44AAFF",family="JetBrains Mono"),x=0.01,xanchor="left"),hoverlabel=dict(bgcolor=_BG,bordercolor="#0D2040",font=dict(family="JetBrains Mono",size=10,color=_TXT)))
    st.plotly_chart(fig_sk,use_container_width=True,config={"displayModeBar":True,"displaylogo":False,"scrollZoom":True,"modeBarButtonsToRemove":["sendDataToCloud","lasso2d","select2d"]})
    if c_str and p_str and c_iv and p_iv:
        atm_call=min(zip(c_str,c_iv),key=lambda x:abs(x[0]-S_ref),default=(0,0))
        atm_put=min(zip(p_str,p_iv),key=lambda x:abs(x[0]-S_ref),default=(0,0))
        otm_put=min([(s,v) for s,v in zip(p_str,p_iv) if s<S_ref*0.95],key=lambda x:abs(x[0]-S_ref*0.92),default=(0,0))
        for _col,(lbl,val,col) in zip(st.columns(5),[("ATM Call IV",f"{atm_call[1]:.2f}%","#44AAFF"),("ATM Put IV",f"{atm_put[1]:.2f}%","#FF6644"),("OTM Put −8%",f"{otm_put[1]:.2f}%","#FF4422"),("Put Skew",f"{(otm_put[1]-atm_put[1]):+.2f}%","#FFB800"),("Feller",f"{'✓' if fok else '✗'} 2κθ={2*kappa*theta:.3f}","#44FF88" if fok else "#FF4422")]):
            with _col: st.markdown(f'<div style="background:{_BG};border:1px solid #0D2040;border-top:2px solid {col};border-radius:3px;padding:7px 10px;"><div style="font-size:8px;color:{_TXT3};letter-spacing:1.5px;text-transform:uppercase;margin-bottom:3px;">{lbl}</div><div style="font-family:Barlow Condensed,sans-serif;font-size:18px;font-weight:700;color:{col};">{val}</div></div>',unsafe_allow_html=True)
    st.markdown(f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;color:{T["t3"]};padding:8px 0;letter-spacing:1px;">FRED API key: a176b710... · NY Fed SOFR · Chicago Fed NFCI · VIX/RV VRP · Heston FFT · CBOE · {datetime.date.today()}</div>',unsafe_allow_html=True)
